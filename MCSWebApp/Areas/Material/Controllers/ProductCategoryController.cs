...
namespace MCSWebApp.Areas.Material.Controllers
{
    ...
    public class ProductCategoryController : BaseController
    {
        ...
        public IActionResult Index()
        {
            ...
            ViewBag.Breadcrumb = WebAppMenu.BreadcrumbText[WebAppMenu.ProductCategory];
            ViewBag.BreadcrumbCode = WebAppMenu.ProductCategory;
        ...
        public async Task<IActionResult> Detail(string Id)
        {
            ...
            try
            {
                ...
                {
                    var svc = new BusinessLogic.Entity.ProductCategory(CurrentUserContext);
        ...
        {
            string sFileName = "ProductCategory.xlsx";
            ...
            using (var fs = new FileStream(Path.Combine(FilePath, sFileName), FileMode.Create, FileAccess.Write))
            {
                ...
                ISheet excelSheet = workbook.CreateSheet("ProductCategory");
                IRow row = excelSheet.CreateRow(0);
                // Setting Cell Heading
                row.CreateCell(0).SetCellValue("Product Category Code");
                ...
                var tabledata = dbContext.product_category.Where(o => o.organization_id == CurrentUserContext.OrganizationId)
                    .OrderByDescending(o => o.created_on);
                // Inserting values to table
                foreach (var baris in tabledata)
                {
                    row = excelSheet.CreateRow(RowCount);
                    row.CreateCell(0).SetCellValue(baris.product_category_code);
                    row.CreateCell(1).SetCellValue(baris.product_category_name);
