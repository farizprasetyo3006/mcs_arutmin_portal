...
{
    [Area("DailyRecord")]
    public class RainfallController : BaseController
    {
        ...
        public IActionResult Index()
        {
            ...
            ViewBag.Breadcrumb = WebAppMenu.BreadcrumbText[WebAppMenu.Rainfall];
            ViewBag.BreadcrumbCode = WebAppMenu.Rainfall;
        ...
        {
            string sFileName = "Rainfall.xlsx";
            ...
            using (var fs = new FileStream(Path.Combine(FilePath, sFileName), FileMode.Create, FileAccess.Write))
            {
                ...
                ISheet excelSheet = workbook.CreateSheet("Rainfall");
                IRow row = excelSheet.CreateRow(0);
                // Setting Cell Heading
                row.CreateCell(0).SetCellValue("Date");
                row.CreateCell(1).SetCellValue("Rainfall Value");
                ...
                row.CreateCell(7).SetCellValue("Is Plan");
                row.CreateCell(8).SetCellValue("Is Forecast");
                ...
                if (Data != null && Data.selectedIds != null)
                {
                    var selectedIds = ((string)Data.selectedIds)
                        ...
                        .ToList();
                    var tabledata = dbContext.rainfall.Where(o => o.organization_id == CurrentUserContext.OrganizationId && selectedIds.Contains(o.id))  
                    ...
                    foreach (var baris in tabledata)
                    {
                        ...
                        if (bu != null) business_uit = bu.business_unit_code.ToString();

                        row = excelSheet.CreateRow(RowCount);
                        row.CreateCell(0).SetCellValue(Convert.ToDateTime(baris.date_time).ToString("yyyy-MM-dd"));
                        row.CreateCell(1).SetCellValue(PublicFunctions.Pecahan(baris.rainfall_value));
                        row.CreateCell(2).SetCellValue(business_area_code);
                        row.CreateCell(3).SetCellValue(shift_code);
                        row.CreateCell(4).SetCellValue(PublicFunctions.Pecahan(baris.duration));
                        row.CreateCell(5).SetCellValue(PublicFunctions.Pecahan(baris.frequency));
                        row.CreateCell(6).SetCellValue(contractor_name);
                        row.CreateCell(7).SetCellValue(PublicFunctions.BenarSalah(baris.is_plan));
                        row.CreateCell(8).SetCellValue(PublicFunctions.BenarSalah(baris.is_forecast));
                        row.CreateCell(9).SetCellValue(business_uit);
