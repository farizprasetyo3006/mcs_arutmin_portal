...
{
    [Area("DailyRecord")]
    public class SlipperyController : BaseController
    {
        private static readonly Logger logger = LogManager.GetCurrentClassLogger();
        private readonly mcsContext dbContext;

        public SlipperyController(IConfiguration Configuration)
            : base(Configuration)
        {
        ...
        public IActionResult Index()
        {
            ...
            ViewBag.Breadcrumb = WebAppMenu.BreadcrumbText[WebAppMenu.Slippery];
            ViewBag.BreadcrumbCode = WebAppMenu.Slippery;
        ...
        {
            string sFileName = "Slippery.xlsx";
            ...
            using (var fs = new FileStream(Path.Combine(FilePath, sFileName), FileMode.Create, FileAccess.Write))
            {
                ...
                ISheet excelSheet = workbook.CreateSheet("Slippery");
                IRow row = excelSheet.CreateRow(0);
                // Setting Cell Heading
                row.CreateCell(0).SetCellValue("Date");
                row.CreateCell(1).SetCellValue("Slippery Value");
                row.CreateCell(2).SetCellValue("Business Area");
                row.CreateCell(3).SetCellValue("Shift");
                row.CreateCell(4).SetCellValue("Contractor");
                row.CreateCell(5).SetCellValue("Is Plan");
                row.CreateCell(6).SetCellValue("Is Forecast");
                ...
                if (Data != null && Data.selectedIds != null)
                {
                    var selectedIds = ((string)Data.selectedIds)
                        ...
                        .ToList();
                    var tabledata = dbContext.slippery.Where(o => o.organization_id == CurrentUserContext.OrganizationId && selectedIds.Contains(o.id))
                    ...
                    foreach (var baris in tabledata)
                    {
                        ...
                        row.CreateCell(1).SetCellValue(PublicFunctions.Pecahan(baris.slippery_value));
                        ...
                        row.CreateCell(5).SetCellValue(PublicFunctions.BenarSalah(baris.is_plan));
                        row.CreateCell(6).SetCellValue(PublicFunctions.BenarSalah(baris.is_forecast));
                        row.CreateCell(7).SetCellValue(business_uit);
