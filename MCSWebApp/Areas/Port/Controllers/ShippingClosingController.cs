...
{
    [Area("Port")]
    public class ShippingClosingController : BaseController
    {
        private static readonly Logger logger = LogManager.GetCurrentClassLogger();
        private readonly mcsContext dbContext;

        public ShippingClosingController(IConfiguration Configuration)
            : base(Configuration)
        {
        ...
        {
            string sFileName = "Shipping Closing.xlsx";
            ...
            using (var fs = new FileStream(Path.Combine(FilePath, sFileName), FileMode.Create, FileAccess.Write))
            {
                ...
                ISheet excelSheet = workbook.CreateSheet("Shipping Closing");
                ...
                var dataTable = await dbContext.shipping_closing
                ...
                foreach (var data in dataTable)
                {
                    ...
                    var itemData = await dbContext.shipping_closing_item
                        .Where(x => x.shipping_closing_id == data.id).ToListAsync();
                    int iCount = 1;
