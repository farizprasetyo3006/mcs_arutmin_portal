...
{
    [Area("Mining")]
    public class CoalMinedClosingController : BaseController
    {
        private static readonly Logger logger = LogManager.GetCurrentClassLogger();
        private readonly mcsContext dbContext;

        public CoalMinedClosingController(IConfiguration Configuration)
            : base(Configuration)
        {
        ...
        {
            string sFileName = "Coal_Mined_Closing.xlsx";
            ...
            using (var fs = new FileStream(Path.Combine(FilePath, sFileName), FileMode.Create, FileAccess.Write))
            {
                ...
                ISheet excelSheet = workbook.CreateSheet("Coal Mined Closing");
                ...
                var dataTable = await dbContext.production_closing
                ...
                foreach (var data in dataTable)
                {
                    ...
                    var itemData = await dbContext.production_closing_item
                        .Where(x => x.production_closing_id == data.id).ToListAsync();
                    int iCount = 1;
