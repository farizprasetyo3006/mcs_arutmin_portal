...
{
    [Area("Mining")]
    public class WasteRemovalClosingController : BaseController
    {
        private static readonly Logger logger = LogManager.GetCurrentClassLogger();
        private readonly mcsContext dbContext;

        public WasteRemovalClosingController(IConfiguration Configuration)
            : base(Configuration)
        {
        ...
        public IActionResult Index()
        {
            ...
            ViewBag.Breadcrumb = WebAppMenu.BreadcrumbText[WebAppMenu.WasteRemovalClosing];
            ViewBag.BreadcrumbCode = WebAppMenu.WasteRemovalClosing;
        ...
        {
            string sFileName = "Waste Removal Closing.xlsx";
            ...
            using (var fs = new FileStream(Path.Combine(FilePath, sFileName), FileMode.Create, FileAccess.Write))
            {
                ...
                ISheet excelSheet = workbook.CreateSheet("Waste Removal Closing");
                ...
                var dataTable = await dbContext.waste_removal_closing
                ...
                foreach (var data in dataTable)
                {
                    ...
                    var itemData = await dbContext.waste_removal_closing_item
                        .Where(x => x.waste_removal_closing_id == data.id).ToListAsync();
                    int iCount = 1;
                    ...
                    foreach ( var item in itemData )
                    {
                        ...
                        row.CreateCell(14).SetCellValue(Convert.ToString(item.loading_quantity));
                        RowCount++;
                        iCount++;
                    }
                }
