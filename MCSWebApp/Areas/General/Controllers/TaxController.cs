...
namespace MCSWebApp.Areas.General.Controllers
{
    ...
    public class TaxController : BaseController
    {
        ...
        public IActionResult Index()
        {
            ...
            ViewBag.Breadcrumb = WebAppMenu.BreadcrumbText[WebAppMenu.Tax];
            ViewBag.BreadcrumbCode = WebAppMenu.Tax;
        ...
        public async Task<IActionResult> ExcelExport()
        {
            ...
            using (var fs = new FileStream(Path.Combine(FilePath, sFileName), FileMode.Create, FileAccess.Write))
            {
                ...
                ISheet excelSheet = workbook.CreateSheet("Tax");
                IRow row = excelSheet.CreateRow(0);
                // Setting Cell Heading
                row.CreateCell(0).SetCellValue("Tax Name");
                ...
                var tabledata = dbContext.tax.Where(o => o.organization_id == CurrentUserContext.OrganizationId);
                // Inserting values to table
                foreach (var baris in tabledata)
                {
                    row = excelSheet.CreateRow(RowCount);
                    row.CreateCell(0).SetCellValue(baris.tax_name);
                    row.CreateCell(1).SetCellValue(Convert.ToDouble(baris.rate));
