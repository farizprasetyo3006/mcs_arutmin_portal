...
namespace MCSWebApp.Areas.General.Controllers
{
    ...
    public class BankController : BaseController
    {
        ...
        public IActionResult Index()
        {
            ...
            ViewBag.Breadcrumb = WebAppMenu.BreadcrumbText[WebAppMenu.Bank];
            ViewBag.BreadcrumbCode = WebAppMenu.Bank;
        ...
        public async Task<IActionResult> ExcelExport()
        {
            ...
            using (var fs = new FileStream(Path.Combine(FilePath, sFileName), FileMode.Create, FileAccess.Write))
            {
                ...
                ISheet excelSheet = workbook.CreateSheet("Bank");
                IRow row = excelSheet.CreateRow(0);
                // Setting Cell Heading
                row.CreateCell(0).SetCellValue("Bank Name");
                row.CreateCell(1).SetCellValue("Bank Code");
                ...
                var tabledata = dbContext.bank.Where(o => o.organization_id == CurrentUserContext.OrganizationId);
                // Inserting values to table
                foreach (var Value in tabledata)
                {
                    row = excelSheet.CreateRow(RowCount);
                    row.CreateCell(0).SetCellValue(Value.bank_name);
                    row.CreateCell(1).SetCellValue(Value.bank_code);
