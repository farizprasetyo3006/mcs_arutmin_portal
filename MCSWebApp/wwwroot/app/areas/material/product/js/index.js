$(function () {
    ...
    $("#grid").dxDataGrid({
        ...
        columns: [
            ...
            },
            {
                ...
                lookup: {
                    dataSource: DevExpress.data.AspNet.createStore({
                        key: "value",
                        loadUrl: "/api/Accounting/Account/CoaIdLookup",
                        onBeforeSend: function (method, ajaxOptions) {
                            ajaxOptions.xhrFields = { withCredentials: true };
                            ajaxOptions.beforeSend = function (request) {
                                request.setRequestHeader("Authorization", "Bearer " + token);
                ...
                lookup: {
                    dataSource: function (options) {
                        return {
                            ...
                            filter: ["item_group", "=", "satui-breakdown"]
                        }
                    },
                    valueExpr: "value",
                    displayExpr: "text"
        ...
        onEditorPreparing: function (e) {
            ...
            if (e.parentType === "dataRow" && e.dataField == "product_code") {
                e.editorOptions.disabled = e.row.data.product_code != null
            }
        },
        ...
        return $("<div>").dxTabPanel({
            items: [
                ...
                },
                {
                    ...
                    template: createProductStandardTabTemplate(masterDetailOptions.data)
                },
            ]
    ...
    function createDetailTabTemplate(masterDetailData) {
        return function () {
            var currentRecord = masterDetailData;
            var detailEntityName = "ProductSpecification";
            ...
            return $("<div>")
                .dxDataGrid({
                    dataSource: DevExpress.data.AspNet.createStore({
                        key: "id",
                        loadUrl: urlDetail + "/ByProductId/" + encodeURIComponent(currentRecord.id),
                    ...
                    onInitNewRow: function (e) {
                        e.data.product_id = currentRecord.id;
                    },
                    onExporting: function (e) {
                        var workbook = new ExcelJS.Workbook();
                        var worksheet = workbook.addWorksheet(entityName);
    ...
    function createProductStandardTabTemplate(masterDetailData) {
        return function () {
            var currentRecord = masterDetailData;
            var detailEntityName = "ProductStandard";
            var urlDetail = "/api/" + areaName + "/" + detailEntityName;
            let idx = "123";
                            ...
                            lookup: {
                                dataSource: function (options) {
                                    return {
                                        store: DevExpress.data.AspNet.createStore({
                                            key: "value",
                                            loadUrl: "/api/Material/Product/ProductIdFilteredLookup/" + idx + "/" + isInserting,
                                            onBeforeSend: function (method, ajaxOptions) {
                                                ajaxOptions.xhrFields = { withCredentials: true };
                                                ajaxOptions.beforeSend = function (request) {
                                                    request.setRequestHeader("Authorization", "Bearer " + token);
                            ...
                            e.editorOptions.onValueChanged = function (e) { // Overiding the standard handler
                                ...
                                $.ajax({
                                    ...
                                    success: function (response) {
                                        ...
                                        grid.cellValue(index, "product_category_id", record.product_category_id)
                                        grid.cellValue(index, "business_unit_id", record.business_unit_id)
    ...
    $('#btnDownloadSelectedRow').on('click', function () {
        if (selectedIds != null && selectedIds != '') {
            ...
            $.ajax({
                url: "/Material/Product/ExcelExport",
                type: 'POST',
                cache: false,
                contentType: "application/json",
                ...
                if (data instanceof Blob) {
                    ...
                    a.href = url;
                    a.download = "Product.xlsx"; // Set the appropriate file name here
    ...
    $('#btnUpload').on('click', function () {
        ...
        reader.onload = function () {
            ...
            $.ajax({
                url: "/api/Material/Product/UploadDocument",
                type: 'POST',
                cache: false,
                contentType: "application/json",
