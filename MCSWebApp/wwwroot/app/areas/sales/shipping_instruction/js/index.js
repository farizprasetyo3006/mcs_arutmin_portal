$(function () {
    ...
    $("#grid").dxDataGrid({
        ...
        columns: [
                    ...
                    dataSource: function (options) {
                        return {
                            store: DevExpress.data.AspNet.createStore({
                                key: "value",
                                loadUrl: "/api/Sales/SalesInvoice/DespatchOrderIdLookup/null",
                                onBeforeSend: function (method, ajaxOptions) {
                                    ajaxOptions.xhrFields = { withCredentials: true };
                                    ajaxOptions.beforeSend = function (request) {
                                        request.setRequestHeader("Authorization", "Bearer " + token);
            ...
            {
                dataField: "quality_contract",
                dataType: "string",
                caption: "Quality Contract",
                allowEditing: true,
                visible: false
            },
            {
                dataField: "country_of_discharge",
                dataType: "string",
           ...
           /* {
                dataField: "advance_royalty_amount",
                dataType: "number",
                caption: "Advance Royalty Amount",
                allowEditing: true,
                visible: false
            },*/
            {
           ...
           /* {
                ...
                editCellTemplate: function (container, options) {
                    var $uploadButton = $("<div>")
                        ...
                        .dxFileUploader({
                            ...
                            uploadUrl: "/api/upload", // Specify your upload URL
                ...
                editCellTemplate: function (container, options) {
                    var $button = $("<div>")
                        .dxButton({
                            ...
                            onClick: function () {
                                // Save the data
                                options.dataGrid.saveEditData();
                                // Trigger file upload process
                                var rowIndex = options.rowIndex;
                                var fileUploader = options.dataGrid.getCellElement(rowIndex, "lampiran1File").find(".dx-fileuploader").dxFileUploader("instance");
                                fileUploader.upload();
                            }
                        })
                        .appendTo(container);
            ...
            },
            {
                dataField: "voy",
                dataType: "string",
                caption: "Master/Voy No.",
                visible: false
            },
            {
                dataField: "place_delivery",
                dataType: "string",
                caption: "Country of Delivery",
                visible: false
            },
            {
                ...
                e.editorOptions.onValueChanged = async function (e) { // Overiding the standard handler                    
                    ...
                    await $.ajax({
                        ...
                        success: function (response) {
                            ...
                            grid.cellValue(index, "type", despatchOrderData.type)
                            grid.cellValue(index, "imo", despatchOrderData.imo_number)
                            grid.cellValue(index, "dwt", despatchOrderData.dwt_gt)
                            grid.cellValue(index, "flag", despatchOrderData.flag)
                            grid.cellValue(index, "royalty_number", despatchOrderData.royalty_reference)
                            grid.cellValue(index, "invoice_number", despatchOrderData.nomor_invoice)
                            grid.cellValue(index, "analyte_standard", despatchOrderData.analyte_standard_id)
                            grid.cellValue(index, "lc_number", despatchOrderData.letter_of_credit)
                            grid.cellValue(index, "royalty_number", despatchOrderData.royalty_number)
                            grid.cellValue(index, "invoice_number", despatchOrderData.nomor_invoice)
                            grid.endUpdate()
                    ...
                    $.ajax({
                        url: '/api/Sales/Royalty/Payment/RoyaltypaymentByShippingOrder?Id=' + despatchOrderId,
                        type: 'GET',
                        contentType: "application/json",
                        beforeSend: function (xhr) {
                            xhr.setRequestHeader("Authorization", "Bearer " + token);
                        ...
                        success: function (response) {
                            ...
                            grid.beginUpdate()
                            grid.cellValue(index, "royalty_amount", despatchOrderData.royalty_value)
                            grid.endUpdate()
   ...
   */
    ...
    function masterDetailTemplate(masterDetailOptions, container) {
        ...
        $("<div>").dxTabPanel({
            items: [
                ...
                {
                    title: "Attachment",
                    template: createAttachmentTab(masterDetailOptions)
                },
                /*{
                    title: "Insurance",
    ...
    function createAttachmentTab(masterDetailData) {
        return function () {
            ...
            documentDataGrid = $("<div>")
                .dxDataGrid({
                    dataSource: DevExpress.data.AspNet.createStore({
                        ...
                        insertUrl: urlDetail + "/InsertAttachment",
                        updateUrl: urlDetail + "/UpdateData",
                        deleteUrl: urlDetail + "/DeleteAttachment",
                        onBeforeSend: function (method, ajaxOptions) {
                            ajaxOptions.xhrFields = { withCredentials: true };
                            ajaxOptions.beforeSend = function (request) {
                                request.setRequestHeader("Authorization", "Bearer " + token);
                    ...
                    columns: [
                        {
                            dataField: "shipping_instuction_id",
                            allowEditing: false,
                            visible: false,
                        ...
                        },
                        {
                            ...
                            buttons: [{
                                ...
                                onClick: function (e) {
                                    ...
                                    xhr.open("GET", "/api/Sales/ShippingInstruction/DownloadDocument/" + documentData.id, true)
    ...
    const documentPopupOptions = {
        ...
        contentTemplate: function (e) {
            ...
            formContainer.dxForm({
                formData: {
                    id: "",
                    shipping_instruction_id: COWData?.id || "",
                    file: "",
                },
                colCount: 2,
                items: [
                    {
                        dataField: "shipping_instruction_id",
                        visible: false
                    },
                        ...
                        buttonOptions: {
                            ...
                            onClick: function () {
                                ...
                                reader.onload = function () {
                                    ...
                                    let newFormData = {
                                        "shipping_instruction_id": formData.shipping_instruction_id,
                                        "fileName": fileName,
                                        "fileSize": fileSize,
                                        "data": data
                                    }
                                    ...
                                    $.ajax({
                                        url: `/api/${areaName}/ShippingInstruction/InsertAttachment`,
        ...
        return function () {
            ...
            documentDataGrid = $("<div>")
                .dxDataGrid({
                    dataSource: DevExpress.data.AspNet.createStore({
                        key: "id",
                        loadUrl: urlDetail + "/BySamplingTemplateId?Id=" + currentRecord.despatch_order_id,
                        insertUrl: urlDetail + "/InsertData",
                        updateUrl: urlDetail + "/UpdateData",
                        deleteUrl: urlDetail + "/DeleteData",
                        onBeforeSend: function (method, ajaxOptions) {
            ...
            documentDataGrid = $("<div>")
                .dxDataGrid({
                    ...
                    columns: [
                        ...
                        {
                            dataField: "number_of_original",
                            dataType: "number",
                            caption: "Number of Original",
                            editorType: "dxNumberBox",
                            editorOptions: {
                                format: "fixedPoint",
                                step: 0
                        ...
                        e.toolbarOptions.items.unshift({
                            ...
                            options: {
                                ...
                                onClick: function (e) {
                                    ...
                                    $.ajax({
                                        url: '/api/Sales/ShippingInstructionDetailSurveyDokumen/FetchShippingDocument/' + encodeURIComponent(currentRecord.id),
                                        type: 'PUT',
                                        contentType: "application/json",
                                        headers: {
                                            "Authorization": "Bearer " + token
        ...
        contentTemplate: function () {
            ...
            var approvalForm = $("<div>").dxForm({
                ...
                items: [
                    {
                    ...
                    },
                     {
                         //itemType: "label",
                         label: {
                             text: "Are You Sure Want To Copy This Data?",
                         },
                         colSpan: 2,
                    ...
                    /*{
                        ...
                        colSpan: 2,
                        editorOptions: {
                            placeholder: "Select a number from 2 to 20",
                            onValueChanged: function (e) {
                ...
                onInitialized: () => {
                    $.ajax({
                        type: "GET",
                        url: "/api/Sales/ShippingInstruction/GetShippingInstruction/" + encodeURIComponent(salesInvoiceApprovalData.id),
                        contentType: "application/json",
                        beforeSend: function (xhr) {
                            xhr.setRequestHeader("Authorization", "Bearer " + token);
                        },
    ...
    const saveApprovalForm = (formData) => {
        $.ajax({
            type: "POST",
            url: "/api/Sales/ShippingInstruction/CopyData",
            data: formData,
            processData: false,
            contentType: false,
            beforeSend: function (xhr) {
...
`).appendTo($("body")[0]);
    ...
    $('#btnDownloadSelectedRow').on('click', function () {
        if (selectedIds != null && selectedIds != '') {
            ...
            $.ajax({
                url: "/Sales/ShippingInstruction/ExcelExport",
                type: 'POST',
                cache: false,
                contentType: "application/json",
                ...
                if (data instanceof Blob) {
                    ...
                    a.href = url;
                    a.download = "Shipping_Instruction.xlsx"; // Set the appropriate file name here
                    document.body.appendChild(a);
                    a.click();
