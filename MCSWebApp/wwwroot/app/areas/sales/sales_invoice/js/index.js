$(function () {

    var token = $.cookie("Token");
    var areaName = "Sales";
    var entityName = "SalesInvoice";
    var url = "/api/" + areaName + "/" + entityName;
    var salesInvoiceData = null;
    const maxFileSize = 52428800;
    var salesTypeId = "";
    var salesTypeName = "";
    var applicationEntityId = "";
    var reportTemplateId = "";
    var recordId = "";
    var salesTypeChanged = false;
    var editors = {};
    var isEditing = null;
    var cif = 0;
    var isInserting = false;
    var ShippingOrderId = null;

    function commaSeparateNumber(val) {
        // remove sign if negative
        var sign = 1;
        if (val < 0) {
            sign = -1;
            val = -val;
        }

        // trim the number decimal point if it exists
        let num = val.toString().includes('.') ? val.toString().split('.')[0] : val.toString();

        while (/(\d+)(\d{3})/.test(num.toString())) {
            // insert comma to 4th last position to the match number
            num = num.toString().replace(/(\d+)(\d{3})/, '$1' + ',' + '$2');
        }

        // add number after decimal point
        if (val.toString().includes('.')) {
            num = num + '.' + val.toString().split('.')[1];
        }

        // return result with - sign if negative
        return sign < 0 ? '-' + num : num;
    }

    $("#grid").dxDataGrid({
        dataSource: DevExpress.data.AspNet.createStore({
            key: "id",
            loadUrl: url + "/DataGrid",
            insertUrl: url + "/InsertData",
            updateUrl: url + "/UpdateData",
            deleteUrl: url + "/DeleteData",
            onBeforeSend: function (method, ajaxOptions) {
                ajaxOptions.xhrFields = { withCredentials: true };
                ajaxOptions.beforeSend = function (request) {
                    request.setRequestHeader("Authorization", "Bearer " + token);
                };
            }
        }),
        selection: {
            mode: "multiple"
        },
        remoteOperations: true,
        allowColumnResizing: true,
        columnResizingMode: "widget",
        dateSerializationFormat: "yyyy-MM-ddTHH:mm:ss",
        columns: [
            {
                dataField: "id",
                dataType: "string",
                caption: "ID",
                visible: false,
                showInColumnChooser: false
            },
            {
                dataField: "invoice_number",
                dataType: "string",
                caption: "Invoice Number",
                validationRules: [{
                    type: "required",
                    message: "The field is required."
                }],
                sortOrder: "asc"
            },
            {
                dataField: "sales_type_id",
                dataType: "string",
                caption: "Sales Type",
                visible: false,
                lookup: {
                    dataSource: function (options) {
                        return {
                            store: DevExpress.data.AspNet.createStore({
                                key: "value",
                                loadUrl: "/api/General/MasterList/MasterListIdLookup",
                                onBeforeSend: function (method, ajaxOptions) {
                                    ajaxOptions.xhrFields = { withCredentials: true };
                                    ajaxOptions.beforeSend = function (request) {
                                        request.setRequestHeader("Authorization", "Bearer " + token);
                                    };
                                }
                            }),
                            filter: ["item_group", "=", "sales-type"]
                        }
                    },
                    valueExpr: "value",
                    displayExpr: "text"
                },
                calculateSortValue: function (data) {
                    var value = this.calculateCellValue(data);
                    return this.lookup.calculateCellValue(value);
                },
                setCellValue: async function (newData, value) {
                    this.defaultSetCellValue(newData, value);
                },
                formItem: {
                    editorOptions: {
                        showClearButton: true
                    }
                },
            },
            {
                dataField: "invoice_date",
                dataType: "date",
                caption: "Invoice Date",
                validationRules: [{
                    type: "required",
                    message: "The field is required."
                }],
                setCellValue: function (rowData, value) {
                    //rowData.invoice_date = value
                    rowData.invoice_date = new Date(value).toLocaleDateString();
                    rowData.currency_exchange_id = null
                },
            },
            {
                dataField: "despatch_order_id",
                dataType: "string",
                caption: "Shipping Order",
                validationRules: [{
                    type: "required",
                    message: "The field is required."
                }],
                lookup: {
                    dataSource: function (options) {
                        return {
                            store: DevExpress.data.AspNet.createStore({
                                key: "value",
                                loadUrl: isInserting === true ? url + "/DespatchOrderIdLookup/" + ShippingOrderId : url + "/AllDespatchOrderIdLookup",
                                onBeforeSend: function (method, ajaxOptions) {
                                    ajaxOptions.xhrFields = { withCredentials: true };
                                    ajaxOptions.beforeSend = function (request) {
                                        request.setRequestHeader("Authorization", "Bearer " + token);
                                    };
                                }
                            }),
                        }
                    },
                    valueExpr: "value",
                    displayExpr: "text"
                },
                calculateSortValue: function (data) {
                    var value = this.calculateCellValue(data);
                    return this.lookup.calculateCellValue(value);
                },
                setCellValue: function (rowData, value) {
                    rowData.despatch_order_id = value
                    rowData.quotation_type_id = null
                    rowData.invoice_type_id = null
                    rowData.currency_exchange_id = null
                },
                formItem: {
                    editorOptions: {
                        showClearButton: true
                    }
                },
                allowSearch: false,
            },
            {
                dataField: "despatch_order_link",
                dataType: "string",
                caption: "Shipping Order",
                visible: false,
                allowFiltering: false,
                showInColumnChooser: false
            },
            {
                dataField: "invoice_type_id",
                dataType: "string",
                caption: "Invoice Type",
                validationRules: [{
                    type: "required",
                    message: "The field is required."
                }],
                lookup: {
                    dataSource: function (options) {
                        return {
                            store: DevExpress.data.AspNet.createStore({
                                key: "value",
                                loadUrl: "/api/General/MasterList/MasterListIdLookup",
                                onBeforeSend: function (method, ajaxOptions) {
                                    ajaxOptions.xhrFields = { withCredentials: true };
                                    ajaxOptions.beforeSend = function (request) {
                                        request.setRequestHeader("Authorization", "Bearer " + token);
                                    };
                                }
                            }),
                            filter: ["item_group", "=", "invoice-type"]
                        }
                    },
                    valueExpr: "value",
                    displayExpr: "text"
                },
                calculateSortValue: function (data) {
                    var value = this.calculateCellValue(data);
                    return this.lookup.calculateCellValue(value);
                },
                setCellValue: function (rowData, value) {
                    rowData.invoice_type_id = value
                },
                formItem: {
                    editorOptions: {
                        showClearButton: true
                    }
                },
                visible: false
            },
            {
                dataField: "refreshButton",
                dataType: "string",
                caption: "Recalculate",
                icon: "refresh",
                visible: false,
                allowFiltering: false,
                allowSearch: false,
                showInColumnChooser: false
            },
            {
                dataField: "invoice_type_name",
                dataType: "string",
                caption: "Invoice Type",
                formItem: {
                    visible: false
                }
            },
            {
                dataField: "quotation_type_id",
                dataType: "string",
                caption: "Quotation Type",
                validationRules: [{
                    type: "required",
                    message: "The field is required."
                }],
                lookup: {
                    dataSource: function (options) {
                        return {
                            store: DevExpress.data.AspNet.createStore({
                                key: "value",
                                loadUrl: "/api/General/MasterList/MasterListIdLookup",
                                onBeforeSend: function (method, ajaxOptions) {
                                    ajaxOptions.xhrFields = { withCredentials: true };
                                    ajaxOptions.beforeSend = function (request) {
                                        request.setRequestHeader("Authorization", "Bearer " + token);
                                    };
                                }
                            }),
                            filter: ["item_group", "=", "quotation-type"]
                        }
                    },
                    valueExpr: "value",
                    displayExpr: "text"
                },
                calculateSortValue: function (data) {
                    var value = this.calculateCellValue(data);
                    return this.lookup.calculateCellValue(value);
                },
                setCellValue: function (rowData, value) {
                    rowData.quotation_type_id = value
                },
                formItem: {
                    editorOptions: {
                        showClearButton: true
                    }
                },
                visible: false
            },
            {
                dataField: "sales_contract_id",
                dataType: "string",
                caption: "Sales Contract",
                visible: false,
                lookup: {
                    dataSource: DevExpress.data.AspNet.createStore({
                        key: "value",
                        loadUrl: "/api/Sales/SalesContract/SalesContractIdLookup",
                        onBeforeSend: function (method, ajaxOptions) {
                            ajaxOptions.xhrFields = { withCredentials: true };
                            ajaxOptions.beforeSend = function (request) {
                                request.setRequestHeader("Authorization", "Bearer " + token);
                            };
                        }
                    }),
                    valueExpr: "value",
                    displayExpr: "text"
                },
                calculateSortValue: function (data) {
                    var value = this.calculateCellValue(data);
                    return this.lookup.calculateCellValue(value);
                },
                allowEditing: false
            },
            {
                dataField: "sales_contract_term_id",
                dataType: "string",
                caption: "Contract Term",
                allowEditing: false,
                visible: false,
                lookup: {
                    dataSource: DevExpress.data.AspNet.createStore({
                        key: "value",
                        loadUrl: "/api/Sales/SalesContractTerm/SalesContractTermIdLookup",
                        onBeforeSend: function (method, ajaxOptions) {
                            ajaxOptions.xhrFields = { withCredentials: true };
                            ajaxOptions.beforeSend = function (request) {
                                request.setRequestHeader("Authorization", "Bearer " + token);
                            };
                        }
                    }),
                    valueExpr: "value",
                    displayExpr: "text"
                },
                calculateSortValue: function (data) {
                    var value = this.calculateCellValue(data);
                    return this.lookup.calculateCellValue(value);
                },
                formItem: {
                    editorOptions: {
                        showClearButton: true
                    }
                },
            },
            {
                dataField: "seller_id",
                dataType: "string",
                caption: "Seller",
                allowEditing: false,
                visible: false,
                lookup: {
                    dataSource: DevExpress.data.AspNet.createStore({
                        key: "value",
                        loadUrl: "/api/SystemAdministration/Organization/OrganizationIdLookup",
                        onBeforeSend: function (method, ajaxOptions) {
                            ajaxOptions.xhrFields = { withCredentials: true };
                            ajaxOptions.beforeSend = function (request) {
                                request.setRequestHeader("Authorization", "Bearer " + token);
                            };
                        }
                    }),
                    valueExpr: "value",
                    displayExpr: "text"
                },
                calculateSortValue: function (data) {
                    var value = this.calculateCellValue(data);
                    return this.lookup.calculateCellValue(value);
                },
                formItem: {
                    editorOptions: {
                        showClearButton: true
                    }
                },
            },
            {
                dataField: "bank_account_id",
                dataType: "string",
                caption: "Advising Bank",
                visible: false,
                lookup: {
                    dataSource: function (options) {
                        return {
                            store: DevExpress.data.AspNet.createStore({
                                key: "value",
                                loadUrl: "/api/General/BankAccount/BankAccountIdLookup", // API for Bank account
                                onBeforeSend: function (method, ajaxOptions) {
                                    ajaxOptions.xhrFields = { withCredentials: true };
                                    ajaxOptions.beforeSend = function (request) {
                                        request.setRequestHeader("Authorization", "Bearer " + token);
                                    };
                                }
                            }),
                            // filter: options.data ? ["bank_id", "=", options.data.bank_id] : null
                        }
                    },
                    valueExpr: "value",
                    displayExpr: "text"
                },
                calculateSortValue: function (data) {
                    var value = this.calculateCellValue(data);
                    return this.lookup.calculateCellValue(value);
                },
                formItem: {
                    editorOptions: {
                        showClearButton: true
                    }
                },
            },
            {
                dataField: "correspondent_bank_id",
                dataType: "string",
                caption: "Issuing Bank",
                visible: false
            },
            {
                dataField: "customer_id",
                dataType: "string",
                caption: "Buyer",
                allowEditing: false,
                visible: false,
                lookup: {
                    dataSource: DevExpress.data.AspNet.createStore({
                        key: "value",
                        loadUrl: "/api/Sales/Customer/CustomerIdLookup",
                        onBeforeSend: function (method, ajaxOptions) {
                            ajaxOptions.xhrFields = { withCredentials: true };
                            ajaxOptions.beforeSend = function (request) {
                                request.setRequestHeader("Authorization", "Bearer " + token);
                            };
                        }
                    }),
                    valueExpr: "value",
                    displayExpr: "text"
                },
                calculateSortValue: function (data) {
                    var value = this.calculateCellValue(data);
                    return this.lookup.calculateCellValue(value);
                },
                formItem: {
                    editorOptions: {
                        showClearButton: true
                    }
                },
            },
            {
                dataField: "bill_to",
                dataType: "string",
                caption: "Bill To (Invoice Target)",
                lookup: {
                    dataSource: function (options) {
                        //var _url = "/api/Sales/SalesContractInvoiceTarget/SalesInvoiceTargetIdLookup"; CustomerIdLookup
                        var _url = "/api/Sales/Customer/CustomerIdLookup";
                        /*if (options !== undefined && options !== null) {
                            if (options.data !== undefined && options.data !== null) {
                                if (options.data.sales_contract_id !== undefined
                                    && options.data.sales_contract_id !== null) {
                                    _url += "?SalesContractId=" + encodeURIComponent(options.data.sales_contract_id);
                                }
                            }
                        }*/
                        return {
                            store: DevExpress.data.AspNet.createStore({
                                key: "value",
                                loadUrl: _url,
                                onBeforeSend: function (method, ajaxOptions) {
                                    ajaxOptions.xhrFields = { withCredentials: true };
                                    ajaxOptions.beforeSend = function (request) {
                                        request.setRequestHeader("Authorization", "Bearer " + token);
                                    };
                                }
                            })
                        }
                    },
                    valueExpr: "value",
                    displayExpr: "text"
                },
                calculateSortValue: function (data) {
                    var value = this.calculateCellValue(data);
                    return this.lookup.calculateCellValue(value);
                },
                formItem: {
                    editorOptions: {
                        showClearButton: true
                    }
                },
                setCellValue: function (rowData, value) {
                    rowData.bill_to = value
                },
                allowEditing: true,
                showInColumnChooser: false
            },
            {
                dataField: "bill_to_name",
                dataType: "string",
                caption: "Bill To (Invoice Target)",
                visible: false,
                formItem: {
                    visible: false,
                },
                allowEditing: false
            },
            {
                dataField: "lc_number",
                dataType: "string",
                caption: "L/C Number",
                allowEditing: false,
                visible: false
            },
            {
                dataField: "lc_status",
                dataType: "string",
                caption: "L/C Specification",
                visible: false
            },
            {
                dataField: "lc_date_issue",
                dataType: "string",
                caption: "L/C Date Issue",
                visible: false
            },
            {
                dataField: "lc_issuing_bank",
                dataType: "string",
                caption: "L/C Issuing Bank",
                visible: false,
                formItem: {
                    visible: false
                }
            },
            {
                dataField: "contract_product_id",
                dataType: "string",
                caption: "Contract Product",
                allowEditing: false,
                lookup: {
                    dataSource: DevExpress.data.AspNet.createStore({
                        key: "value",
                        loadUrl: "/api/Sales/SalesContractProduct/SalesContractProductIdLookup",
                        onBeforeSend: function (method, ajaxOptions) {
                            ajaxOptions.xhrFields = { withCredentials: true };
                            ajaxOptions.beforeSend = function (request) {
                                request.setRequestHeader("Authorization", "Bearer " + token);
                            };
                        }
                    }),
                    valueExpr: "value",
                    displayExpr: "text"
                },
                calculateSortValue: function (data) {
                    var value = this.calculateCellValue(data);
                    return this.lookup.calculateCellValue(value);
                },
                formItem: {
                    editorOptions: {
                        showClearButton: true
                    }
                },
                visible: false
            },
            {
                dataField: "description_of_goods",
                dataType: "string",
                caption: "Description of Goods",
                visible: false,
                formItem: {
                    colSpan: 2,
                    editorType: "dxTextArea"
                }
            },
            {
                dataField: "inco_term",
                dataType: "string",
                caption: "Inco Term",
                allowEditing: false,
                lookup: {
                    dataSource: function (options) {
                        return {
                            store: DevExpress.data.AspNet.createStore({
                                key: "value",
                                loadUrl: "/api/General/MasterList/MasterListIdLookup",
                                onBeforeSend: function (method, ajaxOptions) {
                                    ajaxOptions.xhrFields = { withCredentials: true };
                                    ajaxOptions.beforeSend = function (request) {
                                        request.setRequestHeader("Authorization", "Bearer " + token);
                                    };
                                }
                            }),
                            filter: ["item_group", "=", "delivery-term"]
                        }
                    },
                    valueExpr: "value",
                    displayExpr: "text"
                },
            },
            {
                dataField: "loading_port_id",
                dataType: "string",
                caption: "Loading Port",
                //allowEditing: false,
                visible: false,
                lookup: {
                    dataSource: function (options) {
                        return {
                            store: DevExpress.data.AspNet.createStore({
                                key: "value",
                                loadUrl: "/api/Location/PortLocation/PortLocationIdLookup",
                                onBeforeSend: function (method, ajaxOptions) {
                                    ajaxOptions.xhrFields = { withCredentials: true };
                                    ajaxOptions.beforeSend = function (request) {
                                        request.setRequestHeader("Authorization", "Bearer " + token);
                                    };
                                }
                            })
                        }
                    },
                    valueExpr: "value",
                    displayExpr: "text"
                }
            },
            {
                dataField: "discharge_port",
                dataType: "string",
                caption: "Discharge Port",
                allowEditing: false,
                visible: false
            },
            {
                dataField: "consignee",
                dataType: "string",
                caption: "Consignee",
                visible: false,
            },
            {
                dataField: "quantity",
                dataType: "number",
                caption: "Quantity",
                format: "fixedPoint",
                formItem: {
                    editorType: "dxNumberBox",
                    editorOptions: {
                        format: "#,##0.0000",
                        step: 0
                    }
                },
                allowEditing: false,
                visible: false
            },
            {
                dataField: "unit_price",
                dataType: "number",
                caption: "Base Price",
                formItem: {
                    editorType: "dxNumberBox",
                    editorOptions: {
                        format: "#,##0.00000",
                        step: 0
                    }
                },
                allowEditing: true,
                visible: false
            },
            {
                dataField: "fob_price",
                dataType: "number",
                caption: "FOB Price",
                formItem: {
                    editorType: "dxNumberBox",
                    editorOptions: {
                        format: "#,##0.00000",
                        step: 0
                    }
                },
                allowEditing: true,
                visible: false
            },
            {
                dataField: "gcv_arb",
                dataType: "number",
                caption: "GCV (arb)",
                formItem: {
                    editorType: "dxNumberBox",
                    editorOptions: {
                        format: "#,##0.00",
                        step: 0
                    }
                },
                allowEditing: true,
                visible: false
            },
            {
                dataField: "gcv_adb",
                dataType: "number",
                caption: "GCV (adb)",
                formItem: {
                    editorType: "dxNumberBox",
                    editorOptions: {
                        format: "#,##0.00",
                        step: 0
                    }
                },
                allowEditing: true,
                visible: false
            },
            {
                dataField: "ts_adb",
                dataType: "number",
                caption: "TS (adb)",
                formItem: {
                    editorType: "dxNumberBox",
                    editorOptions: {
                        format: "#,##0.00",
                        step: 0
                    }
                },
                allowEditing: true,
                visible: false
            },
            {
                dataField: "ts_arb",
                dataType: "number",
                caption: "TS (arb)",
                formItem: {
                    editorType: "dxNumberBox",
                    editorOptions: {
                        format: "#,##0.00",
                        step: 0
                    }
                },
                allowEditing: true,
                visible: false
            },
            {
                dataField: "ash_adb",
                dataType: "number",
                caption: "Ash (adb)",
                formItem: {
                    editorType: "dxNumberBox",
                    editorOptions: {
                        format: "#,##0.00",
                        step: 0
                    }
                },
                allowEditing: true,
                visible: false
            },
            {
                dataField: "ash_arb",
                dataType: "number",
                caption: "Ash (arb)",
                formItem: {
                    editorType: "dxNumberBox",
                    editorOptions: {
                        format: "#,##0.00",
                        step: 0
                    }
                },
                allowEditing: true,
                visible: false
            },
            {
                dataField: "tm_arb",
                dataType: "number",
                caption: "TM (arb)",
                formItem: {
                    editorType: "dxNumberBox",
                    editorOptions: {
                        format: "#,##0.00",
                        step: 0
                    }
                },
                allowEditing: true,
                visible: false
            },
            {
                dataField: "im_adb",
                dataType: "number",
                caption: "IM (adb)",
                formItem: {
                    editorType: "dxNumberBox",
                    editorOptions: {
                        format: "#,##0.00",
                        step: 0
                    }
                },
                allowEditing: true,
                visible: false
            },
            {
                dataField: "downpayment",
                dataType: "number",
                caption: "Payment",
                formItem: {
                    editorType: "dxNumberBox",
                    editorOptions: {
                        format: "#,##0.00000",
                        step: 0
                    }
                },
                setCellValue: async function (newData, value) {
                    this.defaultSetCellValue(newData, value);
                },
                allowEditing: true,
                allowFiltering: false,
                visible: false,
            },
            {
                dataField: "freight_cost",
                dataType: "number",
                caption: "Freight Cost",
                formItem: {
                    editorType: "dxNumberBox",
                    editorOptions: {
                        format: "#,##0.00",
                        step: 0
                    }
                },
                setCellValue: async function (newData, value) {
                    this.defaultSetCellValue(newData, value);
                },
                allowEditing: true,
                allowFiltering: false,
                visible: false,
            },
            {
                dataField: "insurance_cost",
                dataType: "number",
                caption: "Insurance Cost",
                formItem: {
                    editorType: "dxNumberBox",
                    editorOptions: {
                        format: "#,##0.00",
                        step: 0
                    }
                },
                allowEditing: false,
                allowFiltering: false,
                visible: false,
            },
            {
                dataField: "subtotal",
                dataType: "number",
                caption: "Subtotal",
                formItem: {
                    editorType: "dxNumberBox",
                    editorOptions: {
                        format: "#,##0.00000",
                        step: 0
                    }
                },
                allowEditing: true,
                allowFiltering: false,
                visible: false,
            },
            {
                dataField: "total_price",
                dataType: "number",
                caption: "Total Price",
                format: "#,##0.00000",
                formItem: {
                    editorType: "dxNumberBox",
                    editorOptions: {
                        //format: "fixedPoint",
                        format: "#,##0.00000",
                        step: 0
                    }
                },
                allowEditing: true,
                allowFiltering: false,
                /*calculateCellValue: function (rowData) {
                    return rowData.quantity * rowData.unit_price
                }*/
            },
            {
                dataField: "uom_id",
                dataType: "string",
                caption: "Unit",
                lookup: {
                    dataSource: DevExpress.data.AspNet.createStore({
                        key: "value",
                        loadUrl: "/api/UOM/UOM/UomIdLookup",
                        onBeforeSend: function (method, ajaxOptions) {
                            ajaxOptions.xhrFields = { withCredentials: true };
                            ajaxOptions.beforeSend = function (request) {
                                request.setRequestHeader("Authorization", "Bearer " + token);
                            };
                        }
                    }),
                    valueExpr: "value",
                    displayExpr: "text"
                },
                calculateSortValue: function (data) {
                    var value = this.calculateCellValue(data);
                    return this.lookup.calculateCellValue(value);
                },
                formItem: {
                    editorOptions: {
                        showClearButton: true
                    }
                },
                allowEditing: false,
                visible: false
            },
            {
                dataField: "currency_id",
                dataType: "string",
                visible: false
            },
            {
                dataField: "currency_exchange_id",
                dataType: "string",
                caption: "Exchange Rate",
                visible: false,
                formItem: {
                    editorOptions: {
                        showClearButton: true
                    }
                },
                lookup: {
                    dataSource: function (options) {
                        let _url = "/api/Sales/SalesInvoice/InvoiceCurrencyExchangeIdLookupByDo";

                        if (options !== undefined && options !== null) {
                            if (options.data !== undefined && options.data !== null) {
                                if (options.data.despatch_order_id !== undefined
                                    && options.data.despatch_order_id !== null) {
                                    _url += "?despatch_order_id=" + encodeURIComponent(options.data.despatch_order_id);
                                }
                                if (options.data.invoice_date !== undefined
                                    && options.data.invoice_date !== null) {
                                    var _invoice_date = new Date(options.data.invoice_date);

                                    _url += "&invoice_date=" + encodeURIComponent(_invoice_date.toISOString());
                                }
                            }
                        }
                        //if (options.data) {
                        //    //console.log('currency id:', options.data.currency_id);
                        //    //console.log('invoice date:', options.data.invoice_date);
                        //}
                        return {
                            store: DevExpress.data.AspNet.createStore({
                                key: "value",
                                //loadUrl: "/api/Sales/SalesInvoice/InvoiceCurrencyExchangeIdLookupByDo?despatch_order_id=333333&invoice_date=4444", // + options.data.currency_id,
                                loadUrl: _url,
                                onBeforeSend: function (method, ajaxOptions) {
                                    ajaxOptions.xhrFields = { withCredentials: true };
                                    ajaxOptions.beforeSend = function (request) {
                                        request.setRequestHeader("Authorization", "Bearer " + token);
                                    };
                                }
                            }),
                            //filter: options.data ? [["source_currency_id", "=", options.data.currency_id], "and", ["start_date", "<=", options.data.invoice_date], "and", ["end_date", ">=", options.data.invoice_date]] : null
                        }
                    },
                    valueExpr: "value",
                    displayExpr: function (item) {
                        return item.text + ' (' + formatNumber(item.xchange).toString() + ')'
                    }
                },
            },
            {
                dataField: "print_status",
                dataType: "string",
                caption: "Print Status",
                visible: false,
                lookup: {
                    dataSource: function (options) {
                        return {
                            store: DevExpress.data.AspNet.createStore({
                                key: "value",
                                loadUrl: url + "/PrintStatusLookup",
                                onBeforeSend: function (method, ajaxOptions) {
                                    ajaxOptions.xhrFields = { withCredentials: true };
                                    ajaxOptions.beforeSend = function (request) {
                                        request.setRequestHeader("Authorization", "Bearer " + token);
                                    };
                                }
                            })
                        }
                    },
                    valueExpr: "value",
                    displayExpr: "text"
                },
                calculateSortValue: function (data) {
                    var value = this.calculateCellValue(data);
                    return this.lookup.calculateCellValue(value);
                },
                formItem: {
                    editorOptions: {
                        showClearButton: true
                    }
                },
            },
            {
                dataField: "notes",
                caption: "Notes",
                dataType: "string",
                visible: false
            },
            {
                dataField: "transaction_code",
                caption: "Transferring Bank Reference",
                dataType: "string",
                visible: false
            },
            /*{
                dataField: "credit_limit",
                caption: "Credit Limit",
                dataType: "number",
                visible: false,
                allowEditing: false
            },*/
            {
                dataField: "created_on",
                caption: "Created On",
                dataType: "string",
                visible: false,
                showInColumnChooser: false,
                sortOrder: "desc"
            },
            {
                dataField: "alias1",
                caption: "B/L Date",
                dataType: "date",
                visible: false,
                editorOptions: { readOnly: true },
            },
            {
                dataField: "alias2",
                caption: "COA Date",
                dataType: "date",
                visible: false,
                editorOptions: { readOnly: true },
            },
            {
                dataField: "submit_date",
                caption: "Submit to Bank",
                dataType: "date",
                visible: false,
                formItem: {
                    visible: false
                }
            },
            {
                dataField: "alias3",
                caption: "Submit to Bank",
                dataType: "date",
                visible: false,
            },
            {
                dataField: "alias4",
                caption: "Payment Date",
                dataType: "date",
                visible: false,
            },
            {
                dataField: "alias5",
                caption: "Harga Dasar Transportasi",
                dataType: "number",
                formItem: {
                    colSpan: 2,
                    visible: false
                },
                visible: false,
            },
            {
                dataField: "alias6",
                caption: "P1 (Harga Keekonomian HSD+PPN+PBBKB)",
                dataType: "number",
                formItem: {
                    colSpan: 2,
                    visible: false
                },
                visible: false,
            },
            {
                dataField: "alias7",
                caption: "Harga Dasar Transportasi Bulanan",
                dataType: "number",
                formItem: {
                    colSpan: 2
                },
                visible: false,
            },
            {
                dataField: "alias8",
                caption: "CIF PLN Adjusted",
                formItem: {
                    colSpan: 2,
                    editorType: "dxNumberBox",
                    editorOptions: {
                        format: "#,##0.00",
                        step: 0
                    }
                },
                dataType: "number",
                visible: false,
                setCellValue: function (rowData, value) {
                    rowData.alias8 = value
                },
            },

            {
                dataField: "alias9",
                dataType: "number",
                caption: "Advance Payment",
                formItem: {
                    editorType: "dxNumberBox",
                    editorOptions: {
                        format: "#,##0.00",
                        step: 0
                    }
                },
                // allowEditing: false,
                allowFiltering: false,
                visible: false,
            },
            {
                dataField: "szfreightinvoiceno",
                dataType: "string",
                caption: "Freight Invoice No",
                formItem: {
                    colSpan: 2
                },
                visible: false
            },
            {
                dataField: "decfreightpriceton",
                dataType: "number",
                caption: "Freight Price/ton",
                formItem: {
                    colSpan: 2,
                    editorType: "dxNumberBox",
                    editorOptions: {
                        format: "#,##0.00",
                        step: 0
                    }
                },
                visible: false,
                setCellValue: function (rowData, value) {
                    rowData.decfreightpriceton = value
                },
            },
            {
                dataField: "decfreightamount",
                dataType: "number",
                caption: "Freight Amount",
                formItem: {
                    colSpan: 2,
                    editorType: "dxNumberBox",
                    editorOptions: {
                        format: "#,##0.00",
                        step: 0
                    }
                },
                visible: false
            },
            {
                dataField: "dtmfreightinvoicedatereceive",
                dataType: "datetime",
                caption: "Freight Invoice Date Receive",
                format: "MM/dd/yyyy",
                formItem: {
                    colSpan: 2
                },
                visible: false
            },
            {
                dataField: "szfreightcontractno",
                dataType: "string",
                caption: "Freight Contract No",
                formItem: {
                    colSpan: 2
                },
                visible: false
            },
            {
                dataField: "hba_type",
                dataType: "string",
                visible: false,
                caption: "HBA Type",
                editorType: "dxSelectBox",
                editorOptions: {
                    dataSource: DevExpress.data.AspNet.createStore({
                        key: "value",
                        loadUrl: "/api/Sales/Royalty/Pricing/HBALookup",
                        onBeforeSend: function (method, ajaxOptions) {
                            ajaxOptions.xhrFields = { withCredentials: true };
                            ajaxOptions.beforeSend = function (request) {
                                request.setRequestHeader("Authorization", "Bearer " + token);
                            };
                        }
                    }),
                    valueExpr: "value",
                    displayExpr: "text"
                },
                formItem: {
                    editorOptions: {
                        showClearButton: true
                    }
                },
                setCellValue: function (rowData, value) {
                    rowData.hba_type = value
                },
            },
            {
                dataField: "hba_value",
                dataType: "number",
                visible: false,
                caption: "HBA Value",
                formItem: {
                    editorType: "dxNumberBox",
                    editorOptions: {
                        step: 0
                    }
                }
            },
            {
                dataField: "formula",
                dataType: "string",
                visible: false,
                caption: "Formula",
                editorType: "dxSelectBox",
                editorOptions: {
                    dataSource: DevExpress.data.AspNet.createStore({
                        key: "value",
                        loadUrl: url + "/SalesChargeIdLookup",
                        onBeforeSend: function (method, ajaxOptions) {
                            ajaxOptions.xhrFields = { withCredentials: true };
                            ajaxOptions.beforeSend = function (request) {
                                request.setRequestHeader("Authorization", "Bearer " + token);
                            };
                        }
                    }),
                    searchEnabled: true,
                    valueExpr: "value",
                    displayExpr: "text"
                },
                formItem: {
                    editorOptions: {
                        showClearButton: true
                    }
                },
                setCellValue: function (rowData, value) {
                    rowData.formula = value
                },
            },
            {
                dataField: "hpb",
                dataType: "number",
                caption: "HPB",
                visible: false,
                formItem: {
                    editorType: "dxNumberBox",
                    editorOptions: {
                        step: 0,
                        format: {
                            type: "fixedPoint",
                            precision: 2
                        }
                    }
                }
            },
            {
                dataField: "hpb_value",
                dataType: "number",
                caption: "HPB Value",
                visible: false,
                formItem: {
                    editorType: "dxNumberBox",
                    editorOptions: {
                        step: 0,
                        format: {
                            type: "fixedPoint",
                            precision: 2
                        }
                    }
                }
            },
            {
                dataField: "is_fobprice",
                dataType: "boolean",
                visible: false,
                caption: "Is FOB Price",
                setCellValue: function (rowData, value) {
                    rowData.is_fobprice = value
                },
            },
            {
                dataField: "total_price_hpb",
                dataType: "number",
                visible: false,
                caption: "Total Price with HPB Value",
                formItem: {
                    editorType: "dxNumberBox",
                    editorOptions: {
                        format: "#,##0.00000",
                        step: 0
                    }
                }
            },
            {
                caption: "Detail",
                type: "buttons",
                width: 80,
                showInColumnChooser: false,
                buttons: [{
                    cssClass: "btn-dxdatagrid",
                    hint: "See Terms Detail",
                    text: "Open",
                    onClick: function (e) {
                        salesInvoiceData = e.row.data
                        showSalesInvoicePopup()
                    }
                }]
            },
            {
                caption: "Print",
                type: "buttons",
                width: 80,
                showInColumnChooser: false,
                buttons: [{
                    cssClass: "btn-dxdatagrid",
                    hint: "See Terms Detail",
                    text: "Print",
                    onClick: function (e) {
                        applicationEntityId = e.row.data.entity_id;
                        recordId = e.row.data.id;

                        $("#print-out-list").select2({
                            dropdownParent: $("#print-out-modal .modal-body"),
                            ajax:
                            {
                                url: "/api/Report/ReportTemplate/PrintOutListSelect2/" +
                                    encodeURIComponent(applicationEntityId),
                                headers: {
                                    "Authorization": "Bearer " + token
                                },
                                dataType: 'json',
                                delay: 250,
                                data: function (params) {
                                    return {
                                        q: params.term, // search term
                                        page: params.page
                                    };
                                },
                                cache: true
                            },
                            allowClear: true,
                            minimumInputLength: 0
                        }).on('select2:select', function (e) {
                            var data = e.params.data;
                            reportTemplateId = data.id;

                            $("#print-out-btn").on("click", function () {
                                let printPage = "/Report/PrintOutViewer/Index/"
                                    + "?Id=" + encodeURIComponent(recordId)
                                    + "&reportTemplateId=" + encodeURIComponent(reportTemplateId);
                                window.open(printPage);
                            });
                        }).on('select2:clear', function (e) {
                            reportTemplateId = "";
                        });

                        $("#print-out-modal").modal("show");
                    }
                }]
            },
            {
                type: "buttons",
                buttons: ["edit", "delete"],
                showInColumnChooser: false
            }
        ],
        masterDetail: {
            enabled: true,
            template: function (container, options) {
                var currentRecord = options.data;

                // Sales Invoice Information Container
                renderSalesInvoiceInformation(currentRecord, container);

                salesInvoiceData = currentRecord;
                // Sales Invoice Product Specification
                renderSalesInvoiceProductSpecification(currentRecord, container);
            }
        },
        onInitNewRow: async function (e) {
            let grid = e.component;
            await deleteAdjustmentAndPrice(grid);
            isInserting = true;
            console.log(isInserting);
            ShippingOrderId = null;
        },
        onRowInserted: function (e) {
            isInserting = false;
            ShippingOrderId = null;
        },
        onRowUpdating: function (e) {
            isInserting = false;
            ShippingOrderId = null;
        },
        onEditingStart: function (e) {
            isInserting = true;
            ShippingOrderId = e.data.despatch_order_id
            console.log(isInserting);
            console.log(ShippingOrderId);
        },
        onRowValidating: function (e) {
            if (!e.brokenRules.length)
                return false

            // Ref: https://supportcenter.devexpress.com/ticket/details/t512157/how-to-automatically-focus-the-first-non-validated-field
            e.brokenRules[0].validator.focus();
        },
        onCellPrepared: function (e) {
            salesTypeName = "";
            salesTypeChanged = false;

            if (e.rowType === "data" && e.column.command === "edit") {
                var $links = e.cellElement.find(".dx-link");
                if (e.row.data.approve_status == "APPROVED") {
                    $links.filter(".dx-link-edit").remove();
                    $links.filter(".dx-link-delete").remove();
                }
            }
        },
        onEditorPreparing: function (e) {
            if (e.parentType === 'searchPanel') {
                e.editorOptions.onValueChanged = function (arg) {
                    if (arg.value.length == 0 || arg.value.length > 2) {
                        e.component.searchByText(arg.value);
                    }
                }
            }

            if (e.parentType === "dataRow" && e.dataField == "hba_type") {
                let standardHandler = e.editorOptions.onValueChanged;
                let index = e.row.rowIndex;
                let grid = e.component;
                let rowData = e.row.data;

                e.editorOptions.onValueChanged = async function (e) { // Overiding the standard handler                    
                    // Get its value (Id) on value changed
                    hbaType = e.value;
                    await $.ajax({
                        url: '/api/Sales/SalesInvoice/HBACalculate/' + encodeURIComponent(rowData.despatch_order_id) + '/' + encodeURIComponent(hbaType),
                        type: 'GET',
                        contentType: "application/json",
                        beforeSend: function (xhr) {
                            xhr.setRequestHeader("Authorization", "Bearer " + token);
                        },
                        success: function (response) {
                            grid.beginUpdate()
                            grid.cellValue(index, "hba_value", response)
                            grid.endUpdate()
                        }
                    })

                    standardHandler(e)
                }
            }

            if (e.parentType === "dataRow" && e.dataField == "formula") {
                let standardHandler = e.editorOptions.onValueChanged;
                let index = e.row.rowIndex;
                let grid = e.component;
                let rowData = e.row.data;

                e.editorOptions.onValueChanged = async function (e) { // Overiding the standard handler                    
                    // Get its value (Id) on value changed
                    let salesChargeId = e.value;
                    let despatch_order = rowData.despatch_order_id;
                    let hbaValue = rowData.hba_value;
                    let quantity = rowData.quantity;
                    await $.ajax({
                        url: '/api/Sales/SalesInvoice/CalculateFormulaById/' + encodeURIComponent(despatch_order) + "/" + encodeURIComponent(salesChargeId)
                            + "/" + encodeURIComponent(hbaValue),
                        type: 'GET',
                        contentType: "application/json",
                        beforeSend: function (xhr) {
                            xhr.setRequestHeader("Authorization", "Bearer " + token);
                        },
                        success: function (response) {
                            let hpb = response.value;
                            let hpbValue = hbaValue + hpb;
                            let totalPrice = hpbValue * quantity;
                            grid.beginUpdate()
                            grid.cellValue(index, "hpb", response.value)
                            grid.cellValue(index, "hpb_value", hpbValue)
                            grid.cellValue(index, "total_price_hpb", totalPrice)
                            grid.endUpdate()
                        }
                    })

                    standardHandler(e)
                }
            }

            if (e.parentType === "dataRow" && e.dataField == "sales_type_id") {
                let standardHandler = e.editorOptions.onValueChanged;
                let index = e.row.rowIndex;
                let grid = e.component;

                e.editorOptions.onValueChanged = async function (e) { // Overiding the standard handler                    
                    // Get its value (Id) on value changed
                    salesTypeId = e.value;

                    // Get another data from API after getting the Id
                    await $.ajax({
                        url: '/api/General/MasterList/MasterListIdStatus?Id=' + salesTypeId,
                        type: 'GET',
                        contentType: "application/json",
                        beforeSend: function (xhr) {
                            xhr.setRequestHeader("Authorization", "Bearer " + token);
                        },
                        success: function (response) {
                            let mlData = response.data[0];
                            salesTypeName = mlData.text.trim().toUpperCase();
                            salesTypeChanged = true;
                        }
                    })

                    standardHandler(e) // Calling the standard handler to save the edited value
                }
            }

            if (e.parentType === "dataRow" && e.dataField == "is_fobprice") {
                let standardHandler = e.editorOptions.onValueChanged;
                let index = e.row.rowIndex;
                let grid = e.component;
                let rowData = e.row.data;
                e.editorOptions.onValueChanged = async function (e) { // Overiding the standard handler                    
                    // Get its value (Id) on value changed
                    let is_true = e.value;
                    if (is_true) {
                        let quantity = rowData.quantity;
                        let hpbValue = rowData.hpb_value;
                        let total = quantity * hpbValue;
                        grid.beginUpdate()
                        grid.cellValue(index, "total_price_hpb", total)
                        grid.endUpdate()
                    }
                    // Get another data from API after getting the Id
                    standardHandler(e) // Calling the standard handler to save the edited value
                }
            }

            if (e.parentType === "dataRow" && e.dataField == "downpayment") {
                if (salesTypeChanged == false || salesTypeName == "")
                    salesTypeName = e.row.data.sales_type_name;

                if (salesTypeName == undefined) salesTypeName = "";
                else salesTypeName = salesTypeName.trim().toUpperCase();

                if (salesTypeName == "DOMESTIC (MANUAL)" || salesTypeName == "EXPORT (MANUAL)" || salesTypeName == "PLN")
                    e.editorOptions.disabled = false;
                else
                    e.editorOptions.disabled = true;
            }

            if (e.parentType === "dataRow" && e.dataField == "alias9") {
                if (salesTypeChanged == false || salesTypeName == "")
                    salesTypeName = e.row.data.sales_type_name;

                if (salesTypeName == undefined) salesTypeName = "";
                else salesTypeName = salesTypeName.trim().toUpperCase();

                if (salesTypeName == "DOMESTIC (MANUAL)" || salesTypeName == "EXPORT (MANUAL)" || salesTypeName == "PLN")
                    e.editorOptions.disabled = false;
                else
                    e.editorOptions.disabled = true;
            }

            if (e.parentType === "dataRow" && e.dataField == "unit_price") {
                if (salesTypeChanged == false || salesTypeName == "")
                    salesTypeName = e.row.data.sales_type_name;

                if (salesTypeName == undefined) salesTypeName = "";
                else salesTypeName = salesTypeName.trim().toUpperCase();

                /*if (salesTypeName == "DOMESTIC (MANUAL)")
                    e.editorOptions.disabled = false;
                else
                    e.editorOptions.disabled = true*/
            }

            if (e.parentType === "dataRow" && e.dataField == "total_price") {
                if (salesTypeChanged == false || salesTypeName == "")
                    salesTypeName = e.row.data.sales_type_name;

                if (salesTypeName == undefined) salesTypeName = "";
                else salesTypeName = salesTypeName.trim().toUpperCase();

                if (salesTypeName == "DOMESTIC (MANUAL)" || salesTypeName == "EXPORT (MANUAL)" || salesTypeName == "PLN")
                    e.editorOptions.disabled = false;
                else
                    e.editorOptions.disabled = true;
            }

            if (e.parentType === "dataRow" && e.dataField == "freight_cost") {
                if (salesTypeChanged == false || salesTypeName == "")
                    salesTypeName = e.row.data.sales_type_name;

                if (salesTypeName == undefined) salesTypeName = "";
                else salesTypeName = salesTypeName.trim().toUpperCase();

                if (salesTypeName == "PLN")
                    e.editorOptions.disabled = false;
                else
                    e.editorOptions.disabled = true;
            }

            // Set onValueChanged for despatch_order_id
            if (e.parentType === "dataRow" && e.dataField == "refreshButton") {
                let standardHandler = e.editorOptions.onValueChanged
                let index = e.row.rowIndex
                let grid = e.component
                let rowData = e.row.data


                if (rowData.despatch_order_id && rowData.invoice_type_id && rowData.quotation_type_id) {
                    e.editorOptions = {
                        text: "Recalculate",
                        icon: "refresh",
                        disabled: false,
                        onClick: function () {
                            let rowData = e.row.data;
                            let despatchOrderId = rowData.despatch_order_id;
                            let invoiceTypeId = rowData.invoice_type_id;
                            let quotationTypeId = rowData.quotation_type_id;
                            let salesContractId = rowData.sales_contract_id;
                            let salesContractTermId = rowData.sales_contract_term_id;

                            if (despatchOrderId && invoiceTypeId && quotationTypeId) {
                                setAdjustmentAndPrice(
                                    e.component,
                                    rowData,
                                    e.row.rowIndex,
                                    despatchOrderId,
                                    salesContractId,
                                    salesContractTermId,
                                    invoiceTypeId,
                                    quotationTypeId
                                );
                                $.ajax({
                                    url: url + '/DespatchOrderDetail?Id=' + despatchOrderId,
                                    type: 'GET',
                                    contentType: "application/json",
                                    beforeSend: function (xhr) {
                                        xhr.setRequestHeader("Authorization", "Bearer " + token);
                                    },
                                    success: function (response) {
                                        let despatchOrderData = response.data[0]

                                        grid.beginUpdate()
                                        if (despatchOrderData.draft_survey_quantity == null) { _qty = despatchOrderData.required_quantity }
                                        else { _qty = despatchOrderData.draft_survey_quantity }
                                        // Set its corresponded field's value
                                        grid.cellValue(index, "inco_term", despatchOrderData.delivery_term_id)
                                        grid.endUpdate()
                                    }
                                })
                            }
                        }
                    };
                    e.editorName = "dxButton";
                }


            }
            if (e.parentType === "dataRow" && e.dataField == "despatch_order_id") {

                let standardHandler = e.editorOptions.onValueChanged
                let index = e.row.rowIndex
                let grid = e.component
                let rowData = e.row.data

                e.editorOptions.onValueChanged = async function (e) { // Overiding the standard handler                    

                    // Get its value (Id) on value changed
                    let despatchOrderId = e.value

                    grid.beginCustomLoading()

                    await deleteAdjustmentAndPrice(grid, index)

                    var _qty = 0;

                    // Get another data from API after getting the Id
                    await $.ajax({
                        url: url + '/DespatchOrderDetail?Id=' + despatchOrderId,
                        type: 'GET',
                        contentType: "application/json",
                        beforeSend: function (xhr) {
                            xhr.setRequestHeader("Authorization", "Bearer " + token);
                        },
                        success: function (response) {
                            let despatchOrderData = response.data[0]

                            grid.beginUpdate()
                            if (despatchOrderData.draft_survey_quantity == null) { _qty = despatchOrderData.required_quantity }
                            else { _qty = despatchOrderData.draft_survey_quantity }

                            // Set its corresponded field's value
                            grid.cellValue(index, "sales_contract_id", despatchOrderData.sales_contract_id)
                            grid.cellValue(index, "sales_contract_term_id", despatchOrderData.contract_term_id)
                            grid.cellValue(index, "customer_id", despatchOrderData.customer_id)
                            grid.cellValue(index, "seller_id", despatchOrderData.seller_id)
                            grid.cellValue(index, "bill_to", despatchOrderData.customer_id)
                            grid.cellValue(index, "contract_product_id", despatchOrderData.contract_product_id)
                            grid.cellValue(index, "loading_port_id", despatchOrderData.loading_port)
                            grid.cellValue(index, "discharge_port", despatchOrderData.discharge_port)
                            grid.cellValue(index, "lc_number", despatchOrderData.letter_of_credit)
                            grid.cellValue(index, "quantity", _qty)
                            grid.cellValue(index, "uom_id", despatchOrderData.uom_id)
                            grid.cellValue(index, "currency_id", despatchOrderData.currency_id)
                            grid.cellValue(index, "inco_term", despatchOrderData.delivery_term_id)
                            grid.cellValue(index, "invoice_number", despatchOrderData.invoice_number)
                            grid.endUpdate()
                        }
                    })

                    $.ajax({
                        url: url + '/TsAdb?Id=' + despatchOrderId,
                        type: 'GET',
                        contentType: "application/json",
                        beforeSend: function (xhr) {
                            xhr.setRequestHeader("Authorization", "Bearer " + token);
                        },
                        success: function (response) {
                            let tsAdb = response.data[0]
                            if (tsAdb == null || 0) {
                                //grid.gcvArb = gcvArb
                                grid.beginUpdate()
                                grid.cellValue(index, "ts_adb", 0)
                                grid.endUpdate()
                            }
                            else {
                                //grid.gcvArb = gcvArb
                                grid.beginUpdate()
                                grid.cellValue(index, "ts_adb", tsAdb.text)
                                grid.endUpdate()
                            }
                        }
                    })
                    $.ajax({
                        url: url + '/ShippingInstructionDetail?Id=' + despatchOrderId,
                        type: 'GET',
                        contentType: "application/json",
                        beforeSend: function (xhr) {
                            xhr.setRequestHeader("Authorization", "Bearer " + token);
                        },
                        success: function (response) {
                            let data = response.data[0]
                            if (data) {
                                grid.beginUpdate()
                                grid.cellValue(index, "correspondent_bank_id", data.issuing_bank)
                                grid.cellValue(index, "bank_account_id", data.advising_bank)
                                grid.endUpdate()
                            }
                        }
                    })
                    $.ajax({
                        url: url + '/TsArb?Id=' + despatchOrderId,
                        type: 'GET',
                        contentType: "application/json",
                        beforeSend: function (xhr) {
                            xhr.setRequestHeader("Authorization", "Bearer " + token);
                        },
                        success: function (response) {
                            let tsAdb = response.data[0]
                            if (tsAdb == null || 0) {
                                //grid.gcvArb = gcvArb
                                grid.beginUpdate()
                                grid.cellValue(index, "ts_arb", 0)
                                grid.endUpdate()
                            }
                            else {
                                //grid.gcvArb = gcvArb
                                grid.beginUpdate()
                                grid.cellValue(index, "ts_arb", tsAdb.text)
                                grid.endUpdate()
                            }
                        }
                    })

                    /* $.ajax({
                         url: '/Api/ContractManagement/ShippingCost/DataDetailByDO?Id=' + despatchOrderId,
                         type: 'GET',
                         contentType: "application/json",
                         beforeSend: function (xhr) {
                             xhr.setRequestHeader("Authorization", "Bearer " + token);
                         },
                         success: function (response) {
                             let tsAdb = response.data[0]
                             if (tsAdb == null || 0) {
                                 //grid.gcvArb = gcvArb
                                 grid.beginUpdate()
                                 grid.cellValue(index, "ts_adb", 0)
                                 grid.endUpdate()
                             }
                             else {
                                 //grid.gcvArb = gcvArb
                                 grid.beginUpdate()
                                 grid.cellValue(index, "ts_adb", tsAdb.text)
                                 grid.endUpdate()
                             }
                         }
                     })
 
                     /* $.ajax({
                          url: '/Api/ContractManagement/ShippingCost/DataDetailByDO?Id=' + despatchOrderId,
                          type: 'GET',
                          contentType: "application/json",
                          beforeSend: function (xhr) {
                              xhr.setRequestHeader("Authorization", "Bearer " + token);
                          },
                          success: function (response) {
                              let data = response.data[0]
                              var result = data.quantity * data.advance_payment;
                              if (result == null || 0) {
                                  //grid.gcvArb = gcvArb
                                  grid.beginUpdate()
                                  grid.cellValue(index, "alias9", 0)
                                  grid.endUpdate()
                              }
                              else {
                                  //grid.gcvArb = gcvArb
                                  grid.beginUpdate()
                                  grid.cellValue(index, "alias9", result)
                                  grid.endUpdate()
                              }
                          }
                      })
  */
                    $.ajax({
                        url: url + '/ImAdb?Id=' + despatchOrderId,
                        type: 'GET',
                        contentType: "application/json",
                        beforeSend: function (xhr) {
                            xhr.setRequestHeader("Authorization", "Bearer " + token);
                        },
                        success: function (response) {
                            let imAdb = response.data[0]
                            if (imAdb == null || 0) {
                                //grid.gcvArb = gcvArb
                                grid.beginUpdate()
                                grid.cellValue(index, "im_adb", 0)
                                grid.endUpdate()
                            }
                            else {
                                //grid.gcvArb = gcvArb
                                grid.beginUpdate()
                                grid.cellValue(index, "im_adb", imAdb.text)
                                grid.endUpdate()
                            }
                        }
                    })

                    $.ajax({
                        url: url + '/TmArb?Id=' + despatchOrderId,
                        type: 'GET',
                        contentType: "application/json",
                        beforeSend: function (xhr) {
                            xhr.setRequestHeader("Authorization", "Bearer " + token);
                        },
                        success: function (response) {
                            let tmArb = response.data[0]
                            if (tmArb == null || 0) {
                                //grid.gcvArb = gcvArb
                                grid.beginUpdate()
                                grid.cellValue(index, "tm_arb", 0)
                                grid.endUpdate()
                            }
                            else {
                                //grid.gcvArb = gcvArb
                                grid.beginUpdate()
                                grid.cellValue(index, "tm_arb", tmArb.text)
                                grid.endUpdate()
                            }
                        }
                    })

                    $.ajax({
                        url: url + '/GcvArb?Id=' + despatchOrderId,
                        type: 'GET',
                        contentType: "application/json",
                        beforeSend: function (xhr) {
                            xhr.setRequestHeader("Authorization", "Bearer " + token);
                        },
                        success: function (response) {
                            let gcvArb = response.data[0]
                            if (gcvArb == null || 0) {
                                //grid.gcvArb = gcvArb
                                grid.beginUpdate()
                                grid.cellValue(index, "gcv_arb", 0)
                                grid.endUpdate()
                            }
                            else {
                                //grid.gcvArb = gcvArb
                                grid.beginUpdate()
                                grid.cellValue(index, "gcv_arb", gcvArb.text)
                                grid.endUpdate()
                            }
                        }
                    })

                    $.ajax({
                        url: url + '/GcvAdb?Id=' + despatchOrderId,
                        type: 'GET',
                        contentType: "application/json",
                        beforeSend: function (xhr) {
                            xhr.setRequestHeader("Authorization", "Bearer " + token);
                        },
                        success: function (response) {
                            let gcvArb = response.data[0]
                            if (gcvArb == null || 0) {
                                //grid.gcvArb = gcvArb
                                grid.beginUpdate()
                                grid.cellValue(index, "gcv_adb", 0)
                                grid.endUpdate()
                            }
                            else {
                                //grid.gcvArb = gcvArb
                                grid.beginUpdate()
                                grid.cellValue(index, "gcv_adb", gcvArb.text)
                                grid.endUpdate()
                            }
                        }
                    })

                    $.ajax({
                        url: url + '/AshAdb?Id=' + despatchOrderId,
                        type: 'GET',
                        contentType: "application/json",
                        beforeSend: function (xhr) {
                            xhr.setRequestHeader("Authorization", "Bearer " + token);
                        },
                        success: function (response) {
                            let ashAdb = response.data[0]
                            if (ashAdb == null || 0) {
                                //grid.gcvArb = gcvArb
                                grid.beginUpdate()
                                grid.cellValue(index, "ash_adb", 0)
                                grid.endUpdate()
                            }
                            else {
                                //grid.gcvArb = gcvArb
                                grid.beginUpdate()
                                grid.cellValue(index, "ash_adb", ashAdb.text)
                                grid.endUpdate()
                            }
                        }
                    })
                    $.ajax({
                        url: url + '/AshArb?Id=' + despatchOrderId,
                        type: 'GET',
                        contentType: "application/json",
                        beforeSend: function (xhr) {
                            xhr.setRequestHeader("Authorization", "Bearer " + token);
                        },
                        success: function (response) {
                            let ashAdb = response.data[0]
                            if (ashAdb == null || 0) {
                                //grid.gcvArb = gcvArb
                                grid.beginUpdate()
                                grid.cellValue(index, "ash_arb", 0)
                                grid.endUpdate()
                            }
                            else {
                                //grid.gcvArb = gcvArb
                                grid.beginUpdate()
                                grid.cellValue(index, "ash_arb", ashAdb.text)
                                grid.endUpdate()
                            }
                        }
                    })
                    $.ajax({
                        url: '/api/StockpileManagement/QualitySampling/DataDetailByDespatchOrder?Id=' + despatchOrderId,
                        type: 'GET',
                        contentType: "application/json",
                        beforeSend: function (xhr) {
                            xhr.setRequestHeader("Authorization", "Bearer " + token);
                        },
                        success: function (response) {
                            let data = response.data[0]

                            if (data) {
                                //grid.gcvArb = gcvArb
                                grid.beginUpdate()
                                grid.cellValue(index, "alias2", data.sampling_datetime)
                                grid.endUpdate()
                            }
                        }
                    })
                    $.ajax({
                        url: '/api/SurveyManagement/COW/DataDetailByDespatchOrder?Id=' + despatchOrderId,
                        type: 'GET',
                        contentType: "application/json",
                        beforeSend: function (xhr) {
                            xhr.setRequestHeader("Authorization", "Bearer " + token);
                        },
                        success: function (response) {
                            let data = response.data[0]

                            if (data) {
                                //grid.gcvArb = gcvArb
                                grid.beginUpdate()
                                grid.cellValue(index, "alias1", data.bill_lading_date)
                                grid.endUpdate()
                            }
                        }
                    })

                    $.ajax({
                        url: url + '/ShippingCostDetail?Id=' + despatchOrderId,
                        type: 'GET',
                        contentType: "application/json",
                        beforeSend: function (xhr) {
                            xhr.setRequestHeader("Authorization", "Bearer " + token);
                        },
                        success: function (response) {
                            if (response.data.length == 0) {
                                grid.beginUpdate()
                                grid.cellValue(index, "freight_cost", 0)
                                grid.cellValue(index, "insurance_cost", 0)
                                grid.endUpdate()
                            }
                            else {
                                let shippingCostData = response.data[0]

                                grid.beginUpdate()
                                //grid.cellValue(index, "freight_cost", shippingCostData.freight_rate * _qty)
                                grid.cellValue(index, "freight_cost", shippingCostData.freight_rate * shippingCostData.quantity)
                                grid.cellValue(index, "insurance_cost", shippingCostData.insurance_cost)

                                grid.endUpdate()

                                var result = shippingCostData.quantity * shippingCostData.advance_payment;
                                if (result == null || 0) {
                                    //grid.gcvArb = gcvArb
                                    grid.beginUpdate()
                                    grid.cellValue(index, "alias9", 0)
                                    grid.endUpdate()
                                }
                                else {
                                    //grid.gcvArb = gcvArb
                                    grid.beginUpdate()
                                    grid.cellValue(index, "alias9", result)
                                    grid.endUpdate()
                                }
                            }
                        }
                    })

                    setTimeout(() => {
                        grid.endCustomLoading()
                    }, 500)

                    standardHandler(e) // Calling the standard handler to save the edited value
                }

            }

            // Enable and Set Despatch Order Link button after Despatch Order selected
            if (e.parentType === "dataRow" && e.dataField == "despatch_order_link") {
                if (e.row.data.despatch_order_id) {
                    let despatchOrderId = e.row.data.despatch_order_id

                    e.editorOptions.onClick = function (e) {
                        window.open("/Sales/DespatchOrder/Index?Id=" + despatchOrderId + "&openEditingForm=true", "_blank")
                    }
                    e.editorOptions.disabled = false
                }
            }

            if (e.parentType === "dataRow" && e.dataField == "invoice_type_id") {

                let standardHandler = e.editorOptions.onValueChanged
                let index = e.row.rowIndex
                let grid = e.component
                let rowData = e.row.data

                isEditing = e.row.isNewRow != true;

                // Disabled if despatch order id is null
                e.editorOptions.disabled = !rowData.despatch_order_id

                if (rowData.sales_type_id !== null && rowData.sales_type_id !== '682ac4d955ad4028bfb382b3bdcf549a') {
                    // Set lookup datasource by Despatch Order Id
                    e.editorOptions.dataSource = DevExpress.data.AspNet.createStore({
                        key: "value",
                        loadUrl: "/api/Sales/SalesInvoice/LookupInvoiceTypeOnDespatchOrder?despatch_order_id=" + rowData.despatch_order_id,
                        onBeforeSend: function (method, ajaxOptions) {
                            ajaxOptions.xhrFields = { withCredentials: true };
                            ajaxOptions.beforeSend = function (request) {
                                request.setRequestHeader("Authorization", "Bearer " + token);
                            };
                        }
                    })

                    // Set onValueChanged for invoice_type_id
                    e.editorOptions.onValueChanged = function (e) { // Overiding the standard handler

                        // Get its value (Id) on value changed
                        let invoiceTypeId = e.value
                        let despatchOrderId = rowData.despatch_order_id
                        let quotationTypeId = rowData.quotation_type_id
                        let salesContractId = rowData.sales_contract_id
                        let salesContractTermId = rowData.sales_contract_term_id
                        // //console.log('sales invoice invo:', rowData);
                        // //console.log('sales invoice invo:', despatchOrderId, '-', invoiceTypeId, '-', quotationTypeId);
                        if (despatchOrderId && invoiceTypeId && quotationTypeId) {
                            setAdjustmentAndPrice(grid, rowData, index, despatchOrderId, salesContractId, salesContractTermId, invoiceTypeId, quotationTypeId)
                            //setPriceIndex(grid, rowData, index, despatchOrderId, salesContractId, salesContractTermId, invoiceTypeId, quotationTypeId)
                        }

                        standardHandler(e) // Calling the standard handler to save the edited value
                    }
                }
            }

            if (e.parentType === "dataRow" && e.dataField == "quotation_type_id") {

                let standardHandler = e.editorOptions.onValueChanged
                let index = e.row.rowIndex
                let grid = e.component
                let rowData = e.row.data

                isEditing = e.row.isNewRow != true;

                // Disabled if despatch order id is null
                e.editorOptions.disabled = !rowData.despatch_order_id

                // Set lookup datasource by Despatch Order Id
                e.editorOptions.dataSource = DevExpress.data.AspNet.createStore({
                    key: "value",
                    loadUrl: "/api/Sales/SalesInvoice/LookupQuotationTypeOnDespatchOrder?despatch_order_id=" + rowData.despatch_order_id,
                    onBeforeSend: function (method, ajaxOptions) {
                        ajaxOptions.xhrFields = { withCredentials: true };
                        ajaxOptions.beforeSend = function (request) {
                            request.setRequestHeader("Authorization", "Bearer " + token);
                        };
                    }
                })

                // Set onValueChanged for quotation_type_id
                e.editorOptions.onValueChanged = function (e) { // Overiding the standard handler
                    // Get its value (Id) on value changed
                    let quotationTypeId = e.value
                    let despatchOrderId = rowData.despatch_order_id
                    let invoiceTypeId = rowData.invoice_type_id
                    let salesContractId = rowData.sales_contract_id
                    let salesContractTermId = rowData.sales_contract_term_id
                    // //console.log('sales invoice quot:',rowData);
                    // //console.log('sales invoice quot:', despatchOrderId, '-', invoiceTypeId, '-', quotationTypeId);
                    if (despatchOrderId && invoiceTypeId && quotationTypeId) {
                        setAdjustmentAndPrice(grid, rowData, index, despatchOrderId, salesContractId, salesContractTermId, invoiceTypeId, quotationTypeId)
                        //setPriceIndex(grid, rowData, index, despatchOrderId, salesContractId, salesContractTermId, invoiceTypeId, quotationTypeId)
                    }
                    standardHandler(e) // Calling the standard handler to save the edited value
                }
            }

            //if (e.parentType == "dataRow" && e.dataField == "invoice_number" && !e.row.isNewRow) {
            //    let index = e.row.rowIndex
            //    let grid = e.component
            //    let rowData = e.row.data

            //    // editIsOpenned state is used for prevent weird looping
            //    // if addColumn is used in this onEditorPreparing
            //    // //console.log("editIsOpenned", grid.editIsOpenned)
            //    if (!grid.editIsOpenned) {
            //        grid.editIsOpenned = true

            //        setTimeout(async () => {
            //            await getAdjustmentAndPrice(grid, rowData, index, rowData.id)
            //            grid.editIsOpenned = false
            //        }, 500)
            //    }
            //}

            if (e.parentType === "dataRow" && e.dataField == "alias8") {

                let standardHandler = e.editorOptions.onValueChanged
                let index = e.row.rowIndex
                let grid = e.component
                let rowData = e.row.data
                e.editorOptions.onValueChanged = async function (e) {
                    let quantityCif = e.value;
                    if (rowData.decfreightpriceton !== null && rowData.decfreightpriceton > 0) {
                        let amount = rowData.decfreightpriceton;
                        let result = quantityCif * amount;
                        grid.beginUpdate();
                        grid.cellValue(index, "decfreightamount", result)
                        grid.endUpdate();
                    }
                    standardHandler(e)
                }
            }

            if (e.parentType === "dataRow" && e.dataField == "decfreightpriceton") {

                let standardHandler = e.editorOptions.onValueChanged
                let index = e.row.rowIndex
                let grid = e.component
                let rowData = e.row.data
                e.editorOptions.onValueChanged = async function (e) {
                    let amount = e.value;
                    if (rowData.alias8 !== null && rowData.alias8 > 0) {
                        let quantityCif = rowData.alias8;
                        let result = quantityCif * amount;
                        grid.beginUpdate();
                        grid.cellValue(index, "decfreightamount", result)
                        grid.endUpdate();
                    }
                    standardHandler(e)
                }
            }
        },
        masterDetail: {
            enabled: true,
            template: function (container, options) {
                var currentRecord = options.data;

                masterDetailTemplate(currentRecord, container)
            }

        },
        filterRow: {
            visible: true
        },
        headerFilter: {
            visible: true
        },
        groupPanel: {
            visible: true
        },
        searchPanel: {
            visible: true,
            width: 240,
            placeholder: "Search..."
        },
        filterPanel: {
            visible: true
        },
        filterBuilderPopup: {
            position: { of: window, at: "top", my: "top", offset: { y: 10 } },
        },
        columnChooser: {
            enabled: true,
            mode: "select"
        },
        paging: {
            pageSize: 20
        },
        pager: {
            allowedPageSizes: [10, 20, 50, 100],
            showNavigationButtons: true,
            showPageSizeSelector: true,
            showInfo: true,
            visible: true
        },
        height: 800,
        showBorders: true,
        editing: {
            mode: "popup",
            allowAdding: true,
            allowUpdating: true,
            allowDeleting: true,
            useIcons: true,
            form: {
                colCount: 1,
                items: [
                    {
                        itemType: "group",
                        caption: "Contract",
                        colCount: 2,
                        items: [
                            {
                                dataField: "despatch_order_id",
                                colSpan: 2
                            },
                            {
                                dataField: "despatch_order_link",
                                editorType: "dxButton",
                                editorOptions: {
                                    text: "See Shipping Order Detail",
                                    disabled: true
                                }
                            },
                            {

                            },
                            {
                                dataField: "invoice_number",
                            },
                            {
                                dataField: "sales_type_id"
                            },
                            {
                                dataField: "quotation_type_id"
                            },
                            {
                                dataField: "invoice_type_id"
                            },
                            {
                                dataField: "refreshButton",
                                editorType: "dxButton",
                                editorOptions: {
                                    text: "Recalculate",
                                    icon: "refresh",
                                    disabled: true
                                }
                            },
                            {
                                dataField: "invoice_date"
                            },
                            {
                                dataField: "sales_contract_id"
                            },
                            {
                                dataField: "sales_contract_term_id"
                            },
                            {
                                dataField: "contract_product_id",
                                colSpan: 2
                            },
                            {
                                dataField: "loading_port_id",
                            },
                            {
                                dataField: "discharge_port",
                            },
                            {
                                dataField: "consignee",

                            },
                            {
                                dataField: "description_of_goods",
                            },
                            {
                                dataField: "inco_term",
                            },
                        ]
                    },
                    {
                        itemType: "group",
                        caption: "Billing",
                        colCount: 2,
                        items: [
                            {
                                dataField: "seller_id",
                            },
                            {
                                dataField: "customer_id"
                            },
                            {
                                dataField: "bank_account_id",
                            },
                            {
                                dataField: "correspondent_bank_id",
                            },
                            {
                                dataField: "bill_to"
                            },
                            {
                                dataField: "lc_number"
                            },
                            //{
                            //    dataField: "lc_status"
                            //},
                            {
                                dataField: "lc_date_issue"
                            },
                            {
                                dataField: "lc_issuing_bank"
                            },
                        ]
                    },
                    {
                        itemType: "group",
                        caption: "Adjustment & Price",
                        colCount: 2,
                        items: [
                            {
                                dataField: "quantity"
                            },
                            {
                                dataField: "uom_id",
                            },
                            {
                                dataField: "alias1"
                            },
                            {
                                dataField: "alias2"
                            },
                            {
                                dataField: "alias3"
                            },
                            //{
                            //    dataField: "alias4"
                            //},
                            {
                                dataField: "unit_price",
                            },
                            {
                                dataField: "fob_price",
                            },
                            {
                                dataField: "gcv_arb",
                            },
                            {
                                dataField: "gcv_adb",
                            },
                            {
                                dataField: "ts_arb",
                            },
                            {
                                dataField: "ts_adb",
                            },
                            {
                                dataField: "ash_arb",
                            },
                            {
                                dataField: "ash_adb",
                            },
                            {
                                dataField: "tm_arb",
                            },
                            {
                                dataField: "im_adb",
                            },
                            /*{
                                dataField: "cv_price_adj", gcv_adb ts_arb ash_arb
                                colSpan: 2
                            },
                            {
                                dataField: "ash_price_adj",
                                colSpan: 2
                            },
                            {
                                dataField: "ts_price_adj",
                                colSpan: 2
                            },*/
                            {
                                dataField: "subtotal",
                                colSpan: 2
                            },
                            {
                                dataField: "freight_cost",
                                colSpan: 2
                            },
                            {
                                dataField: "insurance_cost",
                                colSpan: 2
                            },
                            {
                                dataField: "alias9",
                                colSpan: 2
                            },
                            {
                                dataField: "total_price",
                                colSpan: 2
                            },
                            {
                                dataField: "downpayment",
                                colSpan: 2
                            },
                        ]
                    },
                    {
                        itemType: "group",
                        caption: "Other",
                        colCount: 2,
                        items: [
                            {
                                dataField: "currency_id",
                                visible: false
                            },
                            {
                                dataField: "currency_exchange_id",
                                editorType: "dxSelectBox",
                                colSpan: 2
                            },
                            {
                                dataField: "notes",
                                editorType: "dxTextArea",
                                editorOptions: {
                                    height: 50
                                },
                                colSpan: 2
                            },
                            {
                                dataField: "transaction_code",
                                colSpan: 2
                            },
                            {
                                dataField: "print_status",
                                colSpan: 2
                            },
                            /*{
                                dataField: "credit_limit",
                                dataType: "number",
                            },*/
                        ]
                    },
                    {
                        itemType: "group",
                        caption: "HPB",
                        colCount: 2,
                        items: [
                            {
                                dataField: "hba_type",
                            },
                            {
                                dataField: "hba_value"
                            },
                            {
                                dataField: "formula"
                            },
                            {
                                dataField: "hpb"
                            },
                            {
                                dataField: "hpb_value"
                            },
                            {
                                dataField: "is_fobprice"
                            },
                            {
                                dataField: "total_price_hpb"
                            }
                        ]
                    },
                    {
                        itemType: "group",
                        caption: "PLN",
                        colCount: 2,
                        items: [
                            {
                                dataField: "alias5",
                                colspan: 2
                            },
                            {
                                dataField: "alias6",
                                colspan: 2
                            },
                            {
                                dataField: "alias7",
                                colspan: 2
                            },
                            {
                                dataField: "alias8",
                                colspan: 2
                            },
                        ]
                    },
                    {
                        itemType: "group",
                        caption: "Invoice Freight ETI / 3rd Party",
                        colCount: 2,
                        items: [
                            {
                                dataField: "szfreightinvoiceno"
                            },
                            {
                                dataField: "decfreightpriceton"
                            },
                            {
                                dataField: "decfreightamount"
                            },
                            {
                                dataField: "dtmfreightinvoicedatereceive"
                            },
                            {
                                dataField: "szfreightcontractno"
                            }
                        ]
                    }
                ]
            }
        },
        grouping: {
            contextMenuEnabled: true,
            autoExpandAll: false
        },
        rowAlternationEnabled: true,
        export: {
            enabled: true,
            allowExportSelectedData: true
        },
        onExporting: function (e) {
            var workbook = new ExcelJS.Workbook();
            var worksheet = workbook.addWorksheet(entityName);

            DevExpress.excelExporter.exportDataGrid({
                component: e.component,
                worksheet: worksheet,
                autoFilterEnabled: true
            }).then(function () {
                // https://github.com/exceljs/exceljs#writing-xlsx
                workbook.xlsx.writeBuffer().then(function (buffer) {
                    saveAs(new Blob([buffer], { type: 'application/octet-stream' }), entityName + '.xlsx');
                });
            });
            e.cancel = true;
        }
    });
    /** bob
   */

    $('#btnUpload').on('click', function () {
        var f = $("#fUpload")[0].files;
        var filename = $('#fUpload').val();

        if (f.length == 0) {
            alert("Please select a file.");
            return false;
        }
        else {
            var fileExtension = ['xlsx', 'xlsm', 'xls'];
            var extension = filename.replace(/^.*\./, '');
            if ($.inArray(extension, fileExtension) == -1) {
                alert("Please select only Excel files.");
                return false;
            }
        }

        $('#btnUpload')
            .html('<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span> Uploading ...');

        var reader = new FileReader();
        reader.readAsDataURL(f[0]);
        reader.onload = function () {
            var formData = {
                "filename": f[0].name,
                "filesize": f[0].size,
                "data": reader.result.split(',')[1]
            };
            $.ajax({
                url: "/api/Sales/SalesInvoice/UploadDocument",
                type: 'POST',
                cache: false,
                contentType: "application/json",
                data: JSON.stringify(formData),
                headers: {
                    "Authorization": "Bearer " + token
                }
            }).done(function (result) {
                alert('File berhasil di-upload!');
                //location.reload();
                $("#modal-upload-file").modal('hide');
                $("#grid").dxDataGrid("refresh");
            }).fail(function (jqXHR, textStatus, errorThrown) {
                window.location = '/General/General/UploadError';
                alert('File gagal di-upload!');
            }).always(function () {
                $('#btnUpload').html('Upload');
            });
        };
        reader.onerror = function (error) {
            alert('Error: ' + error);
        };
    });
    
    function masterDetailTemplate(masterDetailOptions, container) {
        renderSalesInvoiceInformation(masterDetailOptions, container);

        $("<div>").dxTabPanel({
            items: [
                {
                    title: "Product Specification",
                    template: createSalesInvoiceProductSpecificationTabTemplate(masterDetailOptions)
                },
                {
                    title: "Attachments",
                    template: createAttachmentsTabTemplate(masterDetailOptions)
                },
                {
                    title: "Price Index",
                    template: createSalesInvoicePriceIndexTemplate(masterDetailOptions)
                },
                {
                    title: "COA & Trace Element",
                    template: createCoaTraceElementTabTemplate(masterDetailOptions)
                },
                {
                    title: "HTML Attachments",
                    template: renderHTMLAttachments(masterDetailOptions)
                },
                //{
                //    title: "Quality Sampling",
                //    template: createQualitySamplingTab(masterDetailOptions.data)
                //}
            ]
        }).appendTo(container);
    }

    const renderHTMLAttachments = function (masterDetailData) {
        return function () {
            let currentRecord = masterDetailData;
            let detailName = "SalesInvoiceHtmlContent";
            let urlDetail = "/api/" + areaName + "/" + detailName;

            return $("<div>")
                .dxDataGrid({
                    dataSource: DevExpress.data.AspNet.createStore({
                        key: "id",
                        loadUrl: urlDetail + "/BySalesInvoiceId/" + encodeURIComponent(masterDetailData.id),
                        insertUrl: urlDetail + "/InsertData",
                        updateUrl: urlDetail + "/UpdateData",
                        deleteUrl: urlDetail + "/DeleteData",
                        onBeforeSend: function (method, ajaxOptions) {
                            ajaxOptions.xhrFields = { withCredentials: true };
                            ajaxOptions.beforeSend = function (request) {
                                request.setRequestHeader("Authorization", "Bearer " + token);
                            };
                        }
                    }),
                    remoteOperations: true,
                    allowColumnResizing: true,
                    dateSerializationFormat: "yyyy-MM-ddTHH:mm:ss",
                    columns: [
                        //{
                        //    dataField: "content_type_id",
                        //    dataType: "string",
                        //    caption: "Content Type",
                        //    lookup: {
                        //        dataSource: function (options) {
                        //            return {
                        //                store: DevExpress.data.AspNet.createStore({
                        //                    key: "value",
                        //                    loadUrl: "/api/General/MasterList/MasterListIdLookup",
                        //                    onBeforeSend: function (method, ajaxOptions) {
                        //                        ajaxOptions.xhrFields = { withCredentials: true };
                        //                        ajaxOptions.beforeSend = function (request) {
                        //                            request.setRequestHeader("Authorization", "Bearer " + token);
                        //                        };
                        //                    }
                        //                }),
                        //                filter: ["item_group", "=", "content-type"]
                        //            }
                        //        },
                        //        valueExpr: "value",
                        //        displayExpr: "text"
                        //    },
                        //},
                        {
                            dataField: "html_content",
                            caption: "HTML Content",
                            formItem: {
                                colSpan: 2,
                                editorType: "dxHtmlEditor",
                                editorOptions: {
                                    height: 610,
                                    toolbar: {
                                        items: [
                                            "undo", "redo", "separator",
                                            {
                                                name: "size",
                                                acceptedValues: ["8pt", "10pt", "12pt", "14pt", "18pt", "24pt", "36pt"]
                                            },
                                            {
                                                name: "font",
                                                acceptedValues: ["Arial", "Georgia", "Tahoma", "Times New Roman", "Verdana"]
                                            },
                                            "separator", "bold", "italic", "strike", "underline", "separator", "alignLeft", "alignCenter", "alignRight",
                                            "alignJustify", "separator", "orderedList", "bulletList", "separator",
                                            {
                                                name: "header",
                                                acceptedValues: [false, 1, 2, 3, 4, 5]
                                            },
                                            "separator", "color", "background", "separator", "link", "image", "separator", "clear", "codeBlock", "blockquote", "separator",
                                            "insertTable", "deleteTable", "insertRowAbove", "insertRowBelow", "deleteRow",
                                            "insertColumnLeft", "insertColumnRight", "deleteColumn"
                                        ],
                                        multiline: true
                                    }
                                }
                            },
                        },
                    ],
                    filterRow: {
                        visible: true
                    },
                    headerFilter: {
                        visible: true
                    },
                    groupPanel: {
                        visible: true
                    },
                    searchPanel: {
                        visible: true,
                        width: 240,
                        placeholder: "Search..."
                    },
                    filterPanel: {
                        visible: true
                    },
                    filterBuilderPopup: {
                        position: { of: window, at: "top", my: "top", offset: { y: 10 } },
                    },
                    columnChooser: {
                        enabled: true,
                        mode: "select"
                    },
                    paging: {
                        pageSize: 10
                    },
                    pager: {
                        allowedPageSizes: [10, 20, 50, 100],
                        showNavigationButtons: true,
                        showPageSizeSelector: true,
                        showInfo: true,
                        visible: true
                    },
                    showBorders: true,
                    editing: {
                        mode: "popup",
                        allowAdding: true,
                        allowUpdating: true,
                        allowDeleting: true,
                        useIcons: true
                    },
                    grouping: {
                        contextMenuEnabled: true,
                        autoExpandAll: false
                    },
                    rowAlternationEnabled: true,
                    export: {
                        enabled: true,
                        allowExportSelectedData: true
                    },
                    onInitNewRow: function (e) {
                        e.data.sales_invoice_id = currentRecord.id;
                    },
                    onExporting: function (e) {
                        var workbook = new ExcelJS.Workbook();
                        var worksheet = workbook.addWorksheet(entityName);

                        DevExpress.excelExporter.exportDataGrid({
                            component: e.component,
                            worksheet: worksheet,
                            autoFilterEnabled: true
                        }).then(function () {
                            // https://github.com/exceljs/exceljs#writing-xlsx
                            workbook.xlsx.writeBuffer().then(function (buffer) {
                                saveAs(new Blob([buffer], { type: 'application/octet-stream' }), detailName + '.xlsx');
                            });
                        });
                        e.cancel = true;
                    }
                });
        }
    };


    function createSalesInvoiceProductSpecificationTabTemplate(masterDetailData) {
        return function () {
            let currentRecord = masterDetailData;
            var urlDetail = "/api/Sales/SalesInvoiceDetail";

            documentDataGrid = $("<div>")
                .dxDataGrid({
                    dataSource: DevExpress.data.AspNet.createStore({
                        key: "id",
                        loadUrl: urlDetail + "/PSDataGrid/" + encodeURIComponent(masterDetailData.id),
                        insertUrl: urlDetail + "/PSInsertData",
                        updateUrl: urlDetail + "/PSUpdateData",
                        deleteUrl: urlDetail + "/PSDeleteData",
                        onBeforeSend: function (method, ajaxOptions) {
                            ajaxOptions.xhrFields = { withCredentials: true };
                            ajaxOptions.beforeSend = function (request) {
                                request.setRequestHeader("Authorization", "Bearer " + token);
                            };
                        }
                    }),
                    remoteOperations: true,
                    allowColumnResizing: true,
                    dateSerializationFormat: "yyyy-MM-ddTHH:mm:ss",
                    columns: [
                        {
                            dataField: "analyte_id",
                            dataType: "string",
                            caption: "Analyte Definition",
                            allowEditing: false,
                            formItem: {
                                visible: false
                            },
                            lookup: {
                                dataSource: DevExpress.data.AspNet.createStore({
                                    key: "value",
                                    loadUrl: "/api/Quality/Analyte/AnalyteIdLookup",
                                    onBeforeSend: function (method, ajaxOptions) {
                                        ajaxOptions.xhrFields = { withCredentials: true };
                                        ajaxOptions.beforeSend = function (request) {
                                            request.setRequestHeader("Authorization", "Bearer " + token);
                                        };
                                    }
                                }),
                                searchEnabled: true,
                                valueExpr: "value",
                                displayExpr: "text"
                            },
                        },
                        {
                            dataField: "nilai_penyesuaian",
                            dataType: "string",
                            caption: "Nilai Penyesuaian",
                            formItem: {
                                editorOptions: {
                                    showClearButton: true
                                }
                            },
                            allowEditing: true
                        },
                        {
                            dataField: "nilai_denda_penolakan",
                            dataType: "number",
                            caption: "Nilai Denda Penolakan",
                            formItem: {
                                editorOptions: {
                                    showClearButton: true
                                }
                            },
                            allowEditing: true
                        },
                        {
                            dataField: "uom_id",
                            dataType: "text",
                            caption: "Unit",
                            formItem: {
                                visible: false
                            },
                            validationRules: [{
                                type: "required",
                                message: "The field is required."
                            }],
                            lookup: {
                                dataSource: DevExpress.data.AspNet.createStore({
                                    key: "value",
                                    loadUrl: "/api/UOM/UOM/UomIdLookup",
                                    onBeforeSend: function (method, ajaxOptions) {
                                        ajaxOptions.xhrFields = { withCredentials: true };
                                        ajaxOptions.beforeSend = function (request) {
                                            request.setRequestHeader("Authorization", "Bearer " + token);
                                        };
                                    }
                                }),
                                valueExpr: "value",
                                displayExpr: "text"
                            }
                        },
                        {
                            dataField: "analyte_value",
                            dataType: "number",
                            caption: "Actual",
                            allowEditing: false,
                            formItem: {
                                visible: false
                            }
                        },
                        {
                            dataField: "target",
                            dataType: "number",
                            caption: "Guaranteed",
                            allowEditing: false,
                            formItem: {
                                visible: false
                            }
                        },
                        {
                            dataField: "coa_display",
                            dataType: "boolean",
                            caption: "COA",
                            allowEditing: true

                        },
                        {
                            dataField: "trace_elements",
                            dataType: "boolean",
                            caption: "Trace Elements",
                            allowEditing: true

                        }
                    ],
                    filterRow: {
                        visible: true
                    },
                    headerFilter: {
                        visible: true
                    },
                    groupPanel: {
                        visible: true
                    },
                    searchPanel: {
                        visible: true,
                        width: 240,
                        placeholder: "Search..."
                    },
                    filterPanel: {
                        visible: true
                    },
                    filterBuilderPopup: {
                        position: { of: window, at: "top", my: "top", offset: { y: 10 } },
                    },
                    columnChooser: {
                        enabled: true,
                        mode: "select"
                    },
                    paging: {
                        pageSize: 10
                    },
                    pager: {
                        allowedPageSizes: [10, 20, 50, 100],
                        showNavigationButtons: true,
                        showPageSizeSelector: true,
                        showInfo: true,
                        visible: true
                    },
                    showBorders: true,
                    editing: {
                        mode: "form",
                        allowAdding: false,
                        allowUpdating: true,
                        allowDeleting: false,
                        useIcons: true,
                        form: {
                            colCount: 2,
                            items: [
                                {
                                    dataField: "analyte_id"
                                },
                                {
                                    dataField: "nilai_penyesuaian"
                                },
                                {
                                    dataField: "nilai_denda_penolakan"
                                },
                                {
                                    dataField: "uom_id"
                                },
                                {
                                    dataField: "analyte_value"
                                },
                                {
                                    dataField: "target"
                                },
                                {
                                    dataField: "coa_display"
                                },
                                {
                                    dataField: "trace_elements"
                                },
                            ]
                        }
                    },
                    grouping: {
                        contextMenuEnabled: true,
                        autoExpandAll: false
                    },
                    rowAlternationEnabled: true,
                    export: {
                        enabled: true,
                        allowExportSelectedData: true
                    },
                    onInitNewRow: function (e) {
                        e.data.sales_invoice_id = currentRecord.id;
                    },
                    onRowUpdating: function (e) {
                        let oldRecord = e.oldData;
                        e.newData.sales_invoice_id = oldRecord.sales_invoice_id;
                        e.newData.analyte_id = oldRecord.analyte_id;
                        e.newData.despatch_order_id = oldRecord.despatch_order_id;
                        e.newData.sampling_analyte_id = oldRecord.sampling_analyte_id;
                    },
                    onExporting: function (e) {
                        var workbook = new ExcelJS.Workbook();
                        var worksheet = workbook.addWorksheet(entityName);

                        DevExpress.excelExporter.exportDataGrid({
                            component: e.component,
                            worksheet: worksheet,
                            autoFilterEnabled: true
                        }).then(function () {
                            // https://github.com/exceljs/exceljs#writing-xlsx
                            workbook.xlsx.writeBuffer().then(function (buffer) {
                                saveAs(new Blob([buffer], { type: 'application/octet-stream' }), detailName + '.xlsx');
                            });
                        });
                        e.cancel = true;
                    }
                });

            return documentDataGrid
        }
    }

    function createSalesInvoicePriceIndexTemplate(masterDetailData) {
        return function () {
            let currentRecord = masterDetailData;
            let urlDetail = "/api/Sales/SalesInvoicePriceIndex";

            documentDataGrid = $("<div>")
                .dxDataGrid({
                    dataSource: DevExpress.data.AspNet.createStore({
                        key: "id",
                        //loadUrl: urlDetail + "/BySalesInvoiceId/" + encodeURIComponent(currentRecord.id),
                        loadUrl: urlDetail + '/PriceIndexHistoryLookup?despatch_order_id=' + currentRecord.despatch_order_id + '&sales_contract_id=' + currentRecord.sales_contract_id
                            + '&sales_contract_term_id=' + currentRecord.sales_contract_term_id + '&invoice_type_id=' + currentRecord.invoice_type_id
                            + '&quotation_type_id=' + currentRecord.quotation_type_id,

                        insertUrl: urlDetail + "/InsertData",
                        updateUrl: urlDetail + "/UpdateData",
                        deleteUrl: urlDetail + "/DeleteData",
                        onBeforeSend: function (method, ajaxOptions) {
                            ajaxOptions.xhrFields = { withCredentials: true };
                            ajaxOptions.beforeSend = function (request) {
                                request.setRequestHeader("Authorization", "Bearer " + token);
                            };
                        }
                    }),
                    remoteOperations: true,
                    allowColumnResizing: true,
                    dateSerializationFormat: "yyyy-MM-ddTHH:mm:ss",
                    columns: [
                        {
                            dataField: "sales_invoice_id",
                            caption: "Sales Invoice",
                            allowEditing: false,
                            visible: false,
                            calculateCellValue: function () {
                                return currentRecord.id;
                            }
                        },
                        {
                            dataField: "index_date",
                            dataType: "date",
                            caption: "Date",
                            validationRules: [{
                                type: "required",
                                message: "This field is required."
                            }],
                            sortOrder: "asc"
                        },
                        {
                            dataField: "index_value",
                            dataType: "number",
                            caption: "Index Value",
                            formItem: {
                                editorType: "dxNumberBox",
                                editorOptions: {
                                    step: 0,
                                    format: {
                                        type: "fixedPoint",
                                        precision: 3
                                    }
                                }
                            },
                            validationRules: [{
                                type: "required",
                                message: "This field is required."
                            }]
                        },
                        {
                            dataField: "note",
                            dataType: "string",
                            caption: "Note"
                        },
                    ],
                    filterRow: {
                        visible: true
                    },
                    headerFilter: {
                        visible: true
                    },
                    groupPanel: {
                        visible: true
                    },
                    searchPanel: {
                        visible: true,
                        width: 240,
                        placeholder: "Search..."
                    },
                    filterPanel: {
                        visible: true
                    },
                    filterBuilderPopup: {
                        position: { of: window, at: "top", my: "top", offset: { y: 10 } },
                    },
                    columnChooser: {
                        enabled: true,
                        mode: "select"
                    },
                    paging: {
                        pageSize: 10
                    },
                    pager: {
                        allowedPageSizes: [10, 20, 50, 100],
                        showNavigationButtons: true,
                        showPageSizeSelector: true,
                        showInfo: true,
                        visible: true
                    },
                    showBorders: true,
                    editing: {
                        mode: "form",
                        allowAdding: false,
                        allowUpdating: false,
                        allowDeleting: false,
                        useIcons: true,
                        form: {
                            colCount: 2,
                            items: [
                                {
                                    dataField: "analyte_id"
                                },
                                {
                                    dataField: "analyte_standard_id"
                                },
                                {
                                    dataField: "value"
                                },
                                {
                                    dataField: "target"
                                },
                            ]
                        }
                    },
                    grouping: {
                        contextMenuEnabled: true,
                        autoExpandAll: false
                    },
                    rowAlternationEnabled: true,
                    export: {
                        enabled: true,
                        allowExportSelectedData: true
                    },
                    onInitNewRow: function (e) {
                        e.data.sales_invoice_id = currentRecord.id;
                    },
                    onExporting: function (e) {
                        var workbook = new ExcelJS.Workbook();
                        var worksheet = workbook.addWorksheet(entityName);

                        DevExpress.excelExporter.exportDataGrid({
                            component: e.component,
                            worksheet: worksheet,
                            autoFilterEnabled: true
                        }).then(function () {
                            // https://github.com/exceljs/exceljs#writing-xlsx
                            workbook.xlsx.writeBuffer().then(function (buffer) {
                                saveAs(new Blob([buffer], { type: 'application/octet-stream' }), detailName + '.xlsx');
                            });
                        });
                        e.cancel = true;
                    }
                });

            return documentDataGrid
        }
    }

    var salesInvoiceData
    function createAttachmentsTabTemplate(masterDetailData) {
        return function () {
            let currentRecord = masterDetailData;
            let detailName = "SalesInvoiceAttachment";
            let urlDetail = "/api/" + areaName + "/" + detailName;
            salesInvoiceData = currentRecord

            documentDataGrid = $("<div>")
                .dxDataGrid({
                    dataSource: DevExpress.data.AspNet.createStore({
                        key: "id",
                        loadUrl: urlDetail + "/DataGridAttachments?salesInvoiceId=" + encodeURIComponent(currentRecord.id),
                        //insertUrl: urlDetail + "/InsertData",
                        deleteUrl: urlDetail + "/DeleteData",
                        onBeforeSend: function (method, ajaxOptions) {
                            ajaxOptions.xhrFields = { withCredentials: true };
                            ajaxOptions.beforeSend = function (request) {
                                request.setRequestHeader("Authorization", "Bearer " + token);
                            };
                        }
                    }),
                    remoteOperations: true,
                    allowColumnResizing: true,
                    dateSerializationFormat: "yyyy-MM-ddTHH:mm:ss",
                    columns: [
                        {
                            dataField: "filename",
                            dataType: "string",
                            caption: "Document"
                        },
                        {
                            caption: "Download",
                            type: "buttons",
                            width: 100,
                            buttons: [{
                                cssClass: "btn-dxdatagrid",
                                hint: "Download attachment",
                                text: "Download",
                                onClick: function (e) {
                                    // Download file from Ajax. Ref: https://stackoverflow.com/a/9970672
                                    let documentData = e.row.data
                                    let documentName = /[^\\]*$/.exec(documentData.filename)[0]

                                    let xhr = new XMLHttpRequest()
                                    xhr.open("GET", "/api/Sales/SalesInvoiceAttachment/DownloadDocument/" + documentData.id, true)
                                    xhr.responseType = "blob"
                                    xhr.setRequestHeader("Authorization", "Bearer " + token)

                                    xhr.onload = function (e) {
                                        let blobURL = window.webkitURL.createObjectURL(xhr.response)

                                        let a = document.createElement("a")
                                        a.href = blobURL
                                        a.download = documentName
                                        document.body.appendChild(a)
                                        a.click()
                                    };

                                    xhr.send()
                                }
                            }]
                        },
                        {
                            type: "buttons",
                            buttons: ["edit", "delete"]
                        }
                    ],
                    onToolbarPreparing: function (e) {
                        let toolbarItems = e.toolbarOptions.items;

                        // Modifies an existing item
                        toolbarItems.forEach(function (item) {
                            if (item.name === "addRowButton") {
                                item.options = {
                                    icon: "plus",
                                    onClick: function (e) {
                                        openDocumentPopup()
                                    }
                                }
                            }

                            if (item.name === "editRowButton") {
                                item.options = {
                                    icon: "edit",
                                    onClick: function (e) {
                                        openDocumentPopup()
                                    }
                                }
                            }
                        });
                    },
                    filterRow: {
                        visible: true
                    },
                    headerFilter: {
                        visible: true
                    },
                    groupPanel: {
                        visible: true
                    },
                    searchPanel: {
                        visible: true,
                        width: 240,
                        placeholder: "Search..."
                    },
                    filterPanel: {
                        visible: true
                    },
                    filterBuilderPopup: {
                        position: { of: window, at: "top", my: "top", offset: { y: 10 } },
                    },
                    columnChooser: {
                        enabled: true,
                        mode: "select"
                    },
                    paging: {
                        pageSize: 10
                    },
                    pager: {
                        allowedPageSizes: [10, 20, 50, 100],
                        showNavigationButtons: true,
                        showPageSizeSelector: true,
                        showInfo: true,
                        visible: true
                    },
                    showBorders: true,
                    editing: {
                        mode: "form",
                        allowAdding: true,
                        allowUpdating: false,
                        allowDeleting: true,
                        useIcons: true
                    },
                    grouping: {
                        contextMenuEnabled: true,
                        autoExpandAll: false
                    },
                    rowAlternationEnabled: true,
                    export: {
                        enabled: true,
                        allowExportSelectedData: true
                    },
                    onInitNewRow: function (e) {
                        e.data.shipping_transaction_id = currentRecord.id;
                    },
                    onExporting: function (e) {
                        var workbook = new ExcelJS.Workbook();
                        var worksheet = workbook.addWorksheet(entityName);

                        DevExpress.excelExporter.exportDataGrid({
                            component: e.component,
                            worksheet: worksheet,
                            autoFilterEnabled: true
                        }).then(function () {
                            // https://github.com/exceljs/exceljs#writing-xlsx
                            workbook.xlsx.writeBuffer().then(function (buffer) {
                                saveAs(new Blob([buffer], { type: 'application/octet-stream' }), detailName + '.xlsx');
                            });
                        });
                        e.cancel = true;
                    }
                });

            return documentDataGrid
        }
    }

    const documentPopupOptions = {
        width: "80%",
        height: "auto",
        showTitle: true,
        title: "Add Attachment",
        visible: false,
        dragEnabled: false,
        hideOnOutsideClick: true,
        contentTemplate: function (e) {
            let formContainer = $("<div>")
            formContainer.dxForm({
                formData: {
                    id: "",
                    sales_invoice_id: salesInvoiceData.id,
                    //quantity: false,
                    //quality: false,
                    file: ""
                },
                colCount: 2,
                items: [
                    {
                        dataField: "sales_invoice_id",
                        label: {
                            text: "Sales Invoice Id"
                        },
                        validationRules: [{
                            type: "required"
                        }],
                        visible: false
                    },
                    {
                        dataField: "file",
                        name: "file",
                        label: {
                            text: "File"
                        },
                        template: function (data, itemElement) {
                            itemElement.append($("<div>").attr("id", "file").dxFileUploader({
                                uploadMode: "useForm",
                                multiple: false,
                                maxFileSize: maxFileSize,
                                invalidMaxFileSizeMessage: "Max. file size is 50 Mb",
                                onValueChanged: function (e) {
                                    data.component.updateData(data.dataField, e.value)
                                }
                            }));
                        },
                        validationRules: [{
                            type: "required"
                        }],
                        colSpan: 2
                    },
                    {
                        itemType: "button",
                        colSpan: 2,
                        horizontalAlignment: "right",
                        buttonOptions: {
                            text: "Save",
                            type: "secondary",
                            useSubmitBehavior: true,
                            onClick: function () {
                                let formData = formContainer.dxForm("instance").option('formData')
                                let file = formData.file[0]

                                var reader = new FileReader();
                                reader.readAsDataURL(file);
                                reader.onload = function () {
                                    let fileName = file.name
                                    let fileSize = file.size
                                    let data = reader.result.split(',')[1]

                                    if (fileSize == 0) {
                                        alert("File content is empty.")
                                        return;
                                    }
                                    if (fileSize >= maxFileSize) {
                                        return;
                                    }

                                    let newFormData = {
                                        "salesInvoiceId": formData.sales_invoice_id,
                                        "fileName": fileName,
                                        "fileSize": fileSize,
                                        "data": data
                                    }

                                    /*//console.log(newFormData)*/

                                    $.ajax({
                                        //url: `/api/Sales/SalesInvoice/InsertAttachmentData`,
                                        url: `/api/Sales/SalesInvoiceAttachment/InsertAttachment`,
                                        data: JSON.stringify(newFormData),
                                        type: "POST",
                                        contentType: "application/json",
                                        beforeSend: function (xhr) {
                                            xhr.setRequestHeader("Authorization", "Bearer " + token);
                                        },
                                        success: function (response) {
                                            documentPopup.hide()
                                            documentDataGrid.dxDataGrid("instance").refresh()
                                        }
                                    })
                                }
                            }
                        }
                    }
                ]
            })
            e.append(formContainer)
        }
    }

    const documentPopup = $("<div>")
        .dxPopup(documentPopupOptions).appendTo("body").dxPopup("instance")

    const openDocumentPopup = function () {
        documentPopup.option("contentTemplate", documentPopupOptions.contentTemplate.bind(this));
        documentPopup.show()
    }

    //ilham
    var salesInvoiceData
    function createCoaTraceElementTabTemplate(masterDetailData) {
        return function () {
            let currentRecord = masterDetailData;
            //let detailName = "SalesInvoice";
            let detailName = "SalesInvoiceTraceElement";
            let urlDetail = "/api/" + areaName + "/" + detailName;
            salesInvoiceData = currentRecord

            documentDataGrid = $("<div>")
                .dxDataGrid({
                    dataSource: DevExpress.data.AspNet.createStore({
                        key: "id",
                        loadUrl: urlDetail + "/DataGridAttachments?salesInvoiceId=" + encodeURIComponent(currentRecord.id),
                        //insertUrl: urlDetail + "/InsertData",
                        deleteUrl: urlDetail + "/DeleteData",
                        onBeforeSend: function (method, ajaxOptions) {
                            ajaxOptions.xhrFields = { withCredentials: true };
                            ajaxOptions.beforeSend = function (request) {
                                request.setRequestHeader("Authorization", "Bearer " + token);
                            };
                        }
                    }),
                    remoteOperations: true,
                    allowColumnResizing: true,
                    dateSerializationFormat: "yyyy-MM-ddTHH:mm:ss",
                    columns: [
                        {
                            dataField: "filename",
                            dataType: "string",
                            caption: "Document"
                        },
                        {
                            caption: "Download",
                            type: "buttons",
                            width: 100,
                            buttons: [{
                                cssClass: "btn-dxdatagrid",
                                hint: "Download attachment",
                                text: "Download",
                                onClick: function (e) {
                                    // Download file from Ajax. Ref: https://stackoverflow.com/a/9970672
                                    let documentData = e.row.data
                                    let documentName = /[^\\]*$/.exec(documentData.filename)[0]

                                    let xhr = new XMLHttpRequest()
                                    xhr.open("GET", "/api/Sales/SalesInvoiceTraceElement/DownloadDocument/" + documentData.id, true)
                                    xhr.responseType = "blob"
                                    xhr.setRequestHeader("Authorization", "Bearer " + token)

                                    xhr.onload = function (e) {
                                        let blobURL = window.webkitURL.createObjectURL(xhr.response)

                                        let a = document.createElement("a")
                                        a.href = blobURL
                                        a.download = documentName
                                        document.body.appendChild(a)
                                        a.click()
                                    };

                                    xhr.send()
                                }
                            }]
                        },
                        {
                            type: "buttons",
                            buttons: ["edit", "delete"]
                        }
                    ],
                    onToolbarPreparing: function (e) {
                        let toolbarItems = e.toolbarOptions.items;

                        // Modifies an existing item
                        toolbarItems.forEach(function (item) {
                            if (item.name === "addRowButton") {
                                item.options = {
                                    icon: "plus",
                                    onClick: function (e) {
                                        openDocumentPopupDetail()
                                    }
                                }
                            }

                            if (item.name === "editRowButton") {
                                item.options = {
                                    icon: "edit",
                                    onClick: function (e) {
                                        openDocumentPopupDetail()
                                    }
                                }
                            }
                        });
                    },
                    filterRow: {
                        visible: true
                    },
                    headerFilter: {
                        visible: true
                    },
                    groupPanel: {
                        visible: true
                    },
                    searchPanel: {
                        visible: true,
                        width: 240,
                        placeholder: "Search..."
                    },
                    filterPanel: {
                        visible: true
                    },
                    filterBuilderPopup: {
                        position: { of: window, at: "top", my: "top", offset: { y: 10 } },
                    },
                    columnChooser: {
                        enabled: true,
                        mode: "select"
                    },
                    paging: {
                        pageSize: 10
                    },
                    pager: {
                        allowedPageSizes: [10, 20, 50, 100],
                        showNavigationButtons: true,
                        showPageSizeSelector: true,
                        showInfo: true,
                        visible: true
                    },
                    showBorders: true,
                    editing: {
                        mode: "form",
                        allowAdding: true,
                        allowUpdating: false,
                        allowDeleting: true,
                        useIcons: true
                    },
                    grouping: {
                        contextMenuEnabled: true,
                        autoExpandAll: false
                    },
                    rowAlternationEnabled: true,
                    export: {
                        enabled: true,
                        allowExportSelectedData: true
                    },
                    onInitNewRow: function (e) {
                        e.data.shipping_transaction_id = currentRecord.id;
                    },
                    onExporting: function (e) {
                        var workbook = new ExcelJS.Workbook();
                        var worksheet = workbook.addWorksheet(entityName);

                        DevExpress.excelExporter.exportDataGrid({
                            component: e.component,
                            worksheet: worksheet,
                            autoFilterEnabled: true
                        }).then(function () {
                            // https://github.com/exceljs/exceljs#writing-xlsx
                            workbook.xlsx.writeBuffer().then(function (buffer) {
                                saveAs(new Blob([buffer], { type: 'application/octet-stream' }), detailName + '.xlsx');
                            });
                        });
                        e.cancel = true;
                    }
                });

            return documentDataGrid
        }
    }

    const documentPopupSet = {
        width: "80%",
        height: "auto",
        showTitle: true,
        title: "Add COA & Trace Element",
        visible: false,
        dragEnabled: false,
        hideOnOutsideClick: true,
        contentTemplate: function (e) {
            let formContainer = $("<div>")
            formContainer.dxForm({
                formData: {
                    id: "",
                    sales_invoice_id: salesInvoiceData.id,
                    //quantity: false,
                    //quality: false,
                    file: ""
                },
                colCount: 2,
                items: [
                    {
                        dataField: "sales_invoice_id",
                        label: {
                            text: "Sales Invoice Id"
                        },
                        validationRules: [{
                            type: "required"
                        }],
                        visible: false
                    },
                    {
                        dataField: "file",
                        name: "file",
                        label: {
                            text: "File"
                        },
                        template: function (data, itemElement) {
                            itemElement.append($("<div>").attr("id", "file").dxFileUploader({
                                uploadMode: "useForm",
                                multiple: false,
                                maxFileSize: maxFileSize,
                                invalidMaxFileSizeMessage: "Max. file size is 50 Mb",
                                onValueChanged: function (e) {
                                    data.component.updateData(data.dataField, e.value)
                                }
                            }));
                        },
                        validationRules: [{
                            type: "required"
                        }],
                        colSpan: 2
                    },
                    {
                        itemType: "button",
                        colSpan: 2,
                        horizontalAlignment: "right",
                        buttonOptions: {
                            text: "Save",
                            type: "secondary",
                            useSubmitBehavior: true,
                            onClick: function () {
                                let formData = formContainer.dxForm("instance").option('formData')
                                let file = formData.file[0]

                                var reader = new FileReader();
                                reader.readAsDataURL(file);
                                reader.onload = function () {
                                    let fileName = file.name
                                    let fileSize = file.size
                                    let data = reader.result.split(',')[1]

                                    if (fileSize == 0) {
                                        alert("File content is empty.")
                                        return;
                                    }
                                    if (fileSize >= maxFileSize) {
                                        return;
                                    }

                                    let newFormData = {
                                        "salesInvoiceId": formData.sales_invoice_id,
                                        "fileName": fileName,
                                        "fileSize": fileSize,
                                        "data": data
                                    }

                                    /*//console.log(newFormData)*/

                                    $.ajax({
                                        //url: `/api/Sales/SalesInvoice/InsertAttachmentData`,
                                        url: `/api/Sales/SalesInvoiceTraceElement/InsertAttachment`,
                                        data: JSON.stringify(newFormData),
                                        type: "POST",
                                        contentType: "application/json",
                                        beforeSend: function (xhr) {
                                            xhr.setRequestHeader("Authorization", "Bearer " + token);
                                        },
                                        success: function (response) {
                                            documentPopupDetail.hide()
                                            documentDataGrid.dxDataGrid("instance").refresh()
                                        }
                                    })
                                }
                            }
                        }
                    }
                ]
            })
            e.append(formContainer)
        }
    }

    const documentPopupDetail = $("<div>")
        .dxPopup(documentPopupSet).appendTo("body").dxPopup("instance")

    const openDocumentPopupDetail = function () {
        documentPopupDetail.option("contentTemplate", documentPopupSet.contentTemplate.bind(this));
        documentPopupDetail.show()
    }

    /**
         * Adjustments
         * Set, Get, and Delete
         */

    // set Adjustments after getting despatchOrderId, invoiceTypeId and quotationTypeId    const setAdjustmentAndPrice = async (grid, rowData, index, despatchOrderId, salesContractId, salesContractTermId, invoiceTypeId, quotationTypeId) => {
    const setAdjustmentAndPrice = async (grid, rowData, index, despatchOrderId, salesContractId, salesContractTermId, invoiceTypeId, quotationTypeId) => {
        grid.beginCustomLoading()
        await deleteAdjustmentAndPrice(grid, index)

        $.ajax({
            url: '/api/Sales/SalesInvoice/CountPrice?despatch_order_id=' + despatchOrderId + '&sales_contract_id=' + salesContractId +
                '&sales_contract_term_id=' + salesContractTermId + '&invoice_type_id=' + invoiceTypeId +
                '&quotation_type_id=' + quotationTypeId + '&isEditing=' + isEditing,
            type: 'GET',
            contentType: "application/json",
            beforeSend: function (xhr) {
                xhr.setRequestHeader("Authorization", "Bearer " + token);
            },
            success: function (response) {
                grid.beginUpdate()
                grid.beginCustomLoading()

                let dynamicAdjustments = response
                let i = 0

                let totalAdjustmentPrice = 0;

                dynamicAdjustments.forEach((adjustment) => {
                    if (adjustment.chargeCode == "quotation_price")
                        return;

                    if (adjustment.chargeCode == "down_payment")
                        return;

                    if (adjustment.chargeCode == "subtotal")
                        return;

                    if (adjustment.chargeCode == "total_price")
                        return;

                    if (adjustment.chargeCode == "credit_limit")
                        return;

                    // Add dynamic adjustment data to grid's column
                    grid.addColumn({
                        dataField: adjustment.chargeCode,
                        dataType: "number",
                        caption: adjustment.chargeName,
                        formItem: {
                            editorType: "dxNumberBox",
                            editorOptions: {
                                format: "#,##0.0000",
                                step: 0
                            },
                            colSpan: 2
                        },
                        allowEditing: true,
                        visible: false,
                    })

                    // Set value
                    // Important! set value with this method
                    // So it will be passed when data saving
                    grid.cellValue(index, adjustment.chargeCode, adjustment.price)
                    totalAdjustmentPrice += adjustment.price;
                    // Add dynamic adjustment data to grid's edit form
                    let newFormItemData = {
                        dataField: adjustment.chargeCode,
                        colSpan: 2
                    }

                    let adjustmentGroupFormItems = grid.option('editing.form.items[2].items')
                    let indexPositionTarget = 2 + i

                    adjustmentGroupFormItems.splice(indexPositionTarget, 0, newFormItemData)

                    i++
                })


                grid.dynamicAdjustments = dynamicAdjustments
                let quotationPrice1 = dynamicAdjustments.find((adjustment) => adjustment.chargeCode === "quotation_price")
                let hasil = quotationPrice1.price + totalAdjustmentPrice;
                grid.cellValue(index, "fob_price", hasil)
                //console.log("Total Adjustment Price:", totalAdjustmentPrice);
                //console.log("Difference:", hasil);
                let quotationPrice = dynamicAdjustments.find((adjustment) => adjustment.chargeCode === "quotation_price")
                grid.cellValue(index, "unit_price", quotationPrice.price)
                let downpaymentPrice = dynamicAdjustments.find((adjustment) => adjustment.chargeCode === "down_payment")
                grid.cellValue(index, "downpayment", downpaymentPrice.price)
                let subtotal = dynamicAdjustments.find((adjustment) => adjustment.chargeCode === "subtotal")
                grid.cellValue(index, "subtotal", subtotal.price)
                let totalPrice = dynamicAdjustments.find((adjustment) => adjustment.chargeCode === "total_price")
                grid.cellValue(index, "total_price", totalPrice.price)
                let creditLimit = dynamicAdjustments.find((adjustment) => adjustment.chargeCode === "credit_limit")
                grid.cellValue(index, "credit_limit", creditLimit.price)

                setTimeout(() => {
                    grid.endCustomLoading()
                }, 500)

                grid.endUpdate()
            },
            error: function (xhr, status, error) {
                // Close edit state if All invoice had already issued
                // if (xhr.responseText.includes('All invoice had already issued') || xhr.responseText.includes('Price is not available') || xhr.responseText.includes('required input parameter is null') || xhr.responseText.includes('no payment term is listed') || xhr.responseText.includes('no defined customer')) {
                if (xhr.responseText.length > 0) {
                    alert(xhr.responseText)
                    grid.cancelEditData()

                    /*let columns = grid.option("columns")
                    columns.forEach(column => {
                        grid.cellValue(index, column.dataField, null)
                    })*/
                }

                grid.endCustomLoading()
            }

        })

        grid.endCustomLoading()

    }

    // get saved sales invoice adjustments
    const getAdjustmentAndPrice = (grid, rowData, index, salesInvoiceId) => {

        return new Promise(async (resolve) => {

            grid.beginCustomLoading()

            await deleteAdjustmentAndPrice(grid)

            $.ajax({
                url: '/api/Sales/SalesInvoice/GetSalesInvoiceAdjusmentBySalesInvoiceId?salesInvoiceId=' + salesInvoiceId,
                type: 'GET',
                contentType: "application/json",
                beforeSend: function (xhr) {
                    xhr.setRequestHeader("Authorization", "Bearer " + token);
                },
                success: function (response) {
                    grid.beginUpdate()

                    let dynamicAdjustments = response.data
                    let i = 0

                    let newDynamicAdjustments = []

                    dynamicAdjustments.forEach((adjustment) => {
                        if (adjustment.charge_code == "quotation_price")
                            return;

                        if (adjustment.chargeCode == "down_payment")
                            return;

                        if (adjustment.chargeCode == "total_price")
                            return;

                        if (adjustment.chargeCode == "credit_limit")
                            return;

                        // Add dynamic adjustment data to grid's column
                        grid.addColumn({
                            dataField: adjustment.sales_charge_code,
                            dataType: "number",
                            caption: adjustment.sales_charge_name,
                            formItem: {
                                editorType: "dxNumberBox",
                                editorOptions: {
                                    format: "#,##0.0000",
                                    step: 0
                                },
                                colSpan: 2
                            },
                            allowEditing: true,
                            visible: false,
                        })

                        // Add dynamic adjustment data to grid's edit form
                        let newFormItemData = {
                            dataField: adjustment.sales_charge_code,
                            colSpan: 2
                        }

                        let adjustmentGroupFormItems = grid.option('editing.form.items[2].items')
                        let indexPositionTarget = 2 + i

                        adjustmentGroupFormItems.splice(indexPositionTarget, 0, newFormItemData)

                        // Set value
                        // Important! set value with this method
                        // So it will be passed when data saving
                        grid.cellValue(index, adjustment.sales_charge_code, adjustment.price)

                        // Create formatted adjusments to save in this grid instance
                        newDynamicAdjustments.push({
                            chargeCode: adjustment.sales_charge_code,
                            chargeName: adjustment.sales_charge_name,
                            price: adjustment.price
                        })

                        i++
                    })

                    grid.dynamicAdjustments = newDynamicAdjustments
                    grid.endUpdate()

                    setTimeout(() => {
                        grid.endCustomLoading()
                    }, 500)

                    resolve(true)

                }
            })
        })
    }

    // delete Adjustments
    const deleteAdjustmentAndPrice = (grid, index = null) => {
        return new Promise(async (resolve) => {
            // //console.log("adjustments: ", grid.dynamicAdjustments)
            // //console.log("index : ", index)
            // grid.beginCustomLoading()
            let dynamicAdjustments = grid.dynamicAdjustments

            // Remove old dynamic adjustment column & form
            // by looping using through saved old dataField keys,
            // except for quotation_price, down_payment, total_price, amd credit_limit
            if (dynamicAdjustments) {

                dynamicAdjustments.forEach((adjustment) => {
                    if (adjustment.chargeCode == "quotation_price")
                        return;

                    if (adjustment.chargeCode == "down_payment")
                        return;

                    if (adjustment.chargeCode == "total_price")
                        return;

                    if (adjustment.chargeCode == "credit_limit")
                        return;

                    // Remove edit form
                    grid.option('editing.form.items[2].items').splice(2, 1)

                    // Remove column
                    grid.deleteColumn(adjustment.chargeCode)
                })

                if (index != null) {
                    // Remove unit price/quotation pricing and downpayment
                    grid.cellValue(index, "unit_price", null)
                    //grid.cellValue(index, "downpayment", null)
                    grid.cellValue(index, "total_price", null)
                }
            }

            // //console.log('form items after deletion: ', grid.option('editing.form.items[2].items'))
            grid.dynamicAdjustments = null

            return resolve(true)

            // grid.endCustomLoading()
        })
    }

    /**
     * End Adjustments section
     */

    // Sales Invoice Information 
    const renderSalesInvoiceInformation = (currentRecord, container) => {
        let dateFormatted = currentRecord.invoice_date ? moment(currentRecord.invoice_date.split('T')[0]).format("D MMM YYYY") : '-'
        $(`
            <div>
                <div class="row align-items-center mb-3">
                    <div class="col-md-6">
                        <h5>Sales Invoice</h5>
                    </div>
                </div>
                
                <div class="row mb-5">
                    <div class="col-md-6">
                        <div class="master-detail-caption mb-2">Overview</div>
                        <div class="card card-mcs card-headline">
                            <div class="row">
                                <div class="col-md-6 pr-0">
                                    <div class="headline-title-container">
                                        <small class="font-weight-normal d-block mb-1">Invoice Number</small>
                                        <h4 class="headline-title font-weight-bold">${(currentRecord.invoice_number ? currentRecord.invoice_number : "-")}</h4>
                                    </div>
                                </div>
                                <div class="col-md-6 pl-0">
                                    <div class="headline-detail-container">
                                        <div class="row">
                                            <div class="col-md-12">
                                                <div class="d-flex align-items-start mb-3">
                                                    <div class="d-inline-block mr-3">
                                                        <div class="icon-circle">
                                                            <i class="fas fa-list fa-sm"></i>
                                                        </div>
                                                    </div>
                                                    <div class="d-inline-block">
                                                        <small class="font-weight-normal text-muted d-block mb-1">Sales Type</small>
                                                        <h5 class="font-weight-bold">${(currentRecord.sales_type_name ? currentRecord.sales_type_name : "-")}</h5>
                                                    </div>
                                                </div>
                                                <div class="d-flex align-items-start mb-3">
                                                    <div class="d-inline-block mr-3">
                                                        <div class="icon-circle">
                                                            <i class="fas fa-check-circle fa-sm"></i>
                                                        </div>
                                                    </div>
                                                    <div class="d-inline-block">
                                                        <small class="font-weight-normal text-muted d-block mb-1">Invoice Type</small>
                                                        <h5 class="font-weight-bold">${(currentRecord.invoice_type_name ? currentRecord.invoice_type_name : "-")}</h5>
                                                    </div>
                                                </div>
                                                <div class="d-flex align-items-start mb-3">
                                                    <div class="d-inline-block mr-3">
                                                        <div class="icon-circle">
                                                            <i class="fas fa-calendar-alt fa-sm"></i>
                                                        </div>
                                                    </div>
                                                    <div class="d-inline-block">
                                                        <small class="font-weight-normal text-muted d-block mb-1">Invoice Date</small>
                                                        <h5 class="font-weight-bold">${dateFormatted}</h5>
                                                    </div>
                                                </div>
                                                <div class="d-flex align-items-start">
                                                    <div class="d-inline-block mr-3">
                                                        <div class="icon-circle">
                                                            <i class="fas fa-file-alt fa-sm"></i>
                                                        </div>
                                                    </div>
                                                    <div class="d-inline-block">
                                                        <small class="font-weight-normal text-muted d-block mb-1">Shipping Order</small>
                                                        <h5 class="font-weight-bold">${(currentRecord.despatch_order_number ? currentRecord.despatch_order_number : "-")}</h5>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="col-md-6">
                        <div class="master-detail-caption mb-2">Invoice Information</div>
                        <div class="card card-mcs card-headline">
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="headline-detail-container">
                                        <div class="row mb-2">
                                            <div class="col-md-6">
                                                <small class="font-weight-normal text-muted d-block mb-1">Contract</small>
                                                <p class="font-weight-bold m-0">${(currentRecord.sales_contract_name ? currentRecord.sales_contract_name : "-")}</p>
                                            </div>
                                            <div class="col-md-6">
                                                <small class="font-weight-normal text-muted d-block mb-1">Contract Term</small>
                                                <p class="font-weight-bold m-0">${(currentRecord.sales_contract_term_name ? currentRecord.sales_contract_term_name : "-")}</p>
                                            </div>
                                        </div>
                                        <div class="row mb-2">
                                            <div class="col-md-6">
                                                <small class="font-weight-normal text-muted d-block mb-1">Contract Product</small>
                                                <p class="font-weight-bold m-0">${(currentRecord.contract_product_name ? currentRecord.contract_product_name : "-")}</p>
                                            </div>
                                        </div>
                                        <div class="row mb-2">
                                            <div class="col-md-6">
                                                <small class="font-weight-normal text-muted d-block mb-1">Quantity</small>
                                                <p class="font-weight-bold m-0">${(currentRecord.quantity ? formatNumber(currentRecord.quantity) : "-")}</p>
                                            </div>
                                            <div class="col-md-6">
                                                <small class="font-weight-normal text-muted d-block mb-1">Unit Price</small>
                                                <p class="font-weight-bold m-0">${(currentRecord.unit_price ? currentRecord.unit_price : "-")}</p>
                                            </div>
                                        </div>
                                        <div class="row mb-2">
                                            <div class="col-md-6">
                                                <small class="font-weight-normal text-muted d-block mb-1">Total Price</small>
                                                <p class="font-weight-bold m-0">${(currentRecord.total_price ? formatNumber(currentRecord.total_price) : "-")}</p>
                                            </div>
                                            <div class="col-md-6">
                                                <small class="font-weight-normal text-muted d-block mb-1">Unit</small>
                                                <p class="font-weight-bold m-0">${(currentRecord.uom_name ? currentRecord.uom_name : "-")}</p>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-6">
                                                <small class="font-weight-normal text-muted d-block mb-1">Notes</small>
                                                <p class="font-weight-bold m-0">${(currentRecord.notes ? currentRecord.notes : "-")}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>

                <div class="row mb-5">
                    <div class="col-md-6">
                        <div class="master-detail-caption mb-2">Billing</div>
                        <div class="card card-mcs card-headline">
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="headline-detail-container">
                                        <div class="row mb-2">
                                            <div class="col-md-6">
                                                <small class="font-weight-normal text-muted d-block mb-1">Seller</small>
                                                <p class="font-weight-bold m-0">${(currentRecord.seller_name ? currentRecord.seller_name : "-")}</p>
                                            </div>
                                            <div class="col-md-6">
                                                <small class="font-weight-normal text-muted d-block mb-1">Bank Account</small>
                                                <p class="font-weight-bold m-0">${(currentRecord.bank_account_id ? currentRecord.account_holder + " - " + currentRecord.account_number : "-")}</p>
                                            </div>
                                        </div>
                                        <div class="row mb-2">
                                            <div class="col-md-6">
                                                <small class="font-weight-normal text-muted d-block mb-1">Buyer</small>
                                                <p class="font-weight-bold m-0">${(currentRecord.customer_name ? currentRecord.customer_name : "-")}</p>
                                            </div>
                                            <div class="col-md-6">
                                                <small class="font-weight-normal text-muted d-block mb-1">Bill To</small>
                                                <p class="font-weight-bold m-0">${(currentRecord.bill_to_name ? currentRecord.bill_to_name : "-")}</p>
                                            </div>
                                        </div>
                                        <div class="row mb-2">
                                            <div class="col-md-6">
                                                <small class="font-weight-normal text-muted d-block mb-1">L/C Number</small>
                                                <p class="font-weight-bold m-0">${(currentRecord.lc_number ? currentRecord.lc_number : "-")}</p>
                                            </div>
                                            <div class="col-md-6">
                                                <small class="font-weight-normal text-muted d-block mb-1">L/C Specification</small>
                                                <p class="font-weight-bold m-0">${(currentRecord.lc_status ? currentRecord.lc_status : "-")}</p>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-6">
                                                <small class="font-weight-normal text-muted d-block mb-1">L/C Date Issue</small>
                                                <p class="font-weight-bold m-0">${(currentRecord.lc_date_issue ? currentRecord.lc_date_issue : "-")}</p>
                                            </div>
                                            <div class="col-md-6">
                                                <small class="font-weight-normal text-muted d-block mb-1">L/C Issuing Bank</small>
                                                <p class="font-weight-bold m-0">${(currentRecord.lc_issuing_bank ? currentRecord.lc_issuing_bank : "-")}</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
					
                    <div class="col-md-6">
                        <div class="master-detail-caption mb-2">History</div>
                        <div class="card card-mcs card-headline">
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="headline-detail-container">
                                        <div class="row mb-2">
                                            <div class="col-md-6">
                                                <small class="font-weight-normal text-muted d-block mb-1">Created By</small>
                                                <p class="font-weight-bold m-0">${currentRecord.record_created_by ?? "-"}</p>
                                            </div>
                                        </div>
                                        <div class="row mb-2">
                                            <div class="col-md-6">
                                                <small class="font-weight-normal text-muted d-block mb-1">Edited By</small>
                                                <p class="font-weight-bold m-0">${currentRecord.record_modified_by ?? "-"}</p>
                                            </div>
                                        </div>
                                        <div class="row mb-2">
                                            <div class="col-md-6">
                                                <small class="font-weight-normal text-muted d-block mb-1">Approved By</small>
                                                <p class="font-weight-bold m-0">${(currentRecord.approve_by ?? "-")}</p>
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-6">
                                                <small class="font-weight-normal text-muted d-block mb-1">Unapproved By</small>
                                                <p class="font-weight-bold m-0">${(currentRecord.disapprove_by ?? "-")}</p>
                                                <br/>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>

                </div>
            </div>
        `).appendTo(container)
    }
    // bob
    const renderSalesInvoiceTabs = function (currentRecord, container) {
        let prodSpecContainer = $("<div>")
        let attachmentsContainer = $("<div>")

        renderSalesInvoiceProductSpecification(currentRecord, prodSpecContainer)
        renderSalesInvoiceAttachment(currentRecord, attachmentsContainer)

        let tabContainer = $(`
            <ul class="nav nav-mcs nav-pills pills-blue mb-3" role="tablist">
                <li class="nav-item"><a class="nav-link active" data-toggle="pill" href="#prodspec-container-${currentRecord.id}"><i class="fas fa-address-card mr-2"></i>Contacts</a></li>
                <li class="nav-item"><a class="nav-link" data-toggle="pill" href="#attachments-container-${currentRecord.id}"><i class="fas fa-file mr-2"></i>Attachments</a></li>
            </ul>
            <div class="tab-content py-3">
                <div class="tab-pane fade show active" id="prodspec-container-${currentRecord.id}" role="tabpanel">
                </div>
                <div class="tab-pane fade" id="attachments-container-${currentRecord.id}" role="tabpanel">
                </div>
            </div>
        `).appendTo(container)

        prodSpecContainer.appendTo(tabContainer.find("#prodspec-container-" + currentRecord.id))
        attachmentsContainer.appendTo(tabContainer.find("#attachments-container-" + currentRecord.id))

    }

    // Sales Invoice Product Specification
    const renderSalesInvoiceProductSpecification = (currentRecord, container) => {
        var urlDetail = "/api/Sales/SalesInvoiceDetail";

        $("<div>")
            .addClass("master-detail-caption")
            .text("Sales Invoice Product Specification")
            .appendTo(container);

        $("<div>")
            .dxDataGrid({
                dataSource: DevExpress.data.AspNet.createStore({
                    key: "id",
                    loadUrl: urlDetail + "/BySalesInvoiceId/" + encodeURIComponent(currentRecord.id),
                    insertUrl: urlDetail + "/InsertData",
                    updateUrl: urlDetail + "/UpdateData",
                    deleteUrl: urlDetail + "/DeleteData",
                    onBeforeSend: function (method, ajaxOptions) {
                        ajaxOptions.xhrFields = { withCredentials: true };
                        ajaxOptions.beforeSend = function (request) {
                            request.setRequestHeader("Authorization", "Bearer " + token);
                        };
                    }
                }),
                remoteOperations: true,
                allowColumnResizing: true,
                dateSerializationFormat: "yyyy-MM-ddTHH:mm:ss",
                columns: [
                    {
                        dataField: "sales_invoice_id",
                        caption: "Sales Invoice",
                        allowEditing: false,
                        visible: false,
                        calculateCellValue: function () {
                            return currentRecord.id;
                        }
                    },
                    {
                        dataField: "analyte_id",
                        dataType: "string",
                        caption: "Analyte Definition",
                        allowEditing: false,
                        lookup: {
                            dataSource: DevExpress.data.AspNet.createStore({
                                key: "value",
                                loadUrl: "/api/Quality/Analyte/AnalyteIdLookup",
                                onBeforeSend: function (method, ajaxOptions) {
                                    ajaxOptions.xhrFields = { withCredentials: true };
                                    ajaxOptions.beforeSend = function (request) {
                                        request.setRequestHeader("Authorization", "Bearer " + token);
                                    };
                                }
                            }),
                            searchEnabled: true,
                            valueExpr: "value",
                            displayExpr: "text"
                        },
                    },
                    {
                        dataField: "analyte_standard_id",
                        dataType: "string",
                        caption: "Standard",
                        allowEditing: false,
                        lookup: {
                            dataSource: function (options) {
                                return {
                                    store: DevExpress.data.AspNet.createStore({
                                        key: "value",
                                        loadUrl: "/api/General/MasterList/MasterListIdLookup",
                                        onBeforeSend: function (method, ajaxOptions) {
                                            ajaxOptions.xhrFields = { withCredentials: true };
                                            ajaxOptions.beforeSend = function (request) {
                                                request.setRequestHeader("Authorization", "Bearer " + token);
                                            };
                                        }
                                    }),
                                    filter: ["item_group", "=", "analyte-standard"]
                                }
                            },
                            valueExpr: "value",
                            displayExpr: "text"
                        },
                    },
                    {
                        dataField: "value",
                        dataType: "number",
                        caption: "Value",
                        allowEditing: false
                    },
                    {
                        dataField: "target",
                        dataType: "number",
                        caption: "Target",
                        allowEditing: false
                    },
                    {
                        dataField: "created_on",
                        caption: "Created On",
                        dataType: "string",
                        visible: false,
                        sortOrder: "asc"
                    },
                ],
                filterRow: {
                    visible: true
                },
                headerFilter: {
                    visible: true
                },
                groupPanel: {
                    visible: true
                },
                searchPanel: {
                    visible: true,
                    width: 240,
                    placeholder: "Search..."
                },
                filterPanel: {
                    visible: true
                },
                filterBuilderPopup: {
                    position: { of: window, at: "top", my: "top", offset: { y: 10 } },
                },
                columnChooser: {
                    enabled: true,
                    mode: "select"
                },
                paging: {
                    pageSize: 10
                },
                pager: {
                    allowedPageSizes: [10, 20, 50, 100],
                    showNavigationButtons: true,
                    showPageSizeSelector: true,
                    showInfo: true,
                    visible: true
                },
                showBorders: true,
                editing: {
                    mode: "form",
                    allowAdding: false,
                    allowUpdating: false,
                    allowDeleting: false,
                    useIcons: true,
                    form: {
                        colCount: 2,
                        items: [
                            {
                                dataField: "analyte_id"
                            },
                            {
                                dataField: "analyte_standard_id"
                            },
                            {
                                dataField: "value"
                            },
                            {
                                dataField: "target"
                            },
                        ]
                    }
                },
                grouping: {
                    contextMenuEnabled: true,
                    autoExpandAll: false
                },
                rowAlternationEnabled: true,
                export: {
                    enabled: true,
                    allowExportSelectedData: true
                },
                onInitNewRow: function (e) {
                    e.data.sales_invoice_id = currentRecord.id;
                },
                onExporting: function (e) {
                    var workbook = new ExcelJS.Workbook();
                    var worksheet = workbook.addWorksheet(entityName);

                    DevExpress.excelExporter.exportDataGrid({
                        component: e.component,
                        worksheet: worksheet,
                        autoFilterEnabled: true
                    }).then(function () {
                        // https://github.com/exceljs/exceljs#writing-xlsx
                        workbook.xlsx.writeBuffer().then(function (buffer) {
                            saveAs(new Blob([buffer], { type: 'application/octet-stream' }), detailName + '.xlsx');
                        });
                    });
                    e.cancel = true;
                }
            }).appendTo(container);
    }

    // Sales Invoice Popup
    let salesInvoicePopupOptions = {
        title: "Invoice Detail",
        height: 750,
        //width: "95%",
        hideOnOutsideClick: true,
        contentTemplate: function () {

            var salesInvoiceDetailContainer = $("<div>")
            $(`<div class="mb-3">
                    <div class="row">
                        <div class="col-12">
                            <h4 class="font-weight-bold">Sales Invoice <span>${salesInvoiceData.invoice_number}</span></h4>
                        </div>
                        <div class="col-12">
                            <table class="sales-invoice-detail-table-js table table-striped mb-3">
                                <thead>
                                    <tr>
                                        <td><b>No.</b></td>
                                        <!-- <td><b>Invoice Type</b></td> -->
                                        <td style="width: 15%"><b>Invoice Item</b></td>
                                        <td style="width: 15%"><b>Type</b></td>
                                        <td><b>Qty</b></td>
                                        <td><b>Adj. Qty</b></td>
                                        <td><b>Price</b></td>
                                        <td><b>Adj. Price</b></td>
                                        <td style="width: 15%"><b>Value</b></td>
                                        <!-- <td style="width: 15%"><b>Sub Total Invoice</b></td> -->
                                    </tr>
                                </thead>
                                <tbody>
                                   <!-- <tr class="invoice-total-container">
                                        <td colspan="8"></td>
                                        <td><b>Total</b></td>
                                        <td class="invoice-total"></td>
                                    </tr>
                                    <tr class="down-payment-container">
                                        <td colspan="8"></td>
                                        <td><b>Payment</b></td>
                                        <td class="down-payment"></td>
                                    </tr> -->
                                    <!-- <tr class="freight-cost-container">
                                        <td colspan="8"></td>
                                        <td><b>Freight Cost</b></td>
                                        <td class="freight-cost"></td>
                                    </tr> -->
                                    <!-- <tr class="advance-payment-container">
                                        <td colspan="8"></td>
                                        <td><b>Advance Payment</b></td>
                                        <td class="advance-payment"></td>
                                    </tr> -->
                                </tbody>
                            </table>
                            <div class="row">
                                <div class="col-6">
                                    <div class="invoice-buyer-name-container mb-2" style="font-size: 16px">
                                        <span><b>Buyer</b> = </span>
                                        <span class="invoice-buyer-name"></span>
                                    </div>
                                    <div class="invoice-contract-name-container mb-2" style="font-size: 16px">
                                        <span><b>Sales Contract</b> = </span>
                                        <span class="invoice-contract-name"></span>
                                    </div>
                                    <div class="invoice-vessel-name-container mb-2" style="font-size: 16px">
                                        <span><b>Vessel</b> = </span>
                                        <span class="invoice-vessel-name"></span>
                                    </div>
                                    <div class="invoice-due-date-container mb-2" style="font-size: 16px">
                                        <span><b>Invoice Date</b> = </span>
                                        <span class="invoice-due-date"></span>
                                    </div>
                                </div>
                                <!-- <div class="col-6" style="text-align: right;">
                                     <button class="tombol-cetak-sales-invoice">
                                        <span class="ikon-cetak-container"></span>
                                        Print
                                    </button>
                                </div> -->

                                <div class="col-6" style="text-align: right; display: block;">
                                    <!-- <button id="btn-unapprove" class="btn btn-warning">
                                        <i class="fal fa-clipboard-check fa-lg"></i>
                                        Unapprove
                                    </button>
                                    <button id="btn-approve" class="btn btn-info">
                                        <i class="fal fa-clipboard-check fa-lg"></i>
                                        Approve
                                    </button> -->

                                    <button id="btn-request-approval" class="btn btn-info" style="display: none">
                                        <i class="fal fa-clipboard-check fa-lg"></i>
                                        Request Approval
                                    </button>
                                </div>

                            </div>
                        </div>
                    </div>
                </div>
            `).appendTo(salesInvoiceDetailContainer);

            salesInvoiceDetailContainer.dxScrollView({
                width: '100%',
                height: '100%',
            });

            // Get sales invoice detail data (sales invoice outline)
            let salesInvoiceDetailData = null
            $.ajax({
                url: "/api/Sales/SalesInvoice/SalesInvoiceOutline?sales_invoice_id=" + salesInvoiceData.id,
                type: "GET",
                contentType: "application/json",
                beforeSend: function (xhr) {
                    xhr.setRequestHeader("Authorization", "Bearer " + token);
                },
                success: function (response) {
                    salesInvoiceDetailData = response;
                    renderSalesInvoiceDetailTable(response, salesInvoiceDetailContainer);
                    $(".tombol-cetak-sales-invoice").on("click", () => { initializeCetak(salesInvoiceDetailData); });
                    const ikon_container = prt_getClassElm("ikon-cetak-container");
                    if (ikon_container) {
                        ikon_container.innerHTML = iconPrint;
                    }

                    if (salesInvoiceData.approve_status != "APPROVED")
                    //if (salesInvoiceData.approve_status == null)
                    {
                        $("#btn-request-approval").show();

                        $("#btn-request-approval").on("click", () => {
                            $.ajax({
                                url: url + "/RequestApproval/" + encodeURIComponent(salesInvoiceData.id),
                                type: 'GET',
                                headers: {
                                    "Authorization": "Bearer " + token
                                }
                            }).done(function (result) {
                                if (result != null) {
                                    alert("Email request saved!");
                                    location.reload();
                                }
                                else {
                                    alert("Data has been approved!");
                                    location.reload();
                                }
                            }).fail(function (jqXHR, textStatus, errorThrown) {
                                alert('Failed');
                            });
                        });
                    }

                    /* $("#btn-approve").on("click", () => {
                        $.ajax({
                            url: url + "/Finalisasi/" + encodeURIComponent(salesInvoiceData.id),
                            type: 'GET',
                            headers: {
                                "Authorization": "Bearer " + token
                            }
                        }).done(function (result) {
                            if (result != null) {
                                alert("Approve process success!");
                                location.reload();
                            }
                            else {
                                alert("Data has been approved!");
                                location.reload();
                            }
                        }).fail(function (jqXHR, textStatus, errorThrown) {
                            alert('Failed');
                        });
                    }); */

                },
                error: function (xhr, status, error) {
                    if (xhr.responseText.length > 0) {
                        alert(xhr.responseText)
                    }
                }
            });

            return salesInvoiceDetailContainer;
        }
    }
    var salesInvoicePopup = $("#invoice-popup").dxPopup(salesInvoicePopupOptions).dxPopup("instance")

    const showSalesInvoicePopup = function () {
        salesInvoicePopup.option("contentTemplate", salesInvoicePopupOptions.contentTemplate.bind(this));
        salesInvoicePopup.show()
    }

    const prt_getClassElm = (prt_className) => {
        return document.getElementsByClassName(prt_className)[0];
    }

    const iconPrint = `<svg xmlns="http://www.w3.org/2000/svg" height="24px" viewBox="0 0 24 24" width="24px" fill="#000000"><path d="M0 0h24v24H0V0z" fill="none"/><path d="M19 8h-1V3H6v5H5c-1.66 0-3 1.34-3 3v6h4v4h12v-4h4v-6c0-1.66-1.34-3-3-3zM8 5h8v3H8V5zm8 12v2H8v-4h8v2zm2-2v-2H6v2H4v-4c0-.55.45-1 1-1h14c.55 0 1 .45 1 1v4h-2z"/><circle cx="18" cy="11.5" r="1"/></svg>`;
    const iconLoading = `<svg class="prt-svg-spinner" width="30"  height="30"  xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" preserveAspectRatio="xMidYMid" class="lds-rolling" style="background: none;"><circle class="loader-svg" cx="50" cy="50" fill="none" stroke="var(--color)" stroke-width="var(--stroke-width)" r="35" stroke-dasharray="164.93361431346415 56.97787143782138" transform="rotate(159.821 50 50)"><animateTransform attributeName="transform" type="rotate" calcMode="linear" values="0 50 50;360 50 50" keyTimes="0;1" dur="0.6s" begin="0s" repeatCount="indefinite"></animateTransform></circle></svg>`;

    const initializeCetak = (all_datas) => {
        //console.log("Data yang akan dicetak", all_datas);
        //loadStyle('/css/displaymodes/mode-normal.css');
        loadStyle('/css/displaymodes/mode-printing.css');
        // <div class="prt-container elemen-cetak">
        //$(``).appendTo($("body")[0]);
        $(`
<div class="prt-container elemen-cetak">
<div class="prt-papersheet">
<div class="prt-paper-pad">
	<div class="prt-d-flex prt-fx-center">
		<div>
		<img src="data:image/png;base64," class="logo-adjust">
		</div>
		<div class="prt-full-w prt-right prt-bold">
		ORIGINAL
		</div>
	</div>
	<div class="prt-right prt-pad-1 prt-bold">

	</div>
	<div class="prt-center prt-bold prt-fsz-large prt-uline">
		Commercial Invoice
	</div>
	<div class="prt-sans prt-bold" style="padding-left: 1mm;">
		Page 1 of 2
	</div>
	<div class="prt-d-flex">
		<div style="width: 50%">
			<table>
				<tr>
					<td>
					<span class="prt-api prt-sans prt-bold prt_field_invoiceBuyerName">

					</span>
					</td>
				</tr>
			</table>
		</div>
		<div style="width: 50%" class="prt-sans">
			<table class="prt-bold">
				<tr>
					<td class="prt-right">No. :</td>
					<td class="prt-value">
					<span class="prt-api prt_field_invoiceName">
					</span>
					</td>
				</tr>
				<tr>
					<td class="prt-right">Date. :</td>
					<td class="prt-value">
					<span class="prt-api prt_field_dateOfIssue">
					</span>
					</td>
				</tr>
				<tr>
					<td class="prt-right">Laycan. :</td>
					<td class="prt-value">
						<span class="prt-api prt_field_laycan">
						</span>
					</td>
				</tr>
			</table>
		</div>
	</div>

	<div class="prt-vpad"></div>

	<div>
		<table class="prt-sans prt-fsz-small">
				<tr>
					<td class="prt-fsz-small prt-key-1">The Name of Vessel</td>
					<td class="prt-fsz-small prt-dot-1">:</td>
					<td class="prt-fsz-small prt-value prt-value-1">
						<span class="prt-api prt_field_invoiceVesselName">
						</span>
					</td>
				</tr>
				<tr>
					<td class="prt-fsz-small prt-key-1">Bill of Lading Number</td>
					<td class="prt-fsz-small prt-dot-1">:</td>
					<td class="prt-fsz-small prt-value prt-value-1">
						<span class="prt-api prt_field_invoiceBillOfLadingNumber">
						</span>
					</td>
				</tr>
				<tr>
					<td class="prt-fsz-small prt-key-1">Bill of Lading Date</td>
					<td class="prt-fsz-small prt-dot-1">:</td>
					<td class="prt-fsz-small prt-value prt-value-1">
						<span class="prt-api prt_field_invoiceBillOfLadingDate">
						</span>
					</td>
				</tr>
				<tr>
					<td class="prt-fsz-small prt-key-1">From</td>
					<td class="prt-fsz-small prt-dot-1">:</td>
					<td class="prt-fsz-small prt-value prt-value-1">
						<span class="prt-api prt_field_vesselFrom">
						</span>
					</td>
				</tr>
				<tr>
					<td class="prt-fsz-small prt-key-1">To</td>
					<td class="prt-fsz-small prt-dot-1">:</td>
					<td class="prt-fsz-small prt-value prt-value-1">
						<span class="prt-api prt_field_vesselTo">
						</span>
					</td>
				</tr>
				<tr>
					<td class="prt-fsz-small prt-key-1">Payment</td>
					<td class="prt-fsz-small prt-dot-1">:</td>
					<td class="prt-fsz-small prt-value prt-value-1">
						<span class="prt-api prt_field_invoicePayment">
						</span>
					</td>
				</tr>
				<tr>
					<td class="prt-fsz-small prt-key-1">Letter of credit number</td>
					<td class="prt-fsz-small prt-dot-1">:</td>
					<td class="prt-fsz-small prt-value prt-value-1">
						<span class="prt-api prt_field_letterOfCreditNumber">
						</span>
					</td>
				</tr>
				<tr>
					<td class="prt-fsz-small prt-key-1">Date of issue L/C</td>
					<td class="prt-fsz-small prt-dot-1">:</td>
					<td class="prt-fsz-small prt-value prt-value-1">
					<span class="prt-api prt_field_lcDateIssue">
					</span>
					</td>
				</tr>
				<tr>
					<td class="prt-fsz-small prt-key-1">Bank of Issuance L/C</td>
					<td class="prt-fsz-small prt-dot-1">:</td>
					<td class="prt-fsz-small prt-value prt-value-1">
					<span class="prt-api prt_field_lcIssuingBank">
					</span>
					</td>
				</tr>
				<tr>
					<td class="prt-fsz-small prt-key-1">Contract Number</td>
					<td class="prt-fsz-small prt-dot-1">:</td>
					<td class="prt-fsz-small prt-value prt-value-1">
					<span class="prt-api prt_field_contractNumber">
					</span>
					</td>
					<td class="prt-fsz-small prt-value prt-value-1">DATED</td>
					<td class="prt-fsz-small prt-dot-1">:</td>
					<td class="prt-fsz-small prt-value prt-value-1">
					<span class="prt-api prt_field_invoiceDueDate">
					</span>
					</td>

				</tr>
		</table>
	</div>

	<div class="prt-v-pad">
		<table class="prt-bold prt-sans prt-table-border prt-full-w">
			<tr>
				<th class="prt-fsz-small prt-head-2 prt-cola-1">
					DESCRIPTION OF GOODS
				</th>
				<th class="prt-fsz-small prt-head-2 prt-cola-2">
					QUANTITY SHIPPED
				</th>
				<th class="prt-fsz-small prt-head-2 prt-cola-3">
					UNIT PRICE
				</th>
				<th class="prt-fsz-small prt-head-2 prt-cola-4">
					TOTAL INVOICE AMOUNT
				</th>
			</tr>
			<tr>
				<td class="prt-fsz-small prt-value-2 prt-cola-1">
					INDONESIAN STEAM COAL BULK
				</td>
				<td class="prt-fsz-small prt-value-2 prt-cola-2 prt-right">
					<span class="prt-api prt_field_retVal__quantity">
					</span>
				</td>
				<td class="prt-fsz-small prt-value-2 prt-cola-3 prt-right">
					<span class="prt-api prt_field_retVal__price">
					</span>
				</td>
				<td class="prt-fsz-small prt-value-2 prt-cola-4 prt-right">
					<span class="prt-api prt_field_invoiceCurrencySymbol"></span> <span class="prt-api prt_field_retVal__value">
					</span>
				</td>
			</tr>
		</table>
	</div>

	<div class="prt-uline">
			<table class="prt-bold prt-full-w prt-sans prt-table-border prt_table_items">
					<tr>
						<th class="prt-fsz-small prt-bold prt-head-2 prt-uline">
							GUARANTEED
						</th>
						<th class="prt-fsz-small prt-bold prt-head-2 prt-uline">
							ACTUAL
						</th>
						<th class="prt-fsz-small prt-bold prt-head-2 prt-uline">
							ADJUSTMENT PRICE
						</th>
						<th class="prt-fsz-small prt-bold prt-head-2 prt-uline">
							TOTAL ADJUSTMENT
						</th>
					</tr>

				</table>
		</div>

	<!-- div class="prt-v-pad">
		<table class="prt-sans">
			<tr>
				<th class="prt-head-3 prt-uline prt-bold" colspan="2">
					Guaranteed
				</th>
				<th class="prt-head-3 prt-uline">
					Actual
				</th>
				<th class="prt-head-3 prt-uline prt-cola-3" colspan="2">
					FOBT Bonus/Penalty Adjusted (Price Adjustment):
				</th>
			</tr>
			<tr>
				<td class="prt-fsz-small prt-value-3 prt-cola-1">
					GCV
				</td>
				<td class="prt-fsz-small prt-value-3 prt-cola-2">
					4200 KCAL/KG
				</td>
				<td class="prt-fsz-small prt-value-3 prt-cola-3">
					.... KCAL/KG
				</td>
				<td class="prt-fsz-small prt-value-3 prt-cola-4">
					( USD ......... X ............ ) / 4200 KCAL/KG
				</td>
				<td class="prt-fsz-small prt-value-3 prt-cola-5">
					= USD. XXX.XX / MT
				</td>
			</tr>
		</table>
	</div -->

	<div class="prt-v-pad prt-right">
		Total Invoice Amount after adjustment is : <span class="prt-api prt_field_invoiceCurrencySymbol">
					</span>. <span class="prt-api prt_field_invoiceTotal">
					</span>
		<!-- table>
			<tr>
				<td class="prt-value-4">
					XX.XXX MT
				</td>
				<td class="prt-value-4">
					=
				</td>
			</tr>
		</table -->
	</div>

	<div class="prt-fsz-small prt-sans prt-v-pad">
		(<span class="prt-api prt_field_invoiceCurrencySymbol"></span>. AMOUNT IN WORDING : <span class="prt-api prt_field_generated__invoiceTotalWords"></span>)
	</div>

	<hr>

	<div class="prt-d-flex prt-sans">
		<div class="prt-note-styling prt-bd-blue prt-t-blue prt-fsz-small prt-m-r">
			<div class="prt-uline prt-bold">
				<div>Note :</div>
				<!-- div>
					Please pay to (In Full Amount)
				</div -->
			</div>
			<!-- div>
				<div>
					 PT. Indexim Coalindo (USD)XX
				</div>
				<table>
					<tr>
						<td class="prt-t-blue prt-fsz-small prt-colb-1">
							PT. Bank Ganesha
						</td>
						<td class="prt-t-blue prt-fsz-small prt-colb-2">
							BANK CORRESPONDENT : 
						</td>
					</tr>
					<tr>
						<td class="prt-t-blue prt-fsz-small prt-colb-1">
							A/C No 0910
						</td>
						<td class="prt-t-blue prt-fsz-small prt-colb-2">
							Bank Negara Indonesia, New York Agency
						</td>
					</tr>
					<tr>
						<td class="prt-t-blue prt-fsz-small prt-colb-1">
							Wisma Hayam Wuruk <br> J. Hayam
						</td>
						<td class="prt-t-blue prt-fsz-small prt-colb-1">
							Swift code: BNINUS33
						</td>
					</tr>
				</table>
			</div -->
			<div>
				<span class="prt-api prt_field_invoiceNotes">
					</span>
			</div>
		</div>
		<div style="width: 50%" class="prt-inline prt-m-l prt-mark-layout-1">
			<div>
				PT. INDEXIM COALINDO
			</div>
			<div class="prt-mark-layout-2">
				<div>BRILIANTO WIDRADJATI</div>
				<div>MANAGER</div>
			</div>
		</div>
	</div>
  </div>
  </div>
  <div class="prt-papersheet">
	<div class="prt-paper-pad">
		<div class="prt-d-flex prt-fx-center">
		<div>
		<img src="data:image/png;base64," class="logo-adjust">
		</div>
		<div class="prt-full-w prt-right prt-bold">
		ORIGINAL
		</div>
	</div>
	<div class="prt-sans prt-bold prt-v-pad">
		Attachment Page 2 of 2
	</div>

	<div class="prt-v-pad">
		<table class="prt-sans prt-bold">
			<tr>
				<td class="prt-key-3">Invoice No</td>
				<td class="prt-dot-3">:</td>
				<td class="prt-value prt-value-3">
					<span class="prt-api prt_field_invoiceName">
					</span>
				</td>
			</tr>
			<tr>
				<td class="prt-key-3">Date</td>
				<td class="prt-dot-3">:</td>
				<td class="prt-value prt-value-3">
					<span class="prt-api prt_field_dateOfIssue">
					</span>
				</td>
			</tr>
			<tr>
				<td class="prt-key-3">Laycan</td>
				<td class="prt-dot-3">:</td>
				<td class="prt-value prt-value-3">
					<span class="prt-api prt_field_laycan">
					</span>
				</td>
			</tr>
		</table>
	</div>

	<div class="prt-v-pad">
		<table class="prt-sans prt-fsz-small">
			<tr>
				<td class="prt-key-4">The Name of Vessel</td>
				<td class="prt-dot-4">:</td>
				<td class="prt-value prt-value-4">
				<span class="prt-api prt_field_invoiceVesselName">
						</span>
				</td>
			</tr>
			<tr>
				<td class="prt-key-4">Bill of Lading Number</td>
				<td class="prt-dot-4">:</td>
				<td class="prt-value prt-value-4">
				<span class="prt-api prt_field_invoiceBillOfLadingNumber">
						</span>
				</td>
			</tr>
			<tr>
				<td class="prt-fsz-small prt-key-3">Bill of Lading Date</td>
				<td class="prt-fsz-small prt-dot-3">:</td>
				<td class="prt-fsz-small prt-value prt-value-3">
					<span class="prt-api prt_field_invoiceBillOfLadingDate">
						</span>
				</td>
			</tr>
			<tr>
				<td class="prt-fsz-small prt-key-3">From</td>
				<td class="prt-fsz-small prt-dot-3">:</td>
				<td class="prt-fsz-small prt-value prt-value-3">
					<span class="prt-api prt_field_vesselFrom">
						</span>
				</td>
			</tr>
			<tr>
				<td class="prt-fsz-small prt-key-3">To</td>
				<td class="prt-fsz-small prt-dot-3">:</td>
				<td class="prt-fsz-small prt-value prt-value-3">
					<span class="prt-api prt_field_vesselTo">
						</span>
				</td>
			</tr>
			<tr>
				<td class="prt-fsz-small prt-key-3">Payment</td>
				<td class="prt-fsz-small prt-dot-3">:</td>
				<td class="prt-fsz-small prt-value prt-value-3">
					<span class="prt-api prt_field_invoicePayment">
						</span>
				</td>
			</tr>
			<tr>
				<td class="prt-fsz-small prt-key-3">Letter of credit number</td>
				<td class="prt-fsz-small prt-dot-3">:</td>
				<td class="prt-fsz-small prt-value prt-value-3">
				<span class="prt-api prt_field_letterOfCreditNumber">
						</span>
				</td>
			</tr>
			<tr>
				<td class="prt-fsz-small prt-key-3">Date of issue L/C</td>
				<td class="prt-fsz-small prt-dot-3">:</td>
				<td class="prt-fsz-small prt-value prt-value-3">
					<span class="prt-api prt_field_lcDateIssue">
					</span>
				</td>
			</tr>
			<tr>
				<td class="prt-fsz-small prt-key-3">Bank of Issuance L/C</td>
				<td class="prt-fsz-small prt-dot-3">:</td>
				<td class="prt-fsz-small prt-value prt-value-3">
					<span class="prt-api prt_field_lcIssuingBank">
					</span>
				</td>
			</tr>
			<tr>
				<td class="prt-fsz-small prt-key-3">Contract Number</td>
				<td class="prt-fsz-small prt-dot-3">:</td>
				<td class="prt-fsz-small prt-value prt-value-3">
				<span class="prt-api prt_field_contractNumber">
					</span>
				</td>
				<td class="prt-fsz-small prt-value prt-value-3">DATED</td>
				<td class="prt-fsz-small prt-dot-3">:</td>
				<td class="prt-fsz-small prt-value prt-value-3">
				<span class="prt-api prt_field_invoiceDueDate">
				</span>
				</td>
			</tr>
		</table>
	</div>

	<hr>

	<div class="prt-sans">
		<div class="prt-uline prt-bold">
			Cert. No : .............................
		</div>

		<!-- div class="prt-bold">
			THE CHARACTERISTICS LISTED BELOW IN ACCORDANCE WITH THE ISO STANDARD:
		</div -->
	</div>

</div>
</div>
</div>
`).appendTo($("body")[0]);

        const prt_salesInvoice = (prt_data) => {
            const prt_fn_table_items = function (tableData) {
                const prt_tableRef = prt_getClassElm("prt_table_items");
                let tableContent = '';
                for (let i = 0; i < tableData.length; i++) {
                    const tableElm = tableData[i];
                    // ${tableElm.invoice_item_type}
                    if (tableElm.invoice_type != 'tax' && tableElm.invoice_item_type != 'Initial Price') {
                        tableContent += `
					<tr>
					<td class="prt-fsz-small prt-colc-1"><span class="prt-api prt_field_table1-c1-r${i + 1}">${tableElm.invoice_item}</span></td>
					
					<td class="prt-fsz-small prt-colc-2 prt-right"><span class="prt-api  prt_field_table1-c2-r${i + 1}">${tableElm.actualValue}</span></td>
					
					<td class="prt-fsz-small prt-colc-3 prt-right "><span class="prt-api  prt_field_table1-c4-r${i + 1}">${tableElm.value}</span></td>
					
					<td class="prt-fsz-small prt-colc-4 prt-right"><span class="prt-api prt_field_table1-c5-r${i + 1}">${tableElm.total_invoice}</span></td>
					</tr>
					`;
                    }
                }
                prt_tableRef.insertAdjacentHTML('beforeend', tableContent);
            }

            const num2text = {
                ones: ['', 'ONE', 'TWO', 'THREE', 'FOUR', 'FIVE', 'SIX', 'SEVEN', 'EIGHT', 'NINE', 'TEN', 'ELEVEN', 'TWELVE', 'THIRTEEN', 'FOURTEEN', 'FIFTEEN', 'SIXTEEN', 'SEVENTEEN', 'EIGHTEEN', 'NINETEEN'],
                tens: ['', '', 'TWENTY', 'THIRTY', 'FOURTH', 'FIFTY', 'SIXTY', 'SEVENTY', 'EIGHTY', 'NINETY'],
                sep: ['', ' THOUSAND ', ' MILLION ', ' BILLION ', ' TRILLION ', ' QUADRILLION ', ' QUINTILLION ', ' SEXTILLION ']
            };
            const convert = function (val, unit) {
                if (val.length === 0) {
                    return '';
                }

                val = val.replace(/,/g, '');
                if (isNaN(val)) {
                    return 'Invalid input.';
                }


                let [val1, val2] = val.split(".")
                let str2 = "";
                if (val2 != null && val2 != '') {
                    //convert the decimals here
                    var digits = (val2 + "0").slice(0, 2).split("");
                    str2 = num2text.tens[+digits[0]] + " " + num2text.ones[+digits[1]]
                }
                let arr = [];
                while (val1) {
                    arr.push(val1 % 1000);
                    val1 = parseInt(val1 / 1000, 10);
                }
                let i = 0;
                let str = "";
                while (arr.length) {
                    str = (function (a) {
                        var x = Math.floor(a / 100),
                            y = Math.floor(a / 10) % 10,
                            z = a % 10;

                        return (x > 0 ? num2text.ones[x] + ' HUNDRED ' : '') +
                            (y >= 2 ? num2text.tens[y] + ' ' + num2text.ones[z] : num2text.ones[10 * y + z]);
                    })(arr.shift()) + num2text.sep[i++] + str;
                }

                return str +
                    ' ' + unit + ' ' +
                    (str2 ? ' AND ' + str2 + ' CENTS' : '') +
                    ' ';
            };

            const prt_fn_toWords = (prt_num, prt_unit) => {
                return convert(prt_num, prt_unit);
            }

            const prt_fn_getMainPrice = (prtInvoices) => {
                return prtInvoices.filter(prtx => prtx.invoice_item_type == "Initial Price")[0];
            }

            const prt_fn_fillAll = (prt_className, prt_fillData) => {
                const prt_classElements = document.getElementsByClassName(prt_className);
                for (let i = 0; i < prt_classElements.length; i++) {
                    prt_classElements[i].innerText = prt_fillData;
                }
            }

            const prt_getClassElm = (prt_className) => {
                return document.getElementsByClassName(prt_className)[0];
            }

            const prt_invoices = prt_data.retVal;
            prt_fn_fillAll("prt_field_laycan", prt_data.invoiceLaycanDate);
            prt_fn_fillAll("prt_field_invoiceVesselName", prt_data.invoiceVesselName);
            prt_fn_fillAll("prt_field_contractNumber", prt_data.salesContractNumber);
            prt_fn_fillAll("prt_field_vesselFrom", prt_data.vesselFrom);
            prt_fn_fillAll("prt_field_vesselTo", prt_data.vesselTo);
            prt_fn_fillAll("prt_field_invoiceBillOfLadingNumber", prt_data.invoiceBillOfLadingNumber);
            prt_fn_fillAll("prt_field_invoiceBillOfLadingDate", prt_data.invoiceBillOfLadingDate);
            prt_fn_fillAll("prt_field_invoicePayment", prt_data.invoicePayment);
            prt_fn_fillAll("prt_field_letterOfCreditNumber", prt_data.letterOfCreditNumber);
            prt_fn_fillAll("prt_field_lcDateIssue", prt_data.lcDateIssue);

            // prt_fn_fillAll("prt_field_invoiceDueDate", prt_data.invoiceDueDate);
            prt_fn_fillAll("prt_field_invoiceNotes", prt_data.invoiceNotes);
            prt_fn_fillAll("prt_field_invoiceCurrencySymbol", prt_data.invoiceCurrencySymbol);
            prt_fn_fillAll("prt_field_invoiceName", prt_data.invoiceName);
            prt_fn_fillAll("prt_field_dateOfIssue", prt_data.dateOfIssue);
            prt_fn_fillAll("prt_field_lcIssuingBank", prt_data.lcIssuingBank);
            // //console.log("elRef", prt_getClassElm("prt_field_issuedByUser"), prt_data.issuedByUser);
            prt_getClassElm("prt_field_invoiceTotal").innerText = prt_data.invoiceTotal;
            prt_getClassElm("prt_field_invoiceBuyerName").innerText = prt_data.invoiceBuyerName;
            prt_getClassElm("prt_field_generated__invoiceTotalWords").innerText = prt_fn_toWords(prt_data.invoiceTotal.toString(), prt_data.invoiceCurrencyName);

            prt_var_mainPrice = prt_fn_getMainPrice(prt_invoices);
            if (prt_var_mainPrice) {
                // //console.log("Initial Price", prt_var_mainPrice);
                prt_getClassElm("prt_field_retVal__quantity").innerText = prt_var_mainPrice.quantity;
                prt_getClassElm("prt_field_retVal__price").innerText = prt_var_mainPrice.price;
                prt_getClassElm("prt_field_retVal__value").innerText = prt_var_mainPrice.value;
            }
            else { alert("Main Price not found!"); }

            prt_fn_table_items(prt_invoices);
        }

        prt_salesInvoice(all_datas);

        const ikon_container = prt_getClassElm("ikon-cetak-container");
        if (ikon_container) {
            ikon_container.innerHTML = iconLoading;
        }

        setTimeout(() => {
            const ikon_container = prt_getClassElm("ikon-cetak-container");
            if (ikon_container) {
                ikon_container.innerHTML = iconPrint;
            }
            window.print();
        }, 2000);

        setTimeout(() => {
            loadStyle('/css/displaymodes/mode-normal.css');
            $(".elemen-cetak").remove();
        }, 4000)
    }

    const loadStyle = (letak) => {
        //console.log("masuk");
        const headTag = $("head")[0];
        const theme = $("#tema-css-klien")[0];
        if (theme) {
            theme.href = letak;
        }
        else {
            const styleElm = document.createElement('link');
            styleElm.href = letak;
            //'/css/components/mode-cetak.css'
            styleElm.rel = 'stylesheet';
            styleElm.id = 'tema-css-klien';
            $(styleElm).appendTo(headTag);
        }
    };

    const renderSalesInvoiceDetailTable = (data, container) => {
        let table = container.find(".sales-invoice-detail-table-js")
        let tbody = table.find("tbody")
        let invoiceTotal = container.find(".invoice-total")
        let invoiceDownPayment = container.find(".down-payment")
        let invoiceBuyerName = container.find(".invoice-buyer-name")
        let invoiceContractName = container.find(".invoice-contract-name")
        let invoiceVesselName = container.find(".invoice-vessel-name")
        let invoiceDueDate = container.find(".invoice-due-date")
        let invoiceCreditLimit = container.find(".invoice-credit-limit")
        let invoiceFreightCost = container.find(".freight-cost")
        let invoiceAdvancePayment = container.find(".advance-payment")

        let dataElements = []

        let i = 1
        data.retVal.forEach(d => {
            if (d.invoice_type == "tax") {
                shownValue = d.value
                if (shownValue < 0) {
                    // 20220803 hide minus percent
                    // shownValue = shownValue * -1 * 100
                    shownValue = shownValue * -1
                }
                //shownTotalTax = d.total_invoice
                //if (shownTotalTax< 0) {
                //    shownTotalTax = shownTotalTax * -1
                //}

                dataElements.push(`<tr>
                                    <td>&nbsp;</td>
                                    <!-- <td>&nbsp;</td> -->
                                    <td>&nbsp;</td>
                                    <td>&nbsp;</td>
                                    <td>&nbsp;</td>
                                    <td>&nbsp;</td>
                                    <td>${d.invoice_item_type}</td>
                                    <td>${formatNumber(shownValue) + "%"}</td>
                                    <td>${(data.invoiceCurrencySymbol ? data.invoiceCurrencySymbol : "Rp") + " " + formatNumber(d.total_invoice)}</td>
                                    <!--<td>&nbsp;</td>-->
                                </tr>`)
            } else if (d.invoice_type == "subtotal") {
                shownValue = d.value
                if (shownValue < 0) {
                    shownValue = shownValue * -1
                }
                //shownTotalTax = d.total_invoice
                //if (shownTotalTax< 0) {
                //    shownTotalTax = shownTotalTax * -1
                //}
                dataElements.push(`<tr>
                                    <td>&nbsp;</td>
                                    <!-- <td>&nbsp;</td> -->
                                    <td>&nbsp;</td>
                                    <td>&nbsp;</td>
                                    <td>&nbsp;</td>
                                    <td>&nbsp;</td>
                                    <td>${d.invoice_item_type}</td>
                                    <!--<td>${formatNumber(shownValue) + "===>"}</td> -->
                                    <td>===============</td>
                                    <td>${(data.invoiceCurrencySymbol ? data.invoiceCurrencySymbol : "Rp") + " " + formatNumber(d.total_invoice)}</td>
                                    <!--<td>&nbsp;</td>-->
                                </tr>`)

            } else if (d.invoice_type == "total") {
                shownValue = d.value
                if (shownValue < 0) {
                    shownValue = shownValue * -1
                }
                //shownTotalTax = d.total_invoice
                //if (shownTotalTax< 0) {
                //    shownTotalTax = shownTotalTax * -1
                //}
                dataElements.push(`<tr>
                                    <td>&nbsp;</td>
                                    <!-- <td>&nbsp;</td> -->
                                    <td>&nbsp;</td>
                                    <td>&nbsp;</td>
                                    <td>&nbsp;</td>
                                    <td>&nbsp;</td>
                                    <td>${d.invoice_item_type}</td>
                                    <!--<td>${formatNumber(shownValue) + "===>"}</td> -->
                                    <td>===============</td>
                                    <td>${(data.invoiceCurrencySymbol ? data.invoiceCurrencySymbol : "Rp") + " " + formatNumber(d.total_invoice)}</td>
                                    <!--<td>&nbsp;</td>-->
                                </tr>`)

            } else
            //20220803 bobby - hide total invoice 
            {
                dataElements.push(`<tr>
                                    <td>${i}</td>
                                    <!-- <td>${d.invoice_type}</td> -->
                                    <td>${d.invoice_item}</td>
                                    <td>${d.invoice_item_type}</td>
                                    <td>${formatNumber(d.quantity)}</td>
                                    <td>${formatNumber(d.adjustment_quantity)}</td>
                                    <td>${formatNumber(d.price)}</td>
                                    <td>${formatNumber(d.adjustment_price)}</td>
                                    <td>${(data.invoiceCurrencySymbol ? data.invoiceCurrencySymbol : "Rp") + " " + formatNumber(d.value)}</td>
                                    <!-- <td>${(data.invoiceCurrencySymbol ? data.invoiceCurrencySymbol : "Rp") + " " + formatNumber(d.total_invoice)} --></td>
                                </tr>`)
                i++
            }
        })

        tbody.prepend(dataElements.join(''))
        invoiceTotal.html((data.invoiceCurrencySymbol ? data.invoiceCurrencySymbol : "Rp") + " " + formatNumber(data.invoiceTotal))
        invoiceDownPayment.html((data.invoiceCurrencySymbol ? data.invoiceCurrencySymbol : "Rp") + " " + formatNumber(data.invoiceDownPayment))
        invoiceDueDate.html(data.invoiceDueDate ? moment(data.invoiceDueDate).format("D MMMM YYYY") : '-')
        invoiceBuyerName.html(data.invoiceBuyerName || '-')
        invoiceVesselName.html(data.invoiceVesselName || '-')
        invoiceContractName.html(data.invoiceContractName || '-')
        invoiceFreightCost.html((data.invoiceCurrencySymbol ? data.invoiceCurrencySymbol : "Rp") + " " + formatNumber(data.invoiceFreightCost))
        invoiceAdvancePayment.html((data.invoiceCurrencySymbol ? data.invoiceCurrencySymbol : "Rp") + " " + formatNumber(data.advance_payment))

        salesInvoicePopup.repaint()

    }

    //bob
    let salesInvoiceAttachmentDataGrid
    const renderSalesInvoiceAttachment = function (currentRecord, container) {
        let attachmentUrlDetail = "/api/Sales/SalesInvoiceAttachment";
        let salesInvoiceAttachmentsContainer = $("<div class='mb-5'>")
        salesInvoiceAttachmentsContainer.appendTo(container)

        let titleContainer = $(`
            <div class="row mb-3 align-items-center">
                <div class="col-md-6">
                    <div class="master-detail-caption">File Attachments</div>
                </div>
            </div>
        `).appendTo(salesInvoiceAttachmentsContainer)

        salesInvoiceAttachmentDataGrid = $("<div>")
            .dxDataGrid({
                dataSource: DevExpress.data.AspNet.createStore({
                    key: "id",
                    loadUrl: attachmentUrlDetail + "/BySalesInvoiceId/" + encodeURIComponent(currentRecord.id),
                    insertUrl: attachmentUrlDetail + "/InsertData",
                    updateUrl: attachmentUrlDetail + "/UpdateData",
                    deleteUrl: attachmentUrlDetail + "/DeleteData",
                    onBeforeSend: function (method, ajaxOptions) {
                        ajaxOptions.xhrFields = { withCredentials: true };
                        ajaxOptions.beforeSend = function (request) {
                            request.setRequestHeader("Authorization", "Bearer " + token);
                        };
                    }
                }),
                remoteOperations: true,
                allowColumnResizing: true,
                columnResizingMode: "widget",
                dateSerializationFormat: "yyyy-MM-ddTHH:mm:ss",
                columns: [
                    {
                        dataField: "sales_invoice_id",
                        dataType: "string",
                        caption: "Sales Invoice ID",
                        formItem: {
                            colSpan: 2
                        },
                        allowEditing: false,
                        visible: false,
                        calculateCellValue: function () {
                            return currentRecord.id;
                        }
                    },
                    {
                        dataField: "filename",
                        dataType: "string",
                        caption: "File name",
                        formItem: {
                            colSpan: 2
                        },
                        cellTemplate: function (container, options) {
                            let attachmentUrl = options.value
                            let attachmentName = /[^\\]*$/.exec(attachmentUrl)[0] // Get only the file name and its extension

                            $(`<span><i class="fas fa-file mr-2"></i>${attachmentName}</span>`).appendTo(container)
                        }

                    },
                    {
                        caption: "Download",
                        type: "buttons",
                        width: 100,
                        buttons: [{
                            cssClass: "btn-dxdatagrid",
                            hint: "Download attachment",
                            text: "Download",
                            onClick: function (e) {
                                // Download file from Ajax. Ref: https://stackoverflow.com/a/9970672
                                let attachment = e.row.data
                                let attachmentName = /[^\\]*$/.exec(attachment.filename)[0]

                                let xhr = new XMLHttpRequest()
                                xhr.open("GET", "/api/Sales/SalesInvoiceAttachment/DownloadDocument/" + attachment.id, true)
                                xhr.responseType = "blob"
                                xhr.setRequestHeader("Authorization", "Bearer " + token)

                                xhr.onload = function (e) {
                                    let blobURL = window.webkitURL.createObjectURL(xhr.response)

                                    let a = document.createElement("a")
                                    a.href = blobURL
                                    a.download = attachmentName
                                    document.body.appendChild(a)
                                    a.click()
                                };

                                xhr.send()
                            }
                        }]
                    },
                    {
                        type: "buttons",
                        buttons: ["edit", "delete"]
                    }
                ],
                onToolbarPreparing: function (e) {
                    let toolbarItems = e.toolbarOptions.items;

                    // Modifies an existing item
                    toolbarItems.forEach(function (item) {
                        if (item.name === "addRowButton") {
                            item.options = {
                                icon: "plus",
                                onClick: function (e) {
                                    openAddAttachmentPopup()
                                }
                            }
                        }
                    });
                },
                filterRow: {
                    visible: true
                },
                headerFilter: {
                    visible: true
                },
                groupPanel: {
                    visible: true
                },
                searchPanel: {
                    visible: true,
                    width: 240,
                    placeholder: "Search..."
                },
                filterPanel: {
                    visible: true
                },
                filterBuilderPopup: {
                    position: { of: window, at: "top", my: "top", offset: { y: 10 } },
                },
                columnChooser: {
                    enabled: true,
                    mode: "select"
                },
                paging: {
                    pageSize: 10
                },
                pager: {
                    allowedPageSizes: [10, 20, 50, 100],
                    showNavigationButtons: true,
                    showPageSizeSelector: true,
                    showInfo: true,
                    visible: true
                },
                showBorders: true,
                editing: {
                    mode: "form",
                    allowAdding: true,
                    allowUpdating: false,
                    allowDeleting: true,
                    useIcons: true
                },
                grouping: {
                    contextMenuEnabled: true,
                    autoExpandAll: false
                },
                rowAlternationEnabled: true,
                export: {
                    enabled: true,
                    allowExportSelectedData: true
                },
                onInitNewRow: function (e) {
                    e.data.sales_invoice_id = currentRecord.id;
                },
                onExporting: function (e) {
                    var workbook = new ExcelJS.Workbook();
                    var worksheet = workbook.addWorksheet(entityName);

                    DevExpress.excelExporter.exportDataGrid({
                        component: e.component,
                        worksheet: worksheet,
                        autoFilterEnabled: true
                    }).then(function () {
                        // https://github.com/exceljs/exceljs#writing-xlsx
                        workbook.xlsx.writeBuffer().then(function (buffer) {
                            saveAs(new Blob([buffer], { type: 'application/octet-stream' }), detailName + '.xlsx');
                        });
                    });
                    e.cancel = true;
                }
            }).appendTo(salesInvoiceAttachmentsContainer).dxDataGrid("instance");
    }

    const addAttachmentPopupOptions = {
        width: 500,
        height: "auto",
        showTitle: true,
        title: "Add Attachment",
        visible: false,
        dragEnabled: false,
        hideOnOutsideClick: true,
        contentTemplate: function () {
            let salesInvoiceIdInput =
                $("<div>")
                    .dxTextBox({
                        name: "sales_invoice_id",
                        value: salesInvoiceData.id,
                        readOnly: true,
                        visible: false
                    })

            let attachmentInput =
                $("<div class='mb-5 dx-fileuploader-mcs'>")
                    .dxFileUploader({
                        uploadMode: "useForm",
                        multiple: false,
                        maxFileSize: maxFileSize,
                        invalidMaxFileSizeMessage: "Max. file size is 50 Mb"
                    })

            let submitButton =
                $("<div>")
                    .dxButton({
                        text: "Submit",
                        onClick: function (e) {
                            let salesInvoiceId = salesInvoiceIdInput.dxTextBox("instance").option("value")
                            let file = attachmentInput.dxFileUploader("instance").option("value")[0]

                            var reader = new FileReader();
                            reader.readAsDataURL(file);
                            reader.onload = function () {
                                let fileName = file.name
                                let fileSize = file.size
                                let data = reader.result.split(',')[1]

                                if (fileSize >= maxFileSize) {
                                    return;
                                }

                                let formData = {
                                    "salesInvoiceId": salesInvoiceId,
                                    "fileName": fileName,
                                    "fileSize": fileSize,
                                    "data": data
                                }

                                $.ajax({
                                    url: "/api/Sales/SalesInvoiceAttachment/InsertData",
                                    data: JSON.stringify(formData),
                                    type: "POST",
                                    contentType: "application/json",
                                    beforeSend: function (xhr) {
                                        xhr.setRequestHeader("Authorization", "Bearer " + token);
                                    },
                                    success: function (response) {
                                        addAttachmentPopup.hide()
                                        salesInvoiceAttachmentDataGrid.refresh()
                                    }
                                })
                            }
                        }
                    })

            let formContainer = $("<form enctype='multipart/form-data'>")
                .append(salesInvoiceIdInput, attachmentInput, submitButton)

            return formContainer;
        }
    }

    const addAttachmentPopup = $("<div>")
        .dxPopup(addAttachmentPopupOptions).appendTo("body").dxPopup("instance")

    const openAddAttachmentPopup = function () {
        addAttachmentPopup.option("contentTemplate", addAttachmentPopupOptions.contentTemplate.bind(this));
        addAttachmentPopup.show()
    }
});