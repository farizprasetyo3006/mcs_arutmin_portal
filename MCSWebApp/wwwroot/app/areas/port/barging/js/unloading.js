$(function () {
    ...
    var unloadingQuantityBeforeEdit;
    var id = null;
    ...
    $("#grid").dxDataGrid({
        ...
        columns: [
            ...
            },
            {
                ...
                lookup: {
                    dataSource: DevExpress.data.AspNet.createStore({
                        key: "value",
                        loadUrl: url + "/Unloading/AccountingPeriodIdLookup",
                        onBeforeSend: function (method, ajaxOptions) {
                            ajaxOptions.xhrFields = { withCredentials: true };
                            ajaxOptions.beforeSend = function (request) {
                                request.setRequestHeader("Authorization", "Bearer " + token);
                    ...
                    dataSource: function (options) {
                        return {
                            store: DevExpress.data.AspNet.createStore({
                                key: "value",
                                loadUrl: isInserting ? url + "/DespatchOrderIdLookupunloading?id=" + id : url + "/Unloading/DespatchOrderIdLookup",
                                onBeforeSend: function (method, ajaxOptions) {
                                    ajaxOptions.xhrFields = { withCredentials: true };
                                    ajaxOptions.beforeSend = function (request) {
                                        request.setRequestHeader("Authorization", "Bearer " + token);
            ...
            },
            {
                ...
                lookup: {
                    dataSource: DevExpress.data.AspNet.createStore({
                        key: "value",
                        loadUrl: url + "/Unloading/ProductIdLookup",
                        onBeforeSend: function (method, ajaxOptions) {
                            ajaxOptions.xhrFields = { withCredentials: true };
                            ajaxOptions.beforeSend = function (request) {
                                request.setRequestHeader("Authorization", "Bearer " + token);
            ...
            },
            {
                ...
                lookup: {
                    dataSource: DevExpress.data.AspNet.createStore({
                        key: "value",
                        loadUrl: url + "/Unloading/EquipmentIdLookup",
                        onBeforeSend: function (method, ajaxOptions) {
                            ajaxOptions.xhrFields = { withCredentials: true };
                            ajaxOptions.beforeSend = function (request) {
                                request.setRequestHeader("Authorization", "Bearer " + token);
                ...
                lookup: {
                    dataSource: function (options) {
                        return {
                            store: DevExpress.data.AspNet.createStore({
                                key: "value",
                                loadUrl: url + "/Unloading/QualitySamplingIdLookup",
                                onBeforeSend: function (method, ajaxOptions) {
                                    ajaxOptions.xhrFields = { withCredentials: true };
                                    ajaxOptions.beforeSend = function (request) {
                                        request.setRequestHeader("Authorization", "Bearer " + token);
            ...
            },
            {
                ...
                lookup: {
                    dataSource: DevExpress.data.AspNet.createStore({
                        key: "value",
                        loadUrl: url + "/Unloading/DraftSurveyIdLookup",
                        onBeforeSend: function (method, ajaxOptions) {
                            ajaxOptions.xhrFields = { withCredentials: true };
                            ajaxOptions.beforeSend = function (request) {
                                request.setRequestHeader("Authorization", "Bearer " + token);
            ...
            },
            {
                ...
                lookup: {
                    dataSource: DevExpress.data.AspNet.createStore({
                        key: "value",
                        loadUrl: url + "/Unloading/BusinessUnitIdLookup",
                        onBeforeSend: function (method, ajaxOptions) {
                            ajaxOptions.xhrFields = { withCredentials: true };
                            ajaxOptions.beforeSend = function (request) {
                                request.setRequestHeader("Authorization", "Bearer " + token);
        ...
        onEditingStart: function (e) {
            isInserting = true;
            id = e.data.despatch_order_id;
            unloadingQuantityBeforeEdit = e.data.quantity;
        },
                ...
                if (!skipValidationCheck && (e.newData.source_location_id != undefined || e.newData.source_location_id != undefined || e.newData.quantity != undefined)) {
                    ...
                    if (e.oldData === undefined) {
                        ...
                        unloadingSourceLocationId = e.newData.source_location_id == undefined ? e.oldData.source_location_id : e.newData.source_location_id;
                         unloadingQuantity = e.newData.quantity !== undefined ? e.newData.quantity : unloadingQuantityBeforeEdit;
                    }
                            ...
                            if (result.data == null) {
                              //  Swal.fire("Error !", 'Cannot find Barge Loading Data.', "error");
                                let popup = $("#popupContainer").dxPopup({
                                    visible: true,
                                    showTitle: true,
                                    width: 250, // Set the width to a smaller value
                                    height: 150,
                                    title: "Error",
                                    contentTemplate: function (contentElement) {
                                        $("<p>").text("Cannot find Barge Loading Data.").appendTo(contentElement);
                                        $("<div>")
                                            .dxButton({
                                                text: "OK",
                                                onClick: function () {
                                                    popup.dispose(); // Close and dispose the popup
                                                }
                                            })
                                ...
                                /*var errorText = "Cannot find Barge Loading Data."
                                ...
                                let onePercent = quantity * 0.01;
                                if (unloadingQuantity > quantity + onePercent || unloadingQuantity < quantity - onePercent) {
                                    //Swal.fire({
                                    ...
                                    //    }
                                    //});
                                    var text = "Loading Quantity and Unloading Quantity are not within 1% range of each other!\nLoading Quantity : " + quantity + "\n\nDo you want to Save ?";
                ...
                e.editorOptions.onValueChanged = function (e) { // Overiding the standard handler
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            grid.cellValue(index, "source_location_id", record.transportId);
                            grid.cellValue(index, "product_id", record.productName);
                        }
                    }),
                ...
                e.editorOptions.onValueChanged = function (e) { // Overiding the standard handler
                    ...
                    $.ajax({
                        ...
                        success: function (response) {
                            ...
                            grid.cellValue(index, "destination_location_id", record.discharge_port)
                    ...
                    $.ajax({
                        url: '/api/Port/Barging/BargingLoadingByDespatchOrder?Id=' + despatchOrderId,
                        type: 'GET',
                        contentType: "application/json",
                        beforeSend: function (xhr) {
                            xhr.setRequestHeader("Authorization", "Bearer " + token);
                        ...
                        success: function (response) {
                            let record = response.data;
                            // Set its corresponded field's value 
                            grid.cellValue(index, "bl_date", record.end_datetime);
                            grid.cellValue(index, "bl_quantity", record.quantity);
                        }
                    })
                    standardHandler(e) // Calling the standard handler to save the edited value
                }
    ...
    function createDetailTab(masterDetailData) {
        return function () {
            let currentRecord = masterDetailData;
            let detailName = "Barging/Unloading";
            let urlDetail = "/api/" + areaName + "/" + detailName;
            ...
            return $("<div>")
                .dxDataGrid({
                    dataSource: DevExpress.data.AspNet.createStore({
                        key: "id",
                        loadUrl: urlDetail + "/GetUnItemsById?Id=" + encodeURIComponent(currentRecord.id),
                        insertUrl: urlDetail + "/InsertUnItemData",
                        updateUrl: urlDetail + "/UpdateUnItemData",
                        deleteUrl: urlDetail + "/DeleteUnItemData",
    ...
    $('#btnDownloadSelectedRow').on('click', function () {
        if (selectedIds != null && selectedIds != '') {
            ...
            $.ajax({
                url: "/Port/Barging/UnloadingExcelExport",
                type: 'POST',
                cache: false,
                contentType: "application/json",
                ...
                if (data instanceof Blob) {
                    ...
                    a.href = url;
                    a.download = "Barging_Unloading.xlsx"; // Set the appropriate file name here
                    document.body.appendChild(a);
                    a.click();
