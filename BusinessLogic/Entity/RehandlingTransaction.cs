...
{
    public partial class RehandlingTransaction: ServiceRepository<rehandling_transaction, vw_rehandling_transaction>
    {
        private static readonly Logger logger = LogManager.GetCurrentClassLogger();

        private readonly UserContext userContext;

        public RehandlingTransaction(UserContext userContext)
            : base(userContext.GetDataContext())
        {
            this.userContext = userContext;
        }

        public static async Task<StandardResult> UpdateStockState(string ConnectionString, rehandling_transaction TransactionRecord)
        {
            ...
            try
            {
                ...
                using (var tx = db.GetTransaction())
                {
                    try
                    {
                        ...
                        {
                            var record = await db.FirstOrDefaultAsync<rehandling_transaction>(
                            ...
                            if(record != null)
                            {
                                ...
                                {
                                    sourceStockState.transaction_datetime = record.loading_datetime;
                                }
                                ...
                                var prevSourceStockState = await db.FirstOrDefaultAsync<stock_state>(
                                    ...
                                    record.source_location_id, record.loading_datetime);
                                ...
                                {
                                    destinationStockState.transaction_datetime = record.loading_datetime;
                                }
                                logger.Debug(db.LastCommand);
                                ...
                                var prevDestinationStockState = await db.FirstOrDefaultAsync<stock_state>(
                                    "WHERE stock_location_id = @0 AND transaction_datetime < @1 ORDER BY transaction_datetime DESC",
                                    record.destination_location_id, record.loading_datetime);
        ...
        public static async Task<StandardResult> UpdateStockStateAnalyte(string ConnectionString, rehandling_transaction TransactionRecord)
        {
            var result = new StandardResult();
